...
```bash
docker pull swr.cn-south-1.myhuaweicloud.com/ascendhub/clusterd:v26.1.0
```
...
```bash
docker pull swr.cn-south-1.myhuaweicloud.com/ascendhub/ascend-k8sclusterd:{version}
```

2. 修改镜像标签

```bash
docker tag swr.cn-south-1.myhuaweicloud.com/ascendhub/ascend-k8sclusterd:{version} ascend-k8sclusterd:{tag}
```
...
```bash
kubectl get pods -n kube-system | grep clusterd
```
