...
```bash
docker pull swr.cn-south-1.myhuaweicloud.com/ascendhub/infer-operator:v26.1.0
```
...
```bash
docker pull swr.cn-south-1.myhuaweicloud.com/ascendhub/infer-operator:{version}
```

2. 修改镜像标签

```bash
docker tag swr.cn-south-1.myhuaweicloud.com/ascendhub/infer-operator:{version} infer-operator:{tag}
```
...
```bash
kubectl get pods -n kube-system | grep infer-operator
```
