...
function build() {
  ...
  ls ${OUTPUT_NAME}
  if [ $? -ne 0 ]; then
    echo "fail to find sriov-cni"
    exit 1
  fi
}

function mv_file() {
  mv "${TOP_DIR}"/cmd/sriov/${OUTPUT_NAME} "${TOP_DIR}"/output
  cp "${TOP_DIR}"/build/sriov-cni.yaml "${TOP_DIR}"/output/sriov-cni-"${build_version}".yaml
  ...
  sed -i "s/sriov-cni:.*/sriov-cni:${build_version}/" "${TOP_DIR}"/output/sriov-cni-"${build_version}".yaml
  sed -i "s/SriovCni Version .*/SriovCni Version ${build_version}/" "${TOP_DIR}"/output/agreement.txt
