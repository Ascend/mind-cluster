...
class TestGetOpticalMetric(unittest.TestCase):
    ...
    def test_typed_threshold_preferred(self):
        ...
        self.assertIs(A5Threshold.get_optical_metric("TX_BIAS_MA", "LPO"), A5Threshold.LPO_TX_BIAS_MA)

    def test_odsp_falls_back_to_base(self):
        """ODSP 为默认类型，直接使用基础阈值（未定义 ODSP_ 前缀阈值）"""
        self.assertIs(A5Threshold.get_optical_metric("TX_BIAS_MA", "ODSP"), A5Threshold.TX_BIAS_MA)
        self.assertIs(A5Threshold.get_optical_metric("TX_POWER_DBM", "odsp"), A5Threshold.TX_POWER_DBM)
    ...
    def test_non_lpo_types_fall_back_to_base(self):
        ...
        for unexpected_type in ("", " ", None, "UNKNOWN", "unknown", "810"):
            self.assertIs(
                A5Threshold.get_optical_metric("TX_BIAS_MA", unexpected_type),
                A5Threshold.TX_BIAS_MA,
    ...
    def test_fallback_without_type(self):
        ...
        self.assertIs(A5Threshold.get_optical_metric("TX_BIAS_MA", ""), A5Threshold.TX_BIAS_MA)
        self.assertIs(A5Threshold.get_optical_metric("TX_BIAS_MA", None), A5Threshold.TX_BIAS_MA)

    def test_fallback_when_typed_not_defined(self):
        """该类型未定义 {TYPE}_{METRIC} 阈值时回退基础阈值（指标扩展无需全类型覆盖）"""
        self.assertIs(BaseThreshold.get_optical_metric("CDR_HOST_SNR_DB", "LPO"), A5Threshold.CDR_HOST_SNR_DB)

    def test_optical_view_attribute_access(self):
        """get_optical_view 返回的视图属性访问与 get_optical_metric 等价"""
        view = A5Threshold.get_optical_view("LPO")
        self.assertIs(view.TX_POWER_DBM, A5Threshold.LPO_TX_POWER_DBM)
...
class TestTypedThresholdInDiagnosis(unittest.TestCase):
    ...
    def test_abnormal_bias_by_optical_type(self):
        ...
        original = A5Threshold.LPO_TX_BIAS_MA
        A5Threshold.LPO_TX_BIAS_MA = Threshold(low_value_alarm="0.4", desc="lpo tx bias", unit="mA")
        try:
            ...
            self.assertEqual(module_lpo.get_abnormal_bias_infos(A5Threshold), "")
            self.assertIn("Lane0", module_odsp.get_abnormal_bias_infos(A5Threshold))
            self.assertIn("Lane0", module_none.get_abnormal_bias_infos(A5Threshold))
        finally:
            A5Threshold.LPO_TX_BIAS_MA = original

    def test_abnormal_snr_by_optical_type(self):
        """host/media SNR 检查：LPO 不支持 SNR（未定义 LPO_ 前缀 SNR 阈值），一律回退基础阈值"""
        self.assertIs(A5Threshold.get_optical_metric("HOST_SNR_DB", "LPO"), A5Threshold.HOST_SNR_DB)
        ...
        self.assertIn("Lane0", module_lpo.get_abnormal_snr_infos(A5Threshold))
        self.assertIn("Lane0", module_odsp.get_abnormal_snr_infos(A5Threshold))
...
class TestTypedThresholdJsonOverride(unittest.TestCase):
    ...
    def test_json_override_typed_threshold(self):
        ...
        self.assertEqual(A5Threshold.LPO_TX_BIAS_MA.high_alarm_th, "1")
        view = A5Threshold.get_optical_view("LPO")
        self.assertEqual(view.TX_BIAS_MA.high_alarm_th, "1")
        # 未配置项保持默认（ODSP 用基础阈值）
        self.assertEqual(A5Threshold.TX_BIAS_MA.high_alarm_th, "13.00")
    ...
    def test_json_override_applies_to_diagnosis(self):
        ...
        self.assertIn("Lane0", module_lpo.get_abnormal_bias_infos(A5Threshold))
        module_odsp = OpticalModuleInfo(lane_power_infos=[LanePowerInfo("0", bias="7.5")], optical_type="ODSP")
        self.assertEqual(module_odsp.get_abnormal_bias_infos(A5Threshold), "")
    ...
    def test_reset_restores_typed_default(self):
        ...
        self._loader.parse(self._tmp_dir)
        self._loader.apply("A5")
        self.assertEqual(A5Threshold.LPO_TX_BIAS_MA.low_alarm_th, "1")
        # 空配置重新应用后恢复代码默认值
        os.remove(os.path.join(self._tmp_dir, CONFIG_FILE))
        self._loader.parse(self._tmp_dir)
        self._loader.apply("A5")
        self.assertEqual(A5Threshold.LPO_TX_BIAS_MA.low_alarm_th, "4.00")
...
class TestReportTypedThresholdConfig(unittest.TestCase):
    ...
    def test_lpo_row_uses_lpo_threshold(self):
        config = ThresholdConfig.for_optical_metric(A5Threshold, "TX_BIAS_MA", "host_tx_bias0", "主机侧TX Bias Lane 0")
        original = A5Threshold.LPO_TX_BIAS_MA
        A5Threshold.LPO_TX_BIAS_MA = Threshold(low_value_alarm="1.8", desc="lpo tx bias", unit="mA")
        try:
    ...
    def test_base_threshold_kept_for_unit_display(self):
        ...
        config = ThresholdConfig.for_optical_metric(A5Threshold, "TX_BIAS_MA", "host_tx_bias0", "主机侧TX Bias Lane 0")
        self.assertIs(config.threshold, A5Threshold.TX_BIAS_MA)
    ...
    def test_peer_type_field(self):
        ...
        config = ThresholdConfig.for_optical_metric(
            A5Threshold, "TX_BIAS_MA", "peer_tx_bias0", "对端TX Bias Lane 0", type_field="peer_optical_type"
        )
        original = A5Threshold.LPO_TX_BIAS_MA
        A5Threshold.LPO_TX_BIAS_MA = Threshold(low_value_alarm="1.8", desc="lpo tx bias", unit="mA")
        try:
