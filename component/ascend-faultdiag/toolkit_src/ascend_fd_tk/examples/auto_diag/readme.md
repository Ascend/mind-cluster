...
采集的带内日志(压缩包或文件夹均可)放置到工具host_dump_cache目录下即可(若需要最新脚本,
...
将以上方式收集到的文本文件放置到工具switch_cli_output_cache目录下即可

### 4.3 BMC日志

手动通过bmc网页下载或通过命令 `ipmcget -d diaginfo` 采集的日志tar.gz包,
直接放到工具bmc_dump_cache目录下即可

## 5. 手动合并采集数据(可选)

当分别部署在不同网络平面的诊断工具采集的数据需要汇总诊断时, 仅需将cache目录打包, 然后传输到执行诊断的环境,
解压覆盖粘贴到cache目录.
