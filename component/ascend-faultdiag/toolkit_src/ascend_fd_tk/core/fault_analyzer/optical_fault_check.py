...
class OpticalFaultChecker:
    ...
    def __init__(self, optical_module_threshold):
        ...
        self._power_rule_map = {  # 1表示异常；0表示正常
            0b1111: ("本端和对端收发光均异常。", "两端光模块异常关断，建议先排查光链路中的端面/光纤和模块是否虚插。"),
            0b1110: ("本端收发光均异常，对端收光异常。", "建议排查本端光模块是否虚插、是否被异常关断和是否本体失效。"),
            0b1101: ("本端收发光均异常，对端发光异常。", self.COMM_SUGGEST),
            0b1100: ("本端收发光均异常，对端收发光均正常。", "可能是鸳鸯纤，建议排查本端和对端光模块连接是否正确。"),
            0b1011: ("本端收光异常，对端收发光均异常。", "建议排查对端光模块是否虚插、是否被异常关断和是否本体失效。"),
            0b1010: ("本端收光异常，对端收光异常。", "建议排查本端和对端光模块双向光链路端面/光纤是否异常。"),
            0b1001: ("本端收光异常，对端发光异常。", "建议排查对端光模块本体发端是否失效。"),
            0b1000: (
                "本端收光异常，对端收发光均正常。",
                "建议排查本端光模块本体接收端是否异常，光链路端面/光纤是否异常。",
            ),
            0b0111: ("本端发光异常，对端收发光均异常。", self.COMM_SUGGEST),
            0b0110: ("本端发光异常，对端收光异常。", "建议排查本端光模块是否虚插、是否被异常关断和是否本体失效。"),
            0b0101: ("本端发光异常，对端发光异常。", self.COMM_SUGGEST),
            0b0100: ("本端发光异常，对端收发光均正常。", "本端光模块上报有误，建议排查本端光模块是否本体失效。"),
            0b0011: ("本端收发光均正常，对端收发光均异常。", "可能是鸳鸯纤，建议排查本段和对端光模块连接是否正确。"),
            0b0010: (
                "本端收发光均正常，对端收光异常。",
                "建议排查对端光模块本体接收端是否异常，光链路端面/光纤是否异常。",
            ),
            0b0001: ("本端收发光均正常，对端发光异常。", "对端光模块上报有误，建议排查对端光模块是否本体失效。"),
        }

        self._single_power_rule_map = {
            0b11: (
                "本端收发光均异常，未收集到对端交换机或者交换机端口的光模块信息。",
                "建议排查本端光模块本体是否失效，并收集对端信息后再分析。",
            ),
            0b10: (
                "本端收光异常，未收集到对端交换机或者交换机端口的光模块信息。",
                "建议排查本端光模块本体接收端是否异常，并收集对端信息后再分析。",
            ),
            0b01: (
                "本端发光异常，未收集到对端交换机或者交换机端口的光模块信息。",
                "建议排查本端光模块本体发送端是否异常，并收集对端信息后再分析。",
            ),
        }

        self._snr_rule_map = {  # 1表示异常；0表示正常
            0b11: ("本端和对端光模块信噪比均异常。", "建议更换两端光模块。"),
            0b10: ("本端光模块信噪比异常。", "建议更换对端光模块。"),
            0b01: ("对端光模块信噪比异常。", "建议更换本端光模块。"),
        }

        self._bias_rule_map = {  # 1表示异常；0表示正常
            0b11: ("本端和对端光模块电流均异常。", "建议更换两端光模块。"),
            0b10: ("本端光模块电流异常。", "建议更换本端光模块。"),
    ...
    def power_analyze(
        ...
        description, suggest = self._check_power_value(
            bool(local_abn_rx_power_infos),
            bool(local_abn_tx_power_infos),
            bool(remote_abn_rx_power_infos),
            bool(remote_abn_tx_power_infos),
        )
        if not description or not suggest:
            return []
    ...
    def _check_power_value(
        self,
        is_local_rx_abnormal: bool,
        is_local_tx_abnormal: bool,
        is_remote_rx_abnormal: bool,
        is_remote_tx_abnormal: bool,
    ):
        bits = (
            (int(is_local_rx_abnormal) << 3)
            | (int(is_local_tx_abnormal) << 2)
            | (int(is_remote_rx_abnormal) << 1)
            | int(is_remote_tx_abnormal)
        )
