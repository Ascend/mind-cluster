...
class HostLoopbackAnalyzer(Analyzer):
    @staticmethod
    def host_loopback_diag(host_info: HostInfo):
        results = []
        ...
        for loopback_info in host_info.loopback_info_list:
            ...
            domain = HostDomain(
                ...
                npu_id=loopback_info.npu_id,
                chip_phy_id=loopback_info.chip_phy_id,
            )
            if not loopback_info.host_input_link_stat.is_first_record_up():
                results.append(
                    DiagResult(
                        domain=domain,
                        fault_info="本端环回类型1后端口down，诊断为本端故障",
                        suggestion="建议交叉模组、线缆，如果有CDR板需要做CDR环回",
                    )
                )
            if loopback_info.media_output_enable and loopback_info.media_output_link_stat.is_first_record_up():
                results.append(
                    DiagResult(
                        domain=domain,
                        fault_info="本端环回类型1后端口up，环回类型2后端口down，诊断为本端端口光模块故障/赃污",
                        suggestion="建议排查本端端口光模块故障/赃污",
                    )
                )
        return results
