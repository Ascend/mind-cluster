...
class HostToSwitchOpticalModuleSheetGenerator(BaseSheetGenerator):
        ...
        merged_header = [
            {
                ...
                "交换机侧": [
                    ...
                    "对端交换机SNR Lane 0",
                    "对端交换机TX Power Lane 1",
                    "对端交换机RX Power Lane 1",
                    "对端交换机SNR Lane 1",
                    "对端交换机TX Power Lane 2",
                    "对端交换机RX Power Lane 2",
                    "对端交换机SNR Lane 2",
                    "对端交换机TX Power Lane 3",
                    "对端交换机RX Power Lane 3",
                    "对端交换机SNR Lane 3",
                ],
            }
        ]

        return header_mapping, merged_header

    @staticmethod
    def _build_host_base_data(
        host_id, host_info, npu_chip_info, optical_module_info, lane_data: Dict[str, str]
    ) -> Dict[str, str]:
        ...
        if hasattr(npu_chip_info, 'hccn_lldp_info') and npu_chip_info.hccn_lldp_info:
            local_port = f"PORT-{npu_chip_info.chip_id}" if npu_chip_info.chip_id else ""
        return {
            "host_id": host_id,
    ...
    def generate_sheet(self) -> None:
        ...
        sheet = create_threshold_report(
            sheet_name="主机NPU<->RoCE交换机端口光模块信息",
            # pylint: disable=duplicate-code  # 已与同类分析器复用逻辑，忽略重复警告
    ...
    def _get_lane_info(self, optical_module_info: OpticalModuleInfo) -> Dict[str, str]:
    ...
    def _get_peer_switch_info(self, npu_chip_info: NpuChipInfo):
        ...
        if not lldp_info:
            return peer_switch_name, peer_port, peer_switch, peer_interface_info
    ...
    def _build_peer_switch_data(self, peer_switch, peer_interface_info) -> Dict[str, str]:
        ...
        peer_optical_sn = ""
        peer_optical_temp = ""
        peer_lane_data = self._build_peer_lane_data(peer_interface_info)
    ...
    def _build_peer_lane_data(self, peer_interface_info) -> Dict[str, str]:
        peer_lane_data = {}
        for i in range(self.LANE_NUM):
            peer_lane_data[f"peer_switch_tx_power{i}"] = ""
            peer_lane_data[f"peer_switch_rx_power{i}"] = ""
            peer_lane_data[f"peer_switch_snr_lane{i}"] = ""
        if not peer_interface_info:
            return peer_lane_data
        peer_optical_info = peer_interface_info.get_optical_module_info()
        if not peer_optical_info:
            return peer_lane_data
        peer_lane_infos = peer_optical_info.lane_power_infos or []
        for i in range(min(self.LANE_NUM, len(peer_lane_infos))):
            peer_lane = peer_lane_infos[i]
            peer_lane_data[f"peer_switch_tx_power{i}"] = peer_lane.tx_power or ""
            peer_lane_data[f"peer_switch_rx_power{i}"] = peer_lane.rx_power or ""
            peer_lane_data[f"peer_switch_snr_lane{i}"] = peer_lane.media_snr or peer_lane.host_snr or ""
        return peer_lane_data
