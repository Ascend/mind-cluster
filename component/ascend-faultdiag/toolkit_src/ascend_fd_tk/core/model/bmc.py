...
class PeriodicRecordingOpticalModuleHistoryLog(OpticalModuleHistoryLog):
    def __init__(
        ...
        location="",
        optical_module_id="",
        tx_power_current_max="",
        tx_power_current_min="",
        rx_power_current_max="",
        rx_power_current_min="",
        tx_bias_current_max="",
        tx_bias_current_min="",
        tx_los="",
        rx_los="",
...
class BmcNpuInfo(JsonObj, OpticalModule):
    def __init__(
        ...
        chip_id="",
        chip_phy_id="",
        link_down_optical_module_history_log: LinkDownOpticalModuleHistoryLog = None,
        lane_len=8,
    ):
        self.npu_id = npu_id
...
class BmcInfo(JsonObj):
    ...
    def get_bmc_npu_infos(self) -> List[BmcNpuInfo]:
        ...
        self._bmc_npu_infos = []
        group_dict = list_tool.group_by_to_dict(
            self.link_down_optical_module_history_logs, key=lambda log: log.location.replace("NPU", "")
        )
        # link_down_optical_module_history_logs中NPU序号范围1-16
        for chip_phy_id, npu_info in DEFAULT_NPU_MAPPING.items():
            optical_module_history_log = group_dict.get(str(int(chip_phy_id) + 1))
            if not optical_module_history_log:
                continue
            # 取最近一次
            last_record = max(optical_module_history_log, key=lambda log: log.log_time)
            bmc_npu_info = BmcNpuInfo(
                npu_id=npu_info.npu_id,
                chip_id=npu_info.chip_id,
                chip_phy_id=chip_phy_id,
                link_down_optical_module_history_log=last_record,
