...
from diag_tool.core.log_parser import parse_script
from diag_tool.core.log_parser.base import LogParsePattern, FindResult, LogParser
from diag_tool.utils.executors import AsyncExecutor, CmdTask
...
class RemoteLogPyScriptParser(LogParser):
    ...
    async def find(self, parse_dir="", log_pattern_map: Dict[str, LogParsePattern] = None) -> List[FindResult]:
        ...
        await self.executor.run_cmd(CmdTask("mkdir -p ~/.diag_tool"))
        await self.executor.upload_file(CommonPath.PARSE_CONFIG_FILE, "~/.diag_tool/parse_config.json")
        await self.executor.upload_file(parse_script.SCRIPT_PATH, "~/.diag_tool/parse_script.py")
        await self.executor.run_cmd(CmdTask(f"python3 ~/.diag_tool/parse_script.py {parse_dir}"))
        await self.executor.download_file("~/.diag_tool/parse_result.json", self.local_parse_result_path)
