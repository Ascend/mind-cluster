...
func (fJob *FaultJob) processFaultStrategies() {
	...
	if err := resource.DeepCopy(newStrategiesMaps, podStrategiesMaps); err != nil {
...
func (fJob *FaultJob) clearProcessedAndTimeOutFault() {
	...
	networkFaultInfo := make([]*faultInfo, 0)
...
func (fJob *FaultJob) initFaultJobByDeviceFault(nodeFaultInfo AdvanceDeviceFaultCm, serverList job.ServerHccl) {
	...
	for _, deviceInfo := range serverList.DeviceList {
		...
		fJob.initFaultInfoByDeviceFault(fault, serverList.ServerName,
			util.IsSliceContain(deviceName, nodeFaultInfo.CardUnHealthy))
	}
}

func (fJob *FaultJob) initFaultInfoByDeviceFault(faultList []constant.DeviceFault, nodeName string, isCardUnhealthy bool) {
	for _, fault := range faultList {
		if isAssociateFault(fault.FaultCode) && !isCardUnhealthy {
			tmpFaultInfo := faultInfo{
				...
				DealMaxTime: getFaultCodeDelMaxTime(fault.FaultCode),
				FaultUid:    nodeName + "-" + fault.NPUName + "-" + fault.FaultCode,
			}
...
func (fJob *FaultJob) initFaultJobBySwitchFault(switchInfo *constant.SwitchInfo, serverList job.ServerHccl) {
	if switchInfo == nil {
		return
	}
