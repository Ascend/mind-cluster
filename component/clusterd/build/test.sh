...
function execute_test() {
  if ! (go test -mod=mod -gcflags=all=-l -v -coverprofile cov.out ${TOP_DIR}/pkg/... >./$file_input); then
    cat ./$file_input
    ...
    gotestsum --junitfile unit-tests.xml -- -gcflags=all=-l "${TOP_DIR}"/pkg/...
...
echo "<html<body><h1>clusterd testCase</h1><table border="1">" >>./$file_detail_output
echo "<html<body><h1>==================================================</h1><table border="2">" >>./$file_detail_output
while read line; do
  echo -e "<tr>
