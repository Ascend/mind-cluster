...
func (sJob *SchedulerJob) setParameterPlaneUnhealthyTolerance() {
	...
	if ok && val == util.ParameterPlaneUnhealthyToleranceIgnoreValue || sJob.NPUTaskNum <= 1 {
...
func (sJob *SchedulerJob) checkNodeNum(taskInfo *api.TaskInfo, vcNode NPUNode) error {
	...
	if int(nodeNPUNum/util.NPUHexKilo) < vcTask.ReqNPUNum {
		return fmt.Errorf("%s %s, task requires %d NPU cards but node idle is %d",
			util.NPUResourceShortageError, vcTask.ReqNPUName, vcTask.ReqNPUNum, int(nodeNPUNum/util.NPUHexKilo))
	}
	return nil
}
