...
import (
	...
	"ascend/dra-example-driver/pkg/flags"
)
...
func CountDavinciDirs() (int, error) {
	count := 0

	re := regexp.MustCompile(`^davinci\d+$`)

	entries, err := os.ReadDir(consts.DevPath)
	if err != nil {
		return 0, fmt.Errorf("read %v: %v\n", consts.DevPath, err)
	}
...
func newApp() *cli.App {
	...
	deviceCnt, err := CountDavinciDirs()
	if err != nil {
		return nil
	}
	klog.Infof("deviceCnt: %v\n", deviceCnt)
	cliFlags := []cli.Flag{
		&cli.StringFlag{
			Name:        "node-name",
	...
	app := &cli.App{
		Name:            "dra-npu-kubeletplugin",
		Usage:           "dra-npu-kubeletplugin implements a DRA driver plugin.",
		ArgsUsage:       " ",
...
func RunPlugin(ctx context.Context, config *Config) error {
	...
	logger.Info(fmt.Sprintf("[DriverPluginPath]: %v", config.DriverPluginPath()))
	err := os.MkdirAll(config.DriverPluginPath(), 0750)
	if err != nil {
		return err
	}

	logger.Info(fmt.Sprintf("[CdiPath]: %v", config.flags.cdiRoot))
