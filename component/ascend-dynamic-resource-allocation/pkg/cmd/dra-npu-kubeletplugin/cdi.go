...
func (cdi *CDIHandler) CreateClaimSpecFile(claimUID string, devices PreparedDevices) error {
	...
	var ascendEnv []string
	for _, device := range devices {
		ascendEnv = append(ascendEnv, consts.AscendPrefix+device.DeviceName[4:])
		claimEdits := cdiapi.ContainerEdits{
			ContainerEdits: &cdispec.ContainerEdits{
				Env: []string{
					fmt.Sprintf("NPU_DEVICE_%s_RESOURCE_CLAIM=%s", device.DeviceName[4:], claimUID),
				},
			},
		}
	...
	spec.ContainerEdits = cdispec.ContainerEdits{
		Env: []string{fmt.Sprintf("%s=%s", consts.AscendVisibleDevicesEnv, strings.Join(ascendEnv, ","))},
	}
	...
	spec.Version = minVersion
	fmt.Println("specName: " + specName)
	fmt.Println("cmd/dra-npu-kubeletplugin/cdi.go::CreateClaimSpecFile [spec]:")
	spew.Dump(spec)
	return cdi.cache.WriteSpec(spec, specName)
}
