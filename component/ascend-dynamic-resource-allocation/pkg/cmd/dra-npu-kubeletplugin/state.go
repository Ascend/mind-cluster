...
import (
	...
	configapi "ascend/dra-example-driver/pkg/api/example.com/resource/gpu/v1alpha1"
	"ascend/dra-example-driver/pkg/consts"
)

type AllocatableDevices map[string]resourceapi.Device
type AllocatedDevices map[string][]resourceapi.Device
...
func NewDeviceState(config *Config, ctx context.Context) (*DeviceState, error) {
...
func (s *DeviceState) Prepare(claim *resourceapi.ResourceClaim) ([]*drapbv1.Device, error) {
	...
	klog.Infoln("cmd/dra-npu-kubeletplugin/state.go::Prepare [checkpoint]:")
	spew.Dump(checkpoint)
	...
	klog.Infoln("start allocating devices")
	for _, result := range claim.Status.Allocation.Devices.Results {
		// result.Device is resource id like npu-2
		s.allocated[claimUID] = append(s.allocated[claimUID], s.allocatable[result.Device])
		delete(s.allocatable, result.Device)
	}
...
func (s *DeviceState) Unprepare(claimUID string) error {
	...
	// If the container deletion fails, there may be issues
	klog.Infoln("start releasing devices")
	for _, deviceInfo := range s.allocated[claimUID] {
		s.allocatable[deviceInfo.Name] = deviceInfo
...
func (s *DeviceState) prepareDevices(claim *resourceapi.ResourceClaim) (PreparedDevices, error) {
	...
	configResultsMap, err := s.funcName(claim)
	if err != nil {
		return nil, err
	}
	...
	klog.Infoln("cmd/dra-npu-kubeletplugin/state.go::prepareDevices [perDeviceCDIContainerEdits]:")
	spew.Dump(perDeviceCDIContainerEdits)
	...
	klog.Infoln("cmd/dra-npu-kubeletplugin/state.go::prepareDevices [preparedDevices]:")
	spew.Dump(preparedDevices)
...
func (s *DeviceState) getConfigResultsMap(claim *resourceapi.ResourceClaim) (map[runtime.Object][]*resourceapi.DeviceRequestAllocationResult, error) {
	...
	klog.Infoln("cmd/dra-npu-kubeletplugin/state.go::prepareDevices [configs]:")
	spew.Dump(configs)
	if err != nil {
	...
	klog.Infoln("cmd/dra-npu-kubeletplugin/state.go::prepareDevices [configs after insert]:")
	spew.Dump(configs)
	...
	klog.Infoln("cmd/dra-npu-kubeletplugin/state.go::prepareDevices [configResultsMap]:")
	spew.Dump(configResultsMap)
...
func GetOpaqueDeviceConfigs(
	...
	fmt.Println("cmd/dra-npu-kubeletplugin/state.go::GetOpaqueDeviceConfigs [possibleConfigs]:")
	spew.Dump(possibleConfigs)
