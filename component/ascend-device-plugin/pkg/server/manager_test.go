...
func TestSetAscendManager(t *testing.T) {
	...
	convey.Convey("test devType is Ascend310", t, func() {
		mockGetDevType := gomonkey.ApplyMethodReturn(reflect.TypeOf(new(devmanager.DeviceInterface)), "GetDevType", common.Ascend310)
		defer mockGetDevType.Reset()
		err := hdm.setAscendManager(devM)
		convey.So(err, convey.ShouldBeNil)
	})
	convey.Convey("test devType is Ascend310P", t, func() {
		mockGetDevType := gomonkey.ApplyMethodReturn(reflect.TypeOf(new(devmanager.DeviceInterface)), "GetDevType", common.Ascend310P)
		defer mockGetDevType.Reset()
		err := hdm.setAscendManager(devM)
		convey.So(err, convey.ShouldBeNil)
	})
