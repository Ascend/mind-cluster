...
func TestHwDevManagerLoadFaultCode(t *testing.T) {
	...
	convey.Convey("Test HwDevManager.loadFaultCode", t, func() {
		patch := gomonkey.ApplyMethodReturn(hdm.manager, "GetKubeClient", &kubeclient.ClientK8s{})
		defer patch.Reset()
		convey.Convey("When configmap not found, load fault config from local file and reset resourceVersion", func() {
			originalVersion := resourceVersion
			resourceVersion = ""
			...
			convey.So(resourceVersion, convey.ShouldEqual, "")
			convey.So(interval, convey.ShouldEqual, common.PollFaultCodeCMInterval)
		})
		convey.Convey("When configmap not found after config loaded, reload from local file and reset resourceVersion", func() {
			originalVersion := resourceVersion
			resourceVersion = "some-cm-version"
			defer func() { resourceVersion = originalVersion }()
			called := false
			patch1 := gomonkey.ApplyFunc(common.LoadFaultCodeFromFile,
		...
		convey.Convey("When part of local fault configs load failed, reset resourceVersion for the next retry", func() {
			originalVersion := resourceVersion
		...
		convey.Convey("When all local fault configs load failed, reset resourceVersion for the next retry", func() {
			originalVersion := resourceVersion
			resourceVersion = "some-version"
			defer func() { resourceVersion = originalVersion }()
			patch1 := gomonkey.ApplyFunc(common.LoadFaultCodeFromFile,
		...
		convey.Convey("When api server unreachable at first load, fallback to local fault config", func() {
			originalVersion := resourceVersion
			resourceVersion = ""
			defer func() { resourceVersion = originalVersion }()
			faultCodeCalled, customizationCalled := false, false
