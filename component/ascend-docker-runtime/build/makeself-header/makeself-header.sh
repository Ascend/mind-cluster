...
check_log()
{
    if ! test -d \${INSTALL_LOG_DIR}; then
        mkdir -p -m 750 \${INSTALL_LOG_DIR}
    fi

    if ! test -f \${INSTALL_LOG_PATH}; then
    ...
    if test \${log_size} -gt \${LOG_SIZE_THRESHOLD}; then
...
if test x"\$verbose" = xy; then
	...
	read yn
	if test x"\$yn" = xn; then
        log "[ERROR]" "extract failed, you say no"
		eval \$finish; exit 1
	fi
fi
...
if test x"\$nodiskspace" = xn; then
    ...
    if test -n "\$leftspace"; then
        if test "\$leftspace" -lt $USIZE; then
            ...
            log "[ERROR]" "extract failed, Not enough space left in "\`dirname \$tmpdir\`" (\$leftspace KB) to decompress \$0 ($USIZE KB)"
            eval \$finish; exit 1
        fi
    fi
fi
...
do
    if MS_dd_Progress "\$0" \$offset \$s | MS_Decompress | ( cd "\$tmpdir"; umask \$ORIG_UMASK ; UnTAR xp ) 1>/dev/null; then
        ...
        log "[ERROR]" "extract failed, Unable to decompress \$0"
		eval \$finish; exit 1
    fi
    offset=\`expr \$offset + \$s\`
done
