...
function build() {
  ...
  ls ${OUTPUT_NAME}
  if [ $? -ne 0 ]; then
    echo "fail to find dpu-exporter"
    exit 1
  fi
}

function mv_file() {
  mv "${TOP_DIR}"/cmd/dpu-exporter/${OUTPUT_NAME} "${TOP_DIR}"/output
  cp "${TOP_DIR}"/build/dpu-exporter.yaml "${TOP_DIR}"/output/dpu-exporter-"${build_version}".yaml
