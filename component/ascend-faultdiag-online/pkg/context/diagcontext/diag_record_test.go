...
func TestUpdateRecord(t *testing.T) {
	...
	assert.Equal(t, 80, len(store.diagRecordItemMap[diagItem.Name].DiagRecords))
	...
	assert.Equal(t, 40, len(abnormalRes))

	// 更新后的诊断记录
	store.UpdateRecord(diagItem, abnormalRes)
	// 现有的诊断记录清除overflowSize个后 + 新增诊断记录中异常的诊断记录。即：[20:80] + 40
	assert.Equal(t, 100, len(store.diagRecordItemMap[diagItem.Name].DiagRecords))
