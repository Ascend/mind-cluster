...
var (
	...
	npuChipInfoDescHealthStatusV2 = prometheus.NewDesc("npu_chip_info_health_status_v2",
		"the v2 npu health status", []string{npuID, modelName, npuUUID, npuPCIEInfo}, nil)
	...
	npuChipInfoDescLinkStatusV2 = prometheus.NewDesc("npu_chip_info_link_status_v2",
		"the v2 npu link status", []string{npuID, modelName, npuUUID, npuPCIEInfo}, nil)
	...
	npuChipInfoDescNetworkStatusV2 = prometheus.NewDesc("npu_chip_info_network_status_v2",
		"the v2 npu network health status", []string{npuID, modelName, npuUUID, npuPCIEInfo}, nil)
...
func npuNetworkInfoCollect(ctx context.Context, group *sync.WaitGroup, n *npuCollector, dmgr devmanager.DeviceInterface) {
	group.Add(1)
...
func describeBaseChipInfo(ch chan<- *prometheus.Desc) {
	...
	ch <- npuChipInfoDescHealthStatusV2
...
func updateNPUNetworkInfo(ch chan<- prometheus.Metric, npu *HuaWeiNPUCard, chip *HuaWeiAIChip) {
	...
	ch <- prometheus.NewMetricWithTimestamp(npu.Timestamp,
		prometheus.MustNewConstMetric(npuChipInfoDescNetworkStatusV2, prometheus.GaugeValue,
			float64(getHealthCodeV2(chip.NetHealthStatus)), []string{strconv.FormatInt(int64(chip.DeviceID), base),
				common.GetNpuName(*chip.ChipIfo), chip.VDieID, chip.PCIeBusInfo}...))
	ch <- prometheus.NewMetricWithTimestamp(npu.Timestamp,
...
func updateNPUCommonInfo(ch chan<- prometheus.Metric, npu *HuaWeiNPUCard, chip *HuaWeiAIChip) {
	...
	ch <- prometheus.NewMetricWithTimestamp(npu.Timestamp, prometheus.MustNewConstMetric(npuChipInfoDescLinkStatus,
		...
		[]string{strconv.FormatInt(int64(chip.DeviceID), base), common.GetNpuName(*chip.ChipIfo), chip.VDieID, chip.PCIeBusInfo}...))
	ch <- prometheus.NewMetricWithTimestamp(npu.Timestamp, prometheus.MustNewConstMetric(npuChipInfoDescLinkStatusV2,
		prometheus.GaugeValue, float64(hccn.GetLinkStatusCodeV2(chip.LinkStatus)),
		[]string{strconv.FormatInt(int64(chip.DeviceID), base), common.GetNpuName(*chip.ChipIfo), chip.VDieID, chip.PCIeBusInfo}...))
	...
	ch <- prometheus.NewMetricWithTimestamp(npu.Timestamp,
		prometheus.MustNewConstMetric(npuChipInfoDescHealthStatus, prometheus.GaugeValue,
			float64(getHealthCode(chip.HealthStatus)), []string{strconv.FormatInt(int64(chip.DeviceID), base),
				common.GetNpuName(*chip.ChipIfo), chip.VDieID, chip.PCIeBusInfo}...))
	ch <- prometheus.NewMetricWithTimestamp(npu.Timestamp,
		prometheus.MustNewConstMetric(npuChipInfoDescHealthStatusV2, prometheus.GaugeValue,
			float64(getHealthCodeV2(chip.HealthStatus)), []string{strconv.FormatInt(int64(chip.DeviceID), base),
				common.GetNpuName(*chip.ChipIfo), chip.VDieID, chip.PCIeBusInfo}...))
...
func getHealthCodeV2(health string) int {
	if Healthy == health {
		return healthyCode
	}
	if UnHealthy == health {
