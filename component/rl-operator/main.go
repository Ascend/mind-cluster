...
import (
	...
	v1 "rl-operator/pkg/controllers/v1"
)

const (
	defaultLogFileName    = "/var/log/mindx-dl/rl-operator/rl-operator.log"
	defaultQPS            = 50.0
	defaultBurst          = 100
	maxQPS                = 10000.0
	maxBurst              = 10000
...
func main() {
	...
	if version {
		fmt.Printf("rl-operator version: %s\n", BuildVersion)
		return
	}
	...
	hwlog.RunLog.Infof("rl-operator starting and the version is %s", BuildVersion)
...
func setUpControllers(mgr ctrl.Manager) {
	reconcilers := []common.ManagerSetUp{
		v1.NewRayClusterReconciler(mgr, enableGangScheduling),
		v1.NewRayHeadReconciler(mgr, enableGangScheduling),
		v1.NewRayWorkerReconciler(mgr, enableGangScheduling),
		v1.NewVerlJobReconciler(mgr, enableGangScheduling),
	}

	for _, reconciler := range reconcilers {
		if err := reconciler.SetupWithManager(mgr); err != nil {
			hwlog.RunLog.Errorf("create %s err: %s", reflect.TypeOf(reconciler).Name(), err)
			return
		}
	}
}
