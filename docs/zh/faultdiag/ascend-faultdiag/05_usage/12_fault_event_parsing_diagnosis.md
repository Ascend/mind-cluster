...
故障事件清洗入参和返回值，请阅读 `API参考 -> SDK接口 -> parse_knowledge_graph` 中的 [parse_knowledge_graph](../06_api/09_sdk_api.md#parse_knowledge_graph) 接口定义。

故障事件诊断入参和返回值，请阅读 `API参考 -> SDK接口 -> diag_knowledge_graph` 中的 [diag_knowledge_graph](../06_api/09_sdk_api.md#diag_knowledge_graph) 接口定义。
