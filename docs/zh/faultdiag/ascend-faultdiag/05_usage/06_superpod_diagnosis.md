...
1. 执行清洗操作，将所有日志清洗到同一目录。

    示例：将 Host、BMC、LCNE 日志清洗到同一目录。

    ```shell
    ascend-fd parse -i 设备0host日志目录 -o 超节点清洗结果输出目录/host/worker-0
    ascend-fd parse --lcne_log 设备0lcne日志目录 -o 超节点清洗结果输出目录/lcne/worker-0
    ascend-fd parse --bmc_log 设备0BMC日志目录 -o 超节点清洗结果输出目录/bmc/worker-0
    ```

    > [!NOTE]
    >
    > - Host 侧日志必须包含 dmidecode.txt 信息
    > - 以设备0为例，其他设备的目录结构与之相同。
...
当缺失 Host、BMC、LCNE 任一日志时，需要手动关联。

1. 手动执行清洗，将不同设备日志清洗到不同的 worker 目录下。

    ```shell
    # 清洗时同时指定 host、bmc 日志
    ascend-fd parse --process_log 设备0plog日志目录 --bmc_log 设备0BMC日志目录 -o 超节点清洗结果输出目录/worker-0
    ascend-fd parse --process_log 设备1plog日志目录 --bmc_log 设备1BMC日志目录 -o 超节点清洗结果输出目录/worker-1
    ascend-fd parse --process_log 设备2plog日志目录 --bmc_log 设备2BMC日志目录 -o 超节点清洗结果输出目录/worker-2
    ```
    ...
    > - 不同的设备，应该对应不同的 worker 目录（如五台设备，分别对应 worker-1 ~ worker-5）
