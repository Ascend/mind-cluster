...
ascend-fd parse \
    ...
    --device_log <Device侧日志目录> \
    --train_log <用户训练及推理日志目录> \
    --process_log <CANN应用类日志目录> \
    --env_check <NPU网口/状态信息/资源信息目录> \
    --dl_log <MindCluster组件日志目录> \
    --mindie_log <MindIE组件日志目录> \
    --amct_log <AMCT组件日志目录> \
    --pymotor_vllm_log <PyMotor/vLLM日志目录> \
    --bmc_log <BMC侧日志目录> \
    --lcne_log <LCNE组件日志目录> \
    -o <清洗输出目录>
```
...
> [!NOTE]
>
> - 同时使用 `-i` 与详细日志目录参数时，会优先读取详细日志目录参数的值，再根据 `-i` 参数读取剩余日志目录。
