...
- ascend-fd 要求 Python 版本 >= 3.7，如需使用性能劣化功能（设备资源分析与网络拥塞分析功能），要求 Python 版本 >= 3.8。安装前请检查 Python 版本是否满足要求。
...
软件包可以通过开源社区下载 zip 包或者自行通过源码进行编译 whl 包。
...
1. 建议修改 umask 为 027 提高安全性
    ...
    > 永久修改 umask 为 027，可以查询[系统安全配置](../07_references/03_security.md#系统安全配置)。
...
1. 下载 [MindCluster_sha256sum.zip](https://gitcode.com/Ascend/mind-cluster/releases)
