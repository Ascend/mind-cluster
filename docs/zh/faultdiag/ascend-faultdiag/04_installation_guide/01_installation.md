...
- 建议使用同一用户进行安装和使用。
- ascend-fd 要求 Python 版本 >= 3.8。安装前请检查 Python 版本是否满足要求。
...
2. 编译所需三方依赖库
    ...
    ```shell
    pip3 install scikit-learn>=1.3.0 pandas>=2.0.3 ply>=3.11 setuptools>=60.3.0 wheel>=0.45.1
    ```

3. 执行 build 脚本

    编译脚本会自动创建 `src/ascend_fd/Version.info` 文件，包含版本信息，默认版本号为 `1.0.0`

    如需修改，请手动在 `src/ascend_fd/Version.info`（没有则创建）中修改, 如：`26.1.0`。

    ```shell
    cd mind-cluster/faultdiag/ascend-faultdiag
    bash build/build.sh
    ```
...
单台设备安装 MindCluster Ascend FaultDiag 组件，请参见《MindCluster Ascend Deployer 用户指南》中的“[安装昇腾软件](https://gitcode.com/Ascend/ascend-deployer/blob/dev/docs/zh/install_softwares.md#%E5%AE%89%E8%A3%85%E6%98%87%E8%85%BE%E8%BD%AF%E4%BB%B6)”章节。

#### 批量安装

批量安装 MindCluster Ascend FaultDiag 组件，请参见《MindCluster Ascend Deployer 用户指南》中的“[安装昇腾软件](https://gitcode.com/Ascend/ascend-deployer/blob/dev/docs/zh/install_softwares.md#%E5%AE%89%E8%A3%85%E6%98%87%E8%85%BE%E8%BD%AF%E4%BB%B6)”章节。
