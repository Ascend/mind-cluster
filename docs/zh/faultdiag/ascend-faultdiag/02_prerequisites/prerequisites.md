...
- **LCNE**：灵衢计算网络引擎，支持大规模计算网络的分域集中式拓扑发现、路由管理、转发控制。
- **BMC**：基板管理控制器，用于监控和管理服务器硬件的控制器，如温度、电压、风扇等。
- **[vLLM](https://docs.vllm.ai/en/latest/)**：是用于大型语言模型（LLM）的推理和服务部署的 Python 库。
- **[MindCluster](https://www.hiascend.com/cn/developer/software/mindcluster)**：是支持 NPU（昇腾 AI 处理器）训练和推理硬件的深度学习组件，使能构建集群全流程运行，提供 NPU 集群作业调度、运维监测、故障恢复等功能。
