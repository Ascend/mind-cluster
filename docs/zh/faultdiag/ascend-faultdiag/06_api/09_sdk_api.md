...
> [!NOTE]
>
> - 日志文件大小不超过 10M，超过限制大小后将自动转储道另一个日志文件。
> - 通 PID 的日志文件数量最大不超过 10 个，超过限制个数后将自动覆盖最早创建的日志。
...
| 参数           | 类型 | 说明                         |
| -------------- | ---- | ---------------------------- |
| input_log_list | List | 用户输入的业务流日志列表     |
...
2. results 字段说明
    ...
    | `fault_description.code`   | Integer      | 是     | 故障代码                            |
...
2. custom_entity 参数说明
    ...
    > 详细字段定义请参考 [自定义故障实体](../06_api/05_command_entity.md#json参数说明)。
