...
ascend-fd 要求 Python 3.7 及以上版本，如需使用性能劣化功能，要求 Python 3.8 及以上版本。请检查 Python 版本：
...
清洗输出目录需要至少 5GB 的可用磁盘空间。请清理磁盘或更换输出目录。

## root 安装后普通用户无法使用

需要配置 PATH 环境变量。以 root 用户查询 ascend-fd 位置：
...
Linux 系统默认最大文件描述符数为 1024。集群规模超过 128 台服务器（1024 卡）时，需要调整文件描述符上限：
...
终端默认仅展示 16 条故障设备信息。完整信息可以在诊断结果文件 `diag_report.json` 中查看。

## 安装完 ascend-fd 提示 command not found

- ascend-fd 安装失败，重新安装 ascend-fd 即可。
- 当前设备可能有多个 Python 版本，ascend-fd 安装在非默认 Python 下。
  ...
  2. 将步骤1查找到的目录添加到 PATH 中：

      ```bash
      export PATH=$PATH:/usr/local/python3.8/bin/
      ```

  3. 再次尝试执行 ascend-fd 命令即可。
