...
| `LPO_TX_BIAS_MA` | LPO 光模块发射偏置电流（仅<term>Ascend 950PR&950DT系列产品</term>）      | mA |
| `LPO_TX_POWER_DBM` | LPO 光模块发射光功率（仅<term>Ascend 950PR&950DT系列产品</term>）        | dBm |
| `LPO_RX_POWER_DBM` | LPO 光模块接收光功率（仅<term>Ascend 950PR&950DT系列产品</term>）        | dBm |
| `LPO_POWER_LANE_DIFF_DB` | LPO 光模块各 lane 间光功率的最大差值（仅<term>Ascend 950PR&950DT系列产品</term>） | dBm |
...
- 网卡（NIC）相关阈值仅<term>Ascend 950PR&950DT系列产品</term>使用。
- 光模块阈值按模块类型区分：支持 ODSP/LPO 两种光模块，未识别到光模块类型时，默认ODSP类型。
