...
[pod_manager]
# PoDManager 为 Atlas 950 SuperPoD 超节点的设备统一管理入口，登录后会切换到各个槽位号采集信息
...
- 工具使用时，需要连接到各个设备上进行命令查询、日志采集等操作。请参考以上 `conn.ini` 配置文件格式，配置集群连接信息。
- 若支持免密连接，可以不填写密码。
...
- 其中 PoDManager 是 Atlas 950 SuperPoD 超节点的设备统一管理入口，请填写到 `pod_manager` 选项框中。配置后工具会登录 PoDManager，按内置槽位表逐槽位切换采集各槽位对应的信息。
