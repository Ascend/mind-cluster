...
ascend-fd-tk 可完成集群设备信息采集与自动化巡检诊断，精准定位服务器、L1 / L2 灵衢交换机、RoCE 交换机及 BMC 链路故障。支持的产品如下表所示：
...
<tr>
<td rowspan="5">Atlas A3 训练系列产品</td>
...
> **注**：交换机需运行 VRP（Versatile Routing Platform）操作系统，支持 `display diagnostic-information`、`display interface`、`display interface transceiver verbose` 等命令。
