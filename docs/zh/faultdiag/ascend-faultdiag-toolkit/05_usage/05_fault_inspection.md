...
ascend-fd-tk 工具完成 [日志采集](02_log_collection.md) 与 [日志清洗](03_log_parse.md)后，使用 `auto_inspection` 命令，按不同客户类型预定义规则进行批量健康检查，定期对集群进行巡检，提前发现潜在链路异常。

> 诊断与巡检的主要区别：
>
> - **诊断**（`auto_diag`）：基于故障已发生时的日志数据进行根因分析，输出 `.xlsx` 诊断报告。
> - **巡检**（`auto_inspection`）：针对未发生明确故障的场景，基于预定义规则批量健康检查，输出 `.csv` 巡检报告。
