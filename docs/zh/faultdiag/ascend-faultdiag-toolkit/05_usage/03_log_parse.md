...
根据不同的采集方式使用 [auto_collect](../06_api/04_parse_diagnosis/01_auto_collect.md) 命令进行数据清洗，将原始日志解析为结构化数据，供后续诊断和巡检使用。

| 采集方式   | 清洗行为                                   |
|--------|----------------------------------------|
| 在线数据采集 | 自动 SSH 采集日志 + 清洗 → 清洗结果落盘到 `家目录/cache` |
| 离线日志采集   | 提前收集离线日志文件 → 清洗 → 清洗结果落盘到 `家目录/cache`  |
...
```text
# 直接将收集的日志压缩包放入到目录即可（目录名可自定义），工具支持自动解压日志压缩包，无需手动解压。至少配置一个日志目录即可进行诊断。
host日志采集目录/
...
当未手动设置连接配置文件 `conn.ini` 或离线日志目录时，工具会自动读取当前执行路径下的以下默认文件或目录，相关文件或目录需用户提前手动创建、配置设备连接信息和放置离线日志到对应目录。

- 连接配置：`conn.ini`（使用当前执行路径下的该文件时，多次执行清洗诊断任务前需执行 `clear_cache` 命令清除缓存）
- BMC 日志目录：`bmc_dump_log/`
- 服务器日志目录：`host_dump_log/`
