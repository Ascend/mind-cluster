...
| 性能   | Atlas A3系列产品                  | Ascend 950PR&950DT系列产品                   |
|------|-------------------------------|------------------------------------------|
| 单机耗时 | 单个Host、BMC、Switch清洗/诊断不超过3分钟  | 单个Host、BMC、Switch、单板BMC、单板UBM清洗/诊断不超过5分钟 |
| 集群耗时 | 集群清洗/诊断不超过`3*集群规模/cpu核心数*0.8`分钟 | 集群清洗/诊断不超过`5*集群规模/cpu核心数*0.8`分钟          |

> [!NOTE]
> 部分交换设备清洗耗时较高，会增大整体清洗时长。
