...
多管理节点场景需要保证所有管理节点配置一致，如集群调度组件镜像、日志目录、运行用户、节点标签等配置需一致。多管理节点下集群调度组件的安装请参见[安装部署](installation_guide/02_installation/manual_installation/00_obtaining_software_packages.md)章节。多管理节点的安装请参见K8s官方文档[利用kubeadm创建高可用集群](https://kubernetes.io/zh-cn/docs/setup/production-environment/tools/kubeadm/high-availability/)。
...
Containerd安装流程请参见[官方资料](https://github.com/containerd/containerd/blob/master/BUILDING.md)。集群调度组件安装时默认使用Ascend Docker Runtime，可以配置Containerd使用Ascend Docker Runtime替代runc，用于在启动容器时自动挂载设备，对Containerd需要做的配置请参见[安装部署](installation_guide/02_installation/manual_installation/00_obtaining_software_packages.md)章节的Containerd场景下安装Ascend Docker Runtime。
...
|产品形态|参考文档|
|--|--|
|Atlas 训练系列产品|<ul><li><span>《[Atlas 中心训练服务器 26.0.RC1 健康管理故障定义](https://support.huawei.com/enterprise/zh/doc/EDOC1100568349)》</span></li><li><span>《[Atlas 中心训练服务器 26.0.RC1 黑匣子错误码信息列表](https://support.huawei.com/enterprise/zh/doc/EDOC1100568359)》</span></li></ul>|
|<term>Atlas A2 训练系列产品</term>|<ul><li><span>《[Atlas A2 中心推理和训练硬件 26.0.RC1 健康管理故障定义](https://support.huawei.com/enterprise/zh/doc/EDOC1100568354)》</span></li><li><span>《[Atlas A2 中心推理和训练硬件 26.0.RC1 黑匣子错误码信息列表](https://support.huawei.com/enterprise/zh/doc/EDOC1100568360)》</span></li></ul>|
|<term>Atlas A3 训练系列产品</term>|<ul><li><span>《[Atlas A3 中心推理和训练硬件 26.0.RC1 健康管理故障定义](https://support.huawei.com/enterprise/zh/doc/EDOC1100568351)》</span></li><li><span>《[Atlas A3 中心推理和训练硬件 26.0.RC1 黑匣子错误码信息列表](https://support.huawei.com/enterprise/zh/doc/EDOC1100568355)》</span></li></ul>|
...
|Atlas 推理系列产品（不包含Atlas 200I SoC A1 核心板）|<ul><li><span>《[Atlas 中心推理卡 26.0.RC1 健康管理故障定义](https://support.huawei.com/enterprise/zh/doc/EDOC1100568361)》</span></li><li><span>《[Atlas 中心推理卡 26.0.RC1 黑匣子错误码信息列表](https://support.huawei.com/enterprise/zh/doc/EDOC1100568352)》</span></li></ul>|
...
1. 搭建训练环境，拉起训练，详细请参见[PyTorch场景适配示例（基于MindSpeed-LLM）](./usage/resumable_training/07_using_resumable_training_on_the_cli.md#适配示例)。
2. 开启进程级在线恢复，详细请参见[配置进程级在线恢复](./usage/resumable_training/04_configuring_fault_handling_policies.md#配置进程级在线恢复)。
...
1. 搭建训练环境，拉起训练，详细请参见[MindSpore场景适配示例（基于MindFormers）](./usage/resumable_training/07_using_resumable_training_on_the_cli.md#适配示例)。
2. 开启进程级在线恢复，详细请参见[配置进程级在线恢复](./usage/resumable_training/04_configuring_fault_handling_policies.md#配置进程级在线恢复)。
...
- 通过文件方式开发自定义指标
    ...
    用户根据[自定义指标文件](./api/npu_exporter/03_custom_metrics_file.md)，创建符合要求的自定义指标文件。启动NPU Exporter时，配置“-textMetricsFilePath”参数，指定该自定义指标文件的路径。详情请参见[NPU Exporter启动参数](installation_guide/02_installation/manual_installation/03_npu_exporter.md#参数说明)。NPU Exporter会在每个数据采集周期读取自定义指标文件，并将文件内容上报给Prometheus或Telegraf。
...
- 安装Kubernetes，支持1.17.x\~1.34.x版本的Kubernetes（推荐使用1.19.x及以上版本），具体操作请参见[安装Kubernetes](https://kubernetes.io/zh/docs/setup/production-environment/tools/)推荐[使用Kubeadm创建集群](https://kubernetes.io/zh-cn/docs/setup/production-environment/tools/kubeadm/create-cluster-kubeadm/)，集群初始化过程中的部分问题可参考[初始化Kubernetes失败](https://gitcode.com/Ascend/mind-cluster/issues/338)。且需解除管理节点隔离。如需解除管理节点隔离，命令示例如下。
