...
|tls_info|可选|TLS的证书配置。| 默认为空，当开启TLS认证时，需要配置证书信息，具体字段应以键值对形式组织。具体配置指导见[导入TLS证书](../../fault_recovery_acceleration/04_security_management_and_hardening.md#导入tls证书)。 |
...
|参数|是否必选|说明|取值要求|
|--|--|--|--|
|decryptor|必选|用户自定义的私钥口令解密函数。|通过 tft_start_controller 和 tft_init_processor 配置TLS加密，并且如果口令为密文，则需注册解密函数。具体配置指导见[导入TLS证书](../../fault_recovery_acceleration/04_security_management_and_hardening.md#导入tls证书)。|
