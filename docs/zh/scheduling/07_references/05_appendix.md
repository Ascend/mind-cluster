...
|/usr/lib64/libummu*|文件，UB驱动提供的用户态库，因为会有多个文件，支持通配符匹配。当宿主机不支持UB协议、不存在这些文件或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载。|
|/usr/lib64/liburma*|文件，UB驱动提供的用户态库，因为会有多个文件，支持通配符匹配。当宿主机不支持UB协议、不存在这些文件或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载。|
|/usr/lib64/urma|目录，UB驱动提供的用户态库目录，当宿主机不支持UB协议、不存在此目录或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载。|
|/usr/lib64/libnl*|文件，Netlink协议接口提供的用户态库，因为会有多个文件，支持通配符匹配。当宿主机不支持UB协议、不存在这些文件或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载。|
|/usr/bin/urma_admin|文件，当宿主机不支持UB协议、不存在此目录或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载此文件。|
|/usr/bin/urma_perftest|文件，当宿主机不支持UB协议、不存在此目录或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载此文件。|
|/usr/bin/urma_ping|文件，当宿主机不支持UB协议、不存在此目录或者指定参数ASCEND_UB_DRV_MOUNT=False不挂载时，不会挂载此文件。|
|/usr/local/Ascend/driver/lib64|目录，驱动提供的用户态库。|
