...
1. 以安装用户 *{MindIO-install-user}* 登录安装节点。
    ...
    >[!NOTE]说明
    >安装用户设置的口令需符合口令复杂度要求（请参见[口令复杂度要求](./07_appendixes.md#口令复杂度要求)）。密码有效期为90天，用户可以在 `/etc/login.defs` 文件中修改有效期的天数，或者通过 **chage** 命令来设置用户的有效期，详情请参见[设置用户有效期](./07_appendixes.md#设置用户有效期)。
