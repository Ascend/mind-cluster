...
支持在执行训练或者推理任务时，对昇腾AI处理器资源各种数据信息的实时监测，可实时获取昇腾AI处理器利用率、温度、电压、内存，以及昇腾AI处理器在容器中的分配状况等信息，实现资源的实时监测。支持对虚拟NPU（vNPU）的AICore利用率、vNPU总内存和vNPU使用中内存进行监测。目前NPU Exporter仅支持对Atlas推理系列产品的vNPU资源监测。
...
- 安装MindCluster中的Ascend Docker Runtime、Ascend Device Plugin和Volcano组件，请参见[安装部署](../07_developer_guide/installation_deployment/manual_installation/00_obtaining_software_packages.md)章节进行操作。
...
使用集群调度组件提供的Volcano组件，可以实现交换机亲和性调度和昇腾AI处理器亲和性调度。Volcano是基于昇腾AI处理器的互联拓扑结构和处理逻辑，实现了昇腾AI处理器最佳利用的调度器组件，可以最大化发挥昇腾AI处理器计算性能。关于交换机亲和性调度和昇腾AI处理器亲和性调度的详细说明，可以参见[亲和性调度](../04_usage/basic_scheduling/01_affinity_scheduling/00_solution_description.md)。
...
1. 安装组件请参见[安装部署](../07_developer_guide/installation_deployment/manual_installation/00_obtaining_software_packages.md)章节进行操作。
2. 特性使用指导请参见[动态vNPU调度（推理）](../04_usage/virtual_instance/virtual_instance_with_hdk/dynamic_vnpu_scheduling/01_dynamic_vnpu_scheduling_inference.md)章节进行操作。
...
1. 安装组件请参见[安装部署](../07_developer_guide/installation_deployment/manual_installation/00_obtaining_software_packages.md)章节进行操作。
2. 特性使用指导请参见[软切分调度（推理）](../04_usage/virtual_instance/virtual_instance_with_vcann_rt/01_soft_allocation_scheduling_inference.md)章节进行操作。

### 弹性训练<a name="ZH-CN_TOPIC_0000002479226936"></a>

>[!NOTE]
>本章节描述的是基于Resilience Controller组件的弹性训练，该组件已经日落，相关资料将于2026年9月30日的版本删除。最新的弹性训练能力请参见[弹性训练](../04_usage/resumable_training/01_solutions_principles.md#弹性训练)。
...
1. 安装组件请参见[安装部署](../07_developer_guide/installation_deployment/manual_installation/00_obtaining_software_packages.md)章节进行操作。
2. 特性使用指导请参见[弹性训练](../06_references/elastic_training.md)章节进行操作。
...
- 安装组件请参见[安装部署](../07_developer_guide/installation_deployment/manual_installation/00_obtaining_software_packages.md)章节进行操作。
