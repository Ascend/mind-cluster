...
在无K8s的场景下，训练或推理进程异常后，没有有效的恢复手段。为了解决上述问题，可以配置容器恢复特性。若要支持容器恢复特性，需要安装Container Manager组件。Container Manager组件的安装操作详细请参见[安装部署](../../installation_guide/02_installation/manual_installation/00_obtaining_software_packages.md)。
...
Container Manager在感知到芯片处于RestartRequest、RestartBusiness、FreeRestartNPU和RestartNPU类型故障时，会按照命令run的启动参数“-ctrStrategy”配置的重启策略，进行容器停止与恢复。具体的容器停止与恢复的范围请参见[Container Manager启动参数](../../installation_guide/02_installation/manual_installation/11_container-manager.md#参数说明)。
