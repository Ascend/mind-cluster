...
|ASCEND_VNPU_SPECS|从物理NPU设备中切分出一定数量的AICore，指定为虚拟设备。支持的取值请参见[表1](../virtual_instance/virtual_instance_with_hdk/03_virtualization_templates.md)。<ul><li>只有支持动态虚拟化的产品形态，才能使用该参数。</li><li>需配合参数“ASCEND_VISIBLE_DEVICES”一起使用，参数“ASCEND_VISIBLE_DEVICES”指定用于虚拟化的物理NPU设备。</li></ul>|ASCEND_VNPU_SPECS=vir04表示切分4个AICore作为虚拟设备，挂载至容器中。|
