...
在具体操作前，用户需要参考[YAML配置说明](../05_api/yaml_configuration.md#yaml_configuration)了解相关YAML示例的参数说明，再进行操作。
