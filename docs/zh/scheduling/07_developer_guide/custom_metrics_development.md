...
- 通过文件方式开发自定义指标
    ...
    用户根据[自定义指标文件](../05_api/npu_exporter/03_custom_metrics_file.md)，创建符合要求的自定义指标文件。启动NPU Exporter时，配置"-textMetricsFilePath"参数，指定该自定义指标文件的路径。详情请参见[NPU Exporter启动参数](installation_deployment/manual_installation/03_npu_exporter.md#参数说明)。NPU Exporter会在每个数据采集周期读取自定义指标文件，并将文件内容上报给Prometheus或Telegraf。
