...
- 通过文件方式开发自定义指标
    ...
    用户根据[自定义指标文件](../06_api/00_npu_exporter/03_custom_metrics_file.md)，创建符合要求的自定义指标文件。启动NPU Exporter时，配置"-textMetricsFilePath"参数，指定该自定义指标文件的路径。详情请参见[NPU Exporter启动参数](00_installation_deployment/00_manual_installation/03_npu_exporter.md#参数说明)。NPU Exporter会在每个数据采集周期读取自定义指标文件，并将文件内容上报给Prometheus或Telegraf。
...
- 通过插件方式开发自定义指标
    ...
    用户可通过编写插件的方式自定义指标，使用该插件前，开发者需要自行学习了解cgo、go相关语言特性。用户可参考提供的demo，或将代码拷贝到plugins目录下，重新编译部署，下面对demo中各文件进行说明：
...
>
>- 自定义的指标不能与已有的指标名重复。
>- 自定义的指标名称、指标label名称仅支持使用字母、数字、下划线，且不能以数字开头。
>- 开发者需对自定义插件的稳定性负责，确保不引入运行时panic等问题。
>- 开发者需要对自定义指标文件格式的正确性负责。
>- 示例代码中的dcmi接口需要根据不同产品代际确定是否可用。
