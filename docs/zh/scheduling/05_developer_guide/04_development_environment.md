...
>[!NOTE]
>
> 如需调整工具链版本、追加依赖或插件，可直接修改`.devcontainer`目录下的`Dockerfile`与`devcontainer.json`，然后重新构建容器。
...
>[!NOTE]
>
>- 编译或执行用例前，建议先在对应组件（含go.mod的目录）下执行一次`go mod tidy`，补全、校正go.mod与go.sum的依赖，避免因依赖缺失或不一致导致编译、测试失败。
>- 从git拉取的脚本在部分环境下可能带有Windows（CRLF）换行符，直接执行会报`\r`、bad interpreter等换行符异常。编译、测试前建议先用`dos2unix`转换脚本换行格式，例如：对编译文件build.sh执行`dos2unix build.sh`。
