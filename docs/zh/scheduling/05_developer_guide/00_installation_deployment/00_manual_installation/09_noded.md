...
- 使用整卡调度、静态vNPU调度、动态vNPU调度、硬件故障恢复、硬件故障重调度、断点续训或弹性训练时，必须安装NodeD。
