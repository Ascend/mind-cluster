...
- 使用整卡调度、静态vNPU调度、动态vNPU调度、断点续训、弹性训练、硬件故障恢复或硬件故障重调度的用户，必须在管理节点安装**调度器**，该调度器可以是Volcano或其他调度器。
...
8. （可选）在“volcano-v<i>\{version\}</i>.yaml“中，配置Volcano使用的集群调度组件为用户提供的重调度时删除Pod的模式、虚拟化方式、交换机亲和性调度、是否自维护可用芯片状态等。
    ...
    **表 3**  参数说明
    ...
    <p id="p1704246153100"><a name="p1704246153100"></a><a name="p1704246153100"></a>取值范围：大于等于0；Atlas 900 A3 SuperPoD 超节点场景下为0~47。设置为0时表示不预留节点。</p>
