...
|-updateTime|int|无| **即将日落，不推荐使用**。全局配置指标更新周期，取值范围为1~60秒。推荐按分组配置指标更新周期，详见[配置文件](#section103551921135917)。<br>如果配置了updateTime参数，该参数仍然有效，且优先级高于metricConfiguration.json/pluginConfiguration.json配置文件中的intervalSeconds。|
|-ip|string|无| 参数无默认值，必须配置。<p>侦听IP地址，要求为合法的IPv4或IPv6格式，在多网卡主机上不建议配置成0.0.0.0。</p>                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|-version|bool|false| 是否查询NPU Exporter版本号与commit ID等详细信息。<ul><li>true：查询。</li><li>false：不查询。</li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                   |
