...
- 安装Kubernetes，支持1.17.x\~1.34.x版本（推荐使用1.19.x及以上版本），具体操作请参见[安装Kubernetes](https://kubernetes.io/zh/docs/setup/production-environment/tools/)，推荐[使用Kubeadm创建集群](https://kubernetes.io/zh-cn/docs/setup/production-environment/tools/kubeadm/create-cluster-kubeadm/)，集群初始化过程中的部分问题可参考[初始化Kubernetes失败](../faq.md#初始化kubernetes失败)。如需解除管理节点隔离，命令示例如下。
