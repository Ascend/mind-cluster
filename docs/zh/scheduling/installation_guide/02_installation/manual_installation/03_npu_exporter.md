...
8. 在任意节点执行以下命令，查看组件是否启动成功。
    ...
    >[!NOTE]
    >
    >- NPU Exporter的使用对进程环境有要求，以容器形式运行时，请确保“/sys“目录和容器运行时通信socket文件挂载至NPU Exporter容器中。若通过调用NPU Exporter的Metrics接口，没有获取到NPU容器的相关信息，该问题可能是因为socket文件路径不正确导致，可以参见[日志出现connecting to container runtime failed](https://gitcode.com/Ascend/mind-cluster/issues/346)章节进行处理。
