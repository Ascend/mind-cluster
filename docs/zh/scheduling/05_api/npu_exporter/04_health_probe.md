...
NPU Exporter启动组件内的HTTP健康探针服务，用于K8s livenessProbe机制探测组件存活状态。探针服务与NPU Exporter的业务HTTP服务（端口 8082）完全解耦，使用独立端口（默认 11251）。
...
> 探针启动参数说明详见[NPU Exporter 启动参数](../../07_developer_guide/installation_deployment/manual_installation/03_npu_exporter.md#参数说明)。
