...
<tr id="row416145918513"><td class="cellrowborder" valign="top" headers="mcps1.2.5.1.1 "><p id="p116115913528"><a name="p116115913528"></a><a name="p116115913528"></a>参数面光链路成员端口故障</p>
</td>
...
NodeD组件的配置文件NodeDConfiguration.json为系统配置文件，若用户无特殊需求，请勿随意修改。若用户需要修改故障码的故障级别，可以通过由NodeDConfiguration.json创建的mindx-dl-node-fault-config文件实现，操作指导请参见[（可选）配置节点硬件故障级别](#可选配置节点硬件故障级别)。故障级别说明及节点状态说明请参见[自定义节点故障](../../../05_api/noded.md#自定义节点故障)。
...
>[!NOTE]
>
>- 每个故障对应的故障码请参见[芯片故障码参考文档](../../../06_references/appendix.md#芯片故障码参考文档)章节。
...
Ascend Device Plugin从驱动获取到芯片故障码后，将根据故障码对设备及业务的影响将故障划分几个级别，详细说明请参见[表1](../../../05_api/ascend_device_plugin.md#自定义芯片故障)。
...
- faultCustomization.json文件的默认配置（默认值）请参见[表2](../../../05_api/ascend_device_plugin.md#自定义芯片故障)。
...
用户不手动修改faultCustomization.json文件时，Ascend Device Plugin按照faultCustomization.json的默认配置（默认值）进行故障处理。faultCustomization.json文件参数说明请参见[表2](../../../05_api/ascend_device_plugin.md#自定义芯片故障)。
...
针对芯片故障的不同级别进行分级处理时，ClusterD组件会获取到当前故障的故障码和故障级别，对于除了NotHandleFault和SubHealthFault级别之外的故障，根据ConfigMap（clusterd-config-cm）中配置的故障频率，将芯片状态置为人工隔离。该ConfigMap的参数说明请参见[表1](../../../07_developer_guide/installation_deployment/manual_installation/06_clusterd.md)。
...
在安装ClusterD时，会自动创建ConfigMap（clusterd-config-cm），作为当前人工隔离芯片的检测依据。该ConfigMap的参数说明请参见[表1](../../../07_developer_guide/installation_deployment/manual_installation/06_clusterd.md)。
...
3. <a name="li01010203"></a>执行以下命令，编辑clusterd-config-cm。
    ...
    根据实际情况，修改人工隔离芯片的故障频率。参数说明请参见[表1](../../../07_developer_guide/installation_deployment/manual_installation/06_clusterd.md)。
...
5. 等clusterd-config-cm更新生效（ClusterD的检测周期为300s）后，查看操作是否成功。
    ...
    2. 通过查询到的组件日志名称，查询ClusterD的组件日志信息。
        ...
        >- 若日志出现“node: xx, dev: xx, code: xx is found in manual fault cache, update last separate time”，表示已经触发人工隔离芯片的故障，再一次达到了人工隔离的故障频率，会刷新clusterd-manual-info-cm中的LastSeparateTime。clusterd-manual-info-cm的说明请参见[clusterd-manual-info-cm](../../../05_api/clusterd/00_cluster_resources.md#clusterd-manual-info-cm)。
...
Ascend Device Plugin从驱动获取到故障码后，将根据故障码对设备及业务的影响将故障划分为几种级别并进行相应的重调度处理，详细说明请参见[故障级别及处理说明表](../../../05_api/ascend_device_plugin.md#自定义灵衢设备故障)。
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>参数面光链路成员端口故障（路由不可收敛）</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>参数面光链路成员端口故障（UBOE故障）</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>参数面光链路成员端口故障（US故障或5808故障）</p>
</td>
...
|DetectDuration|连续检测到卡死状态的次数阈值，达到此次数后上报故障。|5|
...
## 参数面光链路成员端口故障<a name=”ZH-CN_TOPIC_0000002479387566”></a>
...
自定义时需对需配置的机型的形态进行区分，当配置Atlas 850 系列硬件产品中出uboe口的故障时，配置110001024和110000002。为其他及950形态时，配置020001002和110000002。
