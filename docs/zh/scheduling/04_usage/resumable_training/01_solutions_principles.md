...
- **MindSpore场景**
  ...
  3. 在分布式环境初始化完成，能够获取到全局rank之后，修改训练脚本，在训练脚本中拉起TaskD  Manager，在管理进程中拉起TaskD Proxy，在训练进程内部拉起TaskD  Worker。
      ...
      2. <a name="li2302301"></a>拉起TaskD  Worker。
          ...
          >[!NOTE]
          >以上代码init_taskd_worker(rank,5000,'ms')中的入参5000为/user/cluster-info/profiling的上限大小，详细说明请参见[def init\_taskd\_worker\(rank\_id: int, upper\_limit\_of\_disk\_in\_mb: int = 5000, framework: str = "pt"\) -\> bool](../../06_api/taskd/01_taskd_worker_apis.md#def-init_taskd_workerrank_id-int-upper_limit_of_disk_in_mb-int--5000-framework-str--pt---bool)中“upper\_limit\_of\_disk\_in\_mb”参数。
...
包括pod形态的npu到unions及5808的端口故障，以及server形态npu到1825及5808端口的故障等。
