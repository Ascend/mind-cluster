...
NPU发生故障时，故障管理框架获取到故障信息后，将该信息上传给NPU驱动的故障管理框架。故障管理框架收到故障信息后，通过DCMI接口上报给Ascend Device Plugin，如[图2](#fig3951191610267)所示。
...
**图 2**  芯片故障上报<a name="fig3951191610267"></a>
...
**图 3**  上报故障到K8s<a name="fig10951101692610"></a>
...
参数面网络故障检测由设备管理组件Ascend Device Plugin负责，详细原理如[图4](#fig68743107307)所示。

**图 4**  故障检测<a name="fig68743107307"></a>
...
Ascend Device Plugin的故障上报机制如[图5](#fig1587571063011)所示。

**图 5**  故障上报<a name="fig1587571063011"></a>
...
业务面故障发现原理如[图6](#fig1761563615337)所示。

**图 6**  发现原理<a name="fig1761563615337"></a>
...
**图 7**  公共故障上报<a name="fig72618571585"></a>
...
- 对于存在环境变量[MINDX\_TASK\_ID](../../06_api/13_environment_variable_description.md#ascend-operator环境变量说明)的Pod，rank 0数据在容器内的路径为/user/cluster-info/profiling/$MINDX\_TASK\_ID/0。
    ...
    >- /user/cluster-info/profiling达到配置的上限大小（PyTorch场景参考[4.b](#li23023)；MindSpore场景参考[3.b](#li2302301)）后，将进行文件老化，默认每次删除修改时间最早的20%个文件。
...
- 对于存在环境变量[MINDX\_TASK\_ID](../../06_api/13_environment_variable_description.md#ascend-operator环境变量说明)的Pod，rank 0数据在容器内的路径为/user/cluster-info/profiling/$MINDX\_TASK\_ID/0。
    ...
    >[!NOTE]
    >- 如无该环境变量，默认会落盘到名为default\_task\_id\_<i>时间戳</i>的文件夹内。
    >- /user/cluster-info/profiling达到配置的上限大小（PyTorch场景参考[步骤5](#li230238965)；MindSpore场景参考[步骤3](#li23023896501)）后，将进行文件老化，默认每次删除修改时间最早的20%个文件。
    >- 轻量profiling文件以时间戳命名，各条记录以换行分割，每次追加写入rank下最新文件。最新文件大小超过10MB时，TaskD会新建profiling文件。如果使用NFS等网络存储方式，当数据同步较慢时，可能存在文件大小未达到10MB即创建新文件的情况。
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.3 "><p id="p5282161815916"><a name="p5282161815916"></a><a name="p5282161815916"></a>适配因缩容num_microbatches等变化导致的精度梯度变化。</p>
</td>
...
在完成故障处理后，训练进程会被重新拉起，拉起的训练进程需要完成模型权重的保存和加载，才能回到任务中断时的训练状态。在正常训练中，每隔一段时间保存训练模型权重的CKPT（Checkpoint）文件，在任务中断后，新拉起的进程可以加载之前保存的CKPT文件，从而恢复到之前保存点的模型权重状态，减少训练时间。对于不同框架，保存和加载CKPT的方法不一样，以下给出了PyTorch、MindSpore保存和加载CKPT的示例，用户需按照示例修改自己的**训练脚本**。
