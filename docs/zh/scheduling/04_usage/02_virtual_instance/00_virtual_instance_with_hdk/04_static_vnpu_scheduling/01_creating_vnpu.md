...
- 当使用**动态虚拟化**时，无需提前创建vNPU，请跳过本节，直接在容器拉起时按以下要求进行参数配置。
    - 使用Ascend Docker Runtime：参考[动态vNPU调度（推理）](../05_dynamic_vnpu_scheduling/01_dynamic_vnpu_scheduling_inference.md)，通过ASCEND\_VISIBLE\_DEVICES和ASCEND\_VNPU\_SPECS参数从物理芯片上虚拟化出多个vNPU并挂载至容器。
    - 使用MindCluster集群调度组件（Ascend Device Plugin和Volcano）：参考[动态vNPU调度（推理）](../05_dynamic_vnpu_scheduling/01_dynamic_vnpu_scheduling_inference.md)，运行任务时自动按照配置要求调用接口创建vNPU。
