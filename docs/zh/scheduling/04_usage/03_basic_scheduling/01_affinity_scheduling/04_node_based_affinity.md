...
**故障重调度<a name="section1614032710011_reschedule"></a>**
...
- 逻辑超节点的数量必须小于物理超节点数量。
- 逻辑超节点内的节点必须在同一个物理超节点内。
- 逻辑超节点内的NPU卡的rank ID是连续的。
- 下发任务时，建议在每一个物理超节点内预留备份节点，用于故障任务重调度后的快速恢复。
...
- 若逻辑超节点中的某些节点发生硬件故障不可用，则从故障节点所在物理超节点选取节点，其他健康节点保持不变。
- 若物理超节点中剩余节点已不能满足逻辑超节点，则逻辑超节点上的任务全部调度到其他物理超节点。
- 仅仅发生过软件故障的节点，在重调度时可能还会使用该发生过业务故障的节点。
- 若开启了soft软亲和特性（即任务的super-pod-affinity参数值配置为soft，在集群资源不满足超节点亲和性时，支持任务使用集群中碎片资源继续调度），不支持Pod级别重调度。
...
- 若逻辑框中的某些节点发生硬件故障不可用，则从故障节点所在物理框选取节点，其他健康节点保持不变。
