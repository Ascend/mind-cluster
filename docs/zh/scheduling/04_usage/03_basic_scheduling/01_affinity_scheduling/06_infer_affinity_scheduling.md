...
对于Atlas 950 SuperPoD 超节点，同框内节点间网络通信时延最低、吞吐性能最优，同一超节点内不同框的节点之间通信性能次之；对于Atlas 850E 超节点，同一超节点内节点之间的网络通信表现最佳。
基于上述网络特性，Ascend-for-volcano调度插件支持配置推理服务亲和性调度策略：对于Atlas 950 SuperPoD 超节点，优先将同一推理服务的全部实例调度至同一框内，无法满足时，优先将同服务实例调度至同一超节点；对于Atlas 850E 超节点，优先将同服务实例调度至同一超节点，以此充分发挥网络优势，整体提升推理服务运行性能。
...
对于Atlas 950 SuperPoD 超节点，以Deployment资源为例，需要在其Pod模板的labels中添加如下加粗部分内容：
  ...
  template:
    metadata:
      ...
      annotations:
        ...
        huawei.com/schedule_policy: "chip8-node8-ra64-sp" # Atlas 950 SuperPoD 超节点对应的调度策略
    spec:
...
对于Atlas 850E 超节点，以Deployment资源为例，推理服务亲和性调度策略的配置与Atlas 950 SuperPoD 超节点类似，添加如下加粗部分内容：
...
spec:
  ...
  template:
    metadata:
      ...
      annotations:
        sp-block: "8" # 指定逻辑超节点芯片数量，设置成该实例请求的NPU总数
        huawei.com/schedule_policy: "chip8-node8-sp" # Atlas 850E 超节点对应的调度策略
    spec:
      schedulerName: volcano
      automountServiceAccountToken: false
      nodeSelector:
...
> [!NOTE]
>
>- 推理服务亲和性调度策略仅支持Atlas 950 SuperPoD 超节点与Atlas 850E 超节点。
