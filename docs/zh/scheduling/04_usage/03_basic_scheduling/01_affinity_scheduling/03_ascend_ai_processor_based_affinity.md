...
### Atlas 850 系列硬件产品（超节点服务器）<a name="ZH-CN_TOPIC_0000002479386929"></a>

Atlas 850 系列硬件产品（超节点服务器）的昇腾AI处理器是华为研发的高性能AI处理器，其内部由2个计算DIE和2个IO DIE合封成一个芯片。每个Atlas 850 系列硬件产品（超节点服务器）包含8个昇腾AI处理器。昇腾AI处理器之间采用HCCS互联的方式组成Mesh全连接。服务器之间通过5808交换机连接，每个5808下面最多连接16个Atlas 850 系列硬件产品（超节点服务器），最多128个昇腾AI处理器。两层5808交换机组网可支持1k规模超节点，可根据需求灵活定制超节点规模。

单机任务可申请昇腾AI处理器的数量为1-8，申请的昇腾AI处理器需要优先占满整个节点。而分布式任务只支持满卡调度，即配置为8。调度优先级按照通信效率从高到低排序，即优先调度到节点内，若节点资源不足，则调度到超节点内的昇腾AI处理器。

### Atlas 950 SuperPoD<a name="ZH-CN_TOPIC_0000002479386930"></a>
...
**图 4** Atlas 950 SuperPoD互联拓扑<a name="fig1920102817166"></a>
...
Atlas 训练系列产品的昇腾AI处理器的特征和资源利用的规则如[表2](#table1436611225137)所示。

**表 2** Atlas 训练系列产品的AI处理器亲和性策略
...
### Atlas 350 标卡服务器（无互联节点内8卡）

#### 亲和性调度策略

Atlas 350 标卡服务器（无互联节点内8卡）的特征和资源利用的规则如下表所示。

**表 10**  Atlas 350 标卡服务器（无互联节点内8卡）亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在Atlas 350 标卡服务器中，同时下发2卡、6卡任务，存在2卡和6卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|
...
### Atlas 350 标卡服务器（无互联节点内16卡）

#### 亲和性调度策略

Atlas 350 标卡服务器（无互联节点内16卡）的特征和资源利用的规则如下表所示。

**表 11**  Atlas 350 标卡服务器（无互联节点内16卡）亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在Atlas 350 标卡服务器中，同时下发2卡、14卡任务，存在2卡和14卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|
...
### Atlas 350 标卡服务器（4P mesh 8卡）

#### 亲和性调度策略

Atlas 350 标卡服务器（4P mesh 8卡）的特征和资源利用的规则如下表所示。

**表 12** Atlas 350 标卡服务器（4P mesh 8卡）亲和性策略
...
- 服务器内安装8个Atlas 350 标卡，每个Atlas 350 标卡中包含1个昇腾AI处理器，每4个Atlas 350 标卡通过扣板组成FullMesh互联。
- 单机/分布式任务申请的昇腾AI处理器数量支持1、2、3、4、8（满足亲和性），而申请5、6、7时则不保证亲和性。
- 当多个任务Pod调度到单个节点时，不支持Pod间的集合通信。

### Atlas 350 标卡服务器（4P mesh 16卡）

#### 亲和性调度策略

Atlas 350 标卡服务器（4P mesh 16卡）的特征和资源利用的规则如下表所示。

**表 13** Atlas 350 标卡服务器（4P mesh 16卡）亲和性策略
...
- 服务器内安装16个Atlas 350 标卡，每个Atlas 350 标卡中包含1个昇腾AI处理器，每4个Atlas 350 标卡通过扣板组成FullMesh互联。
...
**表 14** Atlas 850 系列硬件产品（普通集群）亲和性策略
...
**表 15** Atlas 850 系列硬件产品（超节点）亲和性策略
