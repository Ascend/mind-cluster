...
### <term>Atlas 训练系列产品</term><a name="ZH-CN_TOPIC_0000002511346851"></a>

<term>Atlas 训练系列产品</term>的昇腾AI处理器是华为研发的一款高性能AI处理器。其内部的处理器之间采用HCCS（例如：A0\~A3为一个HCCS）方式连接。

每台设备具备两个HCCS环共8个昇腾AI处理器（A0\~A7）。每个HCCS存在4个昇腾AI处理器，同一HCCS内AI处理器可做数据交换，不同HCCS内昇腾AI处理器不能通信。同一Pod分配的昇腾AI处理器（若小于或等于4）必须在同一个HCCS环内，否则任务运行失败。<term>Atlas 训练系列产品</term>互联拓扑如[图1](#fig17063331201)所示，其中K0\~K3为鲲鹏处理器。

**图 1** <term>Atlas 训练系列产品</term>互联拓扑<a name="fig17063331201"></a>
...
Atlas 200T A2 Box16 异构子框和Atlas 200I A2 Box16 异构子框的昇腾AI处理器是华为研发的高性能AI处理器，其内部AI处理器之间采用HCCS互联的方式相连接。每台Atlas 200T A2 Box16 异构子框或Atlas 200I A2 Box16 异构子框具备两个HCCS互联共16个昇腾AI处理器，每个HCCS互联存在8个昇腾AI处理器，同一HCCS互联之间可以做数据交换，不同HCCS互联之间不能通信。即任务分配的昇腾AI处理器（若小于或等于8）必须在同一个HCCS互联内，否则任务运行失败。Atlas 200T A2 Box16 异构子框和Atlas 200I A2 Box16 异构子框的互联拓扑图如[图2](#fig1920102817143)所示。

**图 2** Atlas 200T A2 Box16 异构子框和Atlas 200I A2 Box16 异构子框互联拓扑<a name="fig1920102817143"></a>
...
**图 3** 灵衢总线设备节点网络<a name="fig1054553210321"></a>
...
### Atlas 850E 超节点、Atlas 850 超节点<a name="ZH-CN_TOPIC_0000002479386929"></a>

Atlas 850E 超节点、Atlas 850 超节点的昇腾AI处理器是华为研发的高性能AI处理器，其内部由2个计算DIE和2个IO DIE合封成一个芯片。每个超节点包含8个昇腾AI处理器。昇腾AI处理器之间采用HCCS互联的方式组成mesh全连接。服务器之间通过5808交换机连接，每个5808下面最多连接16个超节点，最多128个昇腾AI处理器。两层5808交换机组网可支持1k规模超节点，可根据需求灵活定制超节点规模。

单机任务可申请昇腾AI处理器的数量为1-8，申请的昇腾AI处理器需要优先占满整个节点。而分布式任务只支持满卡调度，即配置为8。调度优先级按照通信效率从高到低排序，即优先调度到节点内，若节点资源不足，则调度到超节点内的昇腾AI处理器。

### Atlas 950 SuperPoD 超节点<a name="ZH-CN_TOPIC_0000002479386930"></a>

Atlas 950 SuperPoD 超节点的昇腾AI处理器是华为研发的高性能AI处理器，其内部由2个计算DIE和2个IO DIE合封成一个芯片。每个Atlas 950 SuperPoD 超节点包含8个昇腾AI处理器。昇腾AI处理器之间采用HCCS互联的方式组成mesh全连接，构成1D-FullMesh中的X轴。Atlas 950 SuperPoD 超节点的机架包含8个OS，机架内Y轴同位置的昇腾AI处理器通过LRS实现跨板互联，8个OS共64个昇腾AI处理器实现2D全互联，构成2D-FullMesh中的Y轴。Atlas 950 SuperPoD 超节点的机架之间通过LRS输出UB x128 IO用于Z轴连接，多个机架组成一个物理超节点，形成直接的全互联，构成3D-FullMesh中的Z轴。匹配LLM流量局部性，X-Y-Z轴提供逐级收敛带宽。

**图 4** Atlas 950 SuperPoD 超节点互联拓扑<a name="fig1920102817166"></a>
...
<term>Atlas 训练系列产品</term>的昇腾AI处理器的特征和资源利用的规则如[表2](#table1436611225137)所示。

**表 2** <term>Atlas 训练系列产品</term>的AI处理器亲和性策略
...
**<term>Atlas 训练系列产品</term>的资源申请约束<a name="section84725398320"></a>**
...
根据亲和性策略和业务模型梳理出的场景如[表3](#table1226225517318)所示。

**表 3** <term>Atlas 训练系列产品</term>亲和性策略场景
...
Atlas 200T A2 Box16 异构子框和Atlas 200I A2 Box16 异构子框的特征和资源利用的规则如[表4](#table768417221315)所示。

**表 4** Atlas 200T A2 Box16 异构子框和Atlas 200I A2 Box16 异构子框亲和性策略
...
Atlas 900 A3 SuperPoD 超节点的资源利用规则如[表5](#table42428468401)所示。

**表 5** Atlas 900 A3 SuperPoD 超节点亲和性策略
...
Atlas 800I A3 超节点服务器和Atlas 800T A3 超节点服务器的资源利用规则如[表6](#table424284684011)所示。

**表 6** Atlas 800I A3 超节点服务器和Atlas 800T A3 超节点服务器亲和性策略
...
A200T A3 Box8 超节点服务器的资源利用规则如[表7](#table424284684013)所示。

**表 7** A200T A3 Box8 超节点服务器亲和性策略
...
推理服务器（插Atlas 300I 推理卡）的特征和资源利用的规则如[表8](#table768417221325)所示。

**表 8** 推理服务器（插Atlas 300I 推理卡）亲和性策略

<a name="table768417221325"></a>
...
**表 9** 推理服务器（插Atlas 300I Duo 推理卡）亲和性策略
...
### 服务器（插Atlas 350 加速卡）（无互联节点内8卡）

#### 亲和性调度策略

服务器（插Atlas 350 加速卡）（无互联节点内8卡）的特征和资源利用的规则如下表所示。

**表 10** 服务器（插Atlas 350 加速卡）（无互联节点内8卡）亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在服务器（插Atlas 350 加速卡）中，同时下发2卡、6卡任务，存在2卡和6卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|

#### 资源申请约束

- Atlas 350 加速卡内有1个昇腾AI处理器，内部无互联。
- 单机/分布式任务申请的昇腾AI处理器数量支持1-8。
- 当多个任务Pod调度到单个节点时，不支持Pod间的集合通信。

### 服务器（插Atlas 350 加速卡）（无互联节点内16卡）

#### 亲和性调度策略

服务器（插Atlas 350 加速卡）（无互联节点内16卡）的特征和资源利用的规则如下表所示。

**表 11** 服务器（插Atlas 350 加速卡）（无互联节点内16卡）亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在服务器（插Atlas 350 加速卡）中，同时下发2卡、14卡任务，存在2卡和14卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|
...
### 服务器（插Atlas 350 加速卡）（4P mesh 8卡）

#### 亲和性调度策略

服务器（插Atlas 350 加速卡）（4P mesh 8卡）的特征和资源利用的规则如下表所示。

**表 12**  服务器（插Atlas 350 加速卡）（4P mesh 8卡）亲和性策略
...
- 服务器内安装8个Atlas 350 加速卡，每个Atlas 350 加速卡中包含1个昇腾AI处理器，每4个Atlas 350 加速卡通过扣板组成FullMesh互联。
- 单机/分布式任务申请的昇腾AI处理器数量支持1、2、3、4、8（满足亲和性），而申请5、6、7时则不保证亲和性。
- 当多个任务Pod调度到单个节点时，不支持Pod间的集合通信。

### 服务器（插Atlas 350 加速卡）（4P mesh 16卡）

#### 亲和性调度策略

服务器（插Atlas 350 加速卡）（4P mesh 16卡）的特征和资源利用的规则如下表所示。

**表 13** 服务器（插Atlas 350 加速卡）（4P mesh 16卡）亲和性策略
...
- 服务器内安装16个Atlas 350 加速卡，每个Atlas 350 加速卡中包含1个昇腾AI处理器，每4个Atlas 350 加速卡通过扣板组成FullMesh互联。
- 单机/分布式任务申请的昇腾AI处理器数量支持1、2、3、4、8、12、16（满足亲和性），而申请5、6、7、9、10、11、13、14、15时则不保证亲和性。
- 当多个任务Pod调度到单个节点时，不支持Pod间的集合通信。

### Atlas 650E 服务器和Atlas 650 服务器

#### 亲和性调度策略

Atlas 650E 服务器和Atlas 650 服务器的特征和资源利用的规则如下表所示。

**表 14** Atlas 650E 服务器和Atlas 650 服务器亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在服务器中，同时下发2卡、6卡任务，存在2卡和6卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|

#### 资源申请约束

- 服务器内8个昇腾AI处理器，内部8个昇腾处理器全互联。
- 单机/分布式任务申请的昇腾AI处理器数量支持1-8（满足亲和性）。
- 当多个任务Pod调度到单个节点时，不支持Pod间的集合通信。

### Atlas 850E 超节点和Atlas 850 超节点

#### 亲和性调度策略

Atlas 850E 超节点和Atlas 850 超节点的特征和资源利用的规则如下表所示。

**表 15** Atlas 850E 超节点和Atlas 850 超节点亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在超节点中，同时下发2卡、6卡任务，存在2卡和6卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|
|2|优先剩余保留节点|当超节点保留节点为2，两个超节点中分别剩余3个节点和2个节点时，优先选择剩余3个节点的超节点。|
|3|优先占满超节点|当超节点保留节点为2，两个超节点中分别剩余4个节点和3个节点时，优先选择剩余3个节点的超节点。|
...
Atlas 950 SuperPoD 超节点的特征和资源利用的规则如下表所示。

**表 16** Atlas 950 SuperPoD 超节点亲和性策略

|优先级|策略名称|策略描述|
|--|--|--|
|1|优先占满节点|节点芯片数量越少，优先级越高。<p>下发单机任务时，任务存在未按照优先占满调度原则占满某个节点。说明如下：</p><ul><li>现象说明：如在Atlas 950 SuperPoD 超节点中，同时下发2卡、6卡任务，存在2卡和6卡任务未调度到同一个节点。</li><li>原因分析：因为Volcano调度完一个任务后，Ascend Device Plugin上报调度后的昇腾AI处理器的拓扑结构到mindx-dl-deviceinfo-${node_name}存在时延，导致Volcano校验该节点昇腾AI处理器数量失败，将任务调度到其他节点上。</li></ul>|
|2|优先剩余保留节点|当超节点保留节点为2，两个超节点中分别剩余3个节点和2个节点时，优先选择剩余3个节点的超节点。|
|3|优先占满超节点|当超节点保留节点为2，两个超节点中分别剩余4个节点和3个节点时，优先选择剩余3个节点的超节点。|
...
**<term>Atlas 训练系列产品</term>分布式亲和性策略<a name="section141921857164115"></a>**
...
**Atlas 650E 服务器、Atlas 650 服务器分布式亲和性策略<a name="section225141656114618"></a>**

- 任务申请的昇腾AI处理器数量不能大于节点的昇腾AI处理器总数。
- 当任务申请的昇腾AI处理器数量小于或等于8个时，需要将任务调度到同一个服务器内。

**Atlas 850E 超节点、Atlas 850 超节点分布式亲和性策略<a name="section235141656114619"></a>**

- Atlas 850E 超节点、Atlas 850 超节点分布式任务每个节点申请的昇腾AI处理器个数固定为8个，服务器内8卡为FullMesh全互联。
- 若为逻辑超节点亲和任务，即任务YAML中的sp-block字段配置了逻辑超节点大小，则申请的昇腾AI处理器数量只能为sp-block的倍数。

**Atlas 950 SuperPoD 超节点分布式亲和性策略<a name="section245141656114620"></a>**

- Atlas 950 SuperPoD 超节点分布式任务每个节点申请的昇腾AI处理器个数固定为8个，OS内8卡为FullMesh全互联。
- 若为逻辑超节点亲和任务，即任务YAML中的sp-block字段配置了逻辑超节点大小，则申请的昇腾AI处理器数量只能为sp-block的倍数。
