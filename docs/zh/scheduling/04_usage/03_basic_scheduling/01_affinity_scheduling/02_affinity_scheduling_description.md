...
|Atlas 350 标卡服务器（4P mesh 8/16卡）|每个Atlas 350 标卡中包含1个昇腾AI处理器，每4个Atlas 350 标卡通过扣板组成FullMesh互联。|<p>单机/分布式任务调度，必须将任务调度到整张Atlas 350 标卡内的1个或多个4P中。</p><p>若任务需要的昇腾AI处理器数量为单数时，使用单个昇腾AI处理器的部分，将优先调度到剩余昇腾AI处理器数量为1的Atlas 350 标卡。</p>|若两个资源的网络情况一致，则选择调度后产生的资源碎片少的资源。|
