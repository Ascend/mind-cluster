...
|Atlas 650E 服务器|每个服务器内8个昇腾AI处理器通过UBC形成8P mesh，服务器间通过PCIe互联。|<p>单机任务调度，必须将任务调度到某个服务器中。</p><p>若任务需要的昇腾AI处理器数量为单数时，使用单个昇腾AI处理器的部分，将优先调度到剩余昇腾AI处理器数量为1的Atlas 650E 服务器。</p>|若两个资源的网络情况一致，则选择调度后产生的资源碎片少的资源。|
|Atlas 850E 超节点|每个服务器内8个昇腾AI处理器通过HCCS互联组成mesh全连接，服务器间通过5808交换机互联，每个交换机下最多连接16个服务器，最多128个昇腾AI处理器。|<p>单机任务可申请昇腾AI处理器的数量为1-8，需要优先占满整个节点；分布式任务每个节点固定为8个昇腾AI处理器。</p>|若两个资源的网络情况一致，则选择调度后产生的资源碎片少的资源（优先级依次为：优先占满节点、优先剩余保留节点、优先占满超节点）。|
...
- 采用灵衢+RoCE连接的产品：Atlas 900 A3 SuperPoD 超节点、Atlas 850E 超节点、Atlas 950 SuperPoD 超节点
