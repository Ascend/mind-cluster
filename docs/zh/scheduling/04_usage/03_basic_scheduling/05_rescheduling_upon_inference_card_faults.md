# 硬件故障重调度<a name="ZH-CN_TOPIC_0000002479387124"></a>

## 使用前必读<a name="ZH-CN_TOPIC_0000002479387116"></a>

故障重调度特性指的是当K8s集群中的硬件资源出现故障之后，集群调度组件能够自动检测到硬件故障并上报隔离，然后根据用户下发的任务配置的重调度策略将其Pod重新调度到新的健康节点或芯片上。

本章节下述内容以推理场景为例，介绍集群调度组件管理的推理芯片资源出现故障后，对故障资源（对应芯片）进行隔离并自动进行重调度。

**前提条件<a name="section166381652174516"></a>**

- 使用推理场景故障重调度特性，需要确保已经安装如下组件。
...
- [通过命令行使用](#ZH-CN_TOPIC_0000002511427039)：安装集群调度组件，通过命令行使用推理场景故障重调度特性。
...
通过命令行使用推理场景故障重调度特性流程可以参见[图1](#zh-cn_topic_0000001559979444_fig242524985412)。
...
- 推荐从[昇腾镜像仓库](https://www.hiascend.com/developer/ascendhub)根据用户的系统架构（ARM或者x86\_64）下载推理基础镜像（如：[mindie](https://www.hiascend.com/developer/ascendhub/detail/af85b724a7e5469ebd7ea13c3439d48f)）。

    >[!NOTE]
    >基础镜像中不包含推理模型、脚本等文件，因此，用户需要根据自己的需求进行定制化修改（如加入推理脚本代码、模型等）后才能使用。

- （可选）如果用户需要更个性化的推理环境，可基于已下载的推理基础镜像，再[使用Dockerfile对其进行修改](../../07_references/02_common_operations.md#使用dockerfile构建容器镜像pytorch)。
...
1. 下载YAML文件。
    ...
    **表 1**  任务类型与硬件型号对应YAML文件
    ...
    <td class="cellrowborder" valign="top" width="20%" headers="mcps1.2.5.1.4 "><p id="p784716567219"><a name="p784716567219"></a><a name="p784716567219"></a><a href="https://gitcode.com/Ascend/mindxdl-deploy/tree/branch_v26.1.0/samples/inference/volcano/infer-deploy-310p-1usoc.yaml" target="_blank" rel="noopener noreferrer">获取YAML</a></p>
    </td>
    </tr>
    <tr>
    ...
    <td class="cellrowborder" valign="top" headers="mcps1.2.5.1.3 "><p><a href="https://gitcode.com/Ascend/mindxdl-deploy/blob/branch_v26.1.0/samples/inference/volcano/infer-deploy.yaml" target="_blank" rel="noopener noreferrer">获取YAML</a></p>
    </td>
    </tr>
    ...
    <td class="cellrowborder" rowspan="1" valign="top" width="20%" headers="mcps1.2.5.1.4 "><p id="p15442424164"><a name="p15442424164"></a><a name="p15442424164"></a><a href="https://gitcode.com/Ascend/mindxdl-deploy/blob/branch_v26.1.0/samples/inference/volcano/infer-vcjob-910.yaml" target="_blank" rel="noopener noreferrer">获取YAML</a></p>
    </td>
    </tr>
    <tr>
    <td class="cellrowborder" valign="top" headers="mcps1.2.5.1.1 "><p>Atlas 950 SuperPoD</p><p>Atlas 850 系列硬件产品（超节点）</p><p>Atlas 350 标卡</p></td>
...
2. 在[整卡调度](./03_full_npu_scheduling.md#准备任务yaml)或者[动态vNPU调度](../02_virtual_instance/00_virtual_instance_with_hdk/05_dynamic_vnpu_scheduling/01_dynamic_vnpu_scheduling_inference.md#准备任务yaml)的YAML配置基础上，增加如下字段启用重调度功能，以整卡调度的infer-deploy.yaml为例。
...
### 查看推理场景故障重调度结果<a name="ZH-CN_TOPIC_0000002511347069"></a>
