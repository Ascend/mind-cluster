...
|ASCEND_UB_DRV_MOUNT|是否挂载UnifiedBus驱动相关文件功能，在Atlas 850E 超节点、Atlas 650E 服务器和Atlas 950 SuperPoD 超节点等需要支持UnifiedBus通信协议的场景下，通过指定该参数决定是否挂载宿主机的UnifiedBus驱动相关文件到容器的功能。<p>其他设备如<term>Atlas 训练系列产品</term>、<term>Atlas A2 训练系列产品</term>和<term>Atlas A3 训练系列产品</term>等可以使用该参数，但因其不依赖UnifiedBus驱动相关文件的场景，所以无需额外指定该参数。</p>|<ul><li>ASCEND_UB_DRV_MOUNT=False，表示在Atlas 850E 超节点、Atlas 650E 服务器和Atlas 950 SuperPoD 超节点场景下不挂载UnifiedBus驱动相关文件，此时可通过在容器内安装UnifiedBus驱动用户态软件包的方式支持相关功能。</li><li>ASCEND_UB_DRV_MOUNT=True或者不指定该参数，Atlas 850E 超节点、Atlas 650E 服务器和Atlas 950 SuperPoD 超节点将挂载UnifiedBus驱动相关文件。由于UnifiedBus驱动相关文件存在软链接，为了成功挂载，会自动跳过软链接校验，无需指定ASCEND_ALLOW_LINK=True环境变量。</li></ul>|
