...
自定义指标文件中的字段说明如[表1](#ZH-CN_TOPIC_0000002501343480_table5395205714441)所示。通过自定义指标文件可开发自定义指标，详情请参见[自定义指标开发](../../05_developer_guide/01_custom_metrics_development.md)。
...
| name      | string    | 指标名称。不能为空，长度不能超过128。仅支持使用字母、数字、下划线，且不能以数字开头。                                  |
| desc      | string    | 指标的详情介绍。不能为空，长度不能超过1024。                                                      |
| timestamp | timestamp | 指标的更新时间戳，单位为us。                                                               |
| data_list | list      | 非空数组，长度不能超过128。                                                               |
| -value    | float     | 指标的值。                                                                         |
| -label    | json      | 指标的标签，JSON的key和value都必须是字符串类型。其中key仅支持使用字母、数字、下划线，且不能以数字开头。JSON的子元素个数不能超过10个。 |

## 约束说明<a name="section342514924413"></a>

- 不支持软链接。
- 不支持指定一个目录。
- 指定的文件路径不能超过10个，多个文件之间以逗号分隔。
