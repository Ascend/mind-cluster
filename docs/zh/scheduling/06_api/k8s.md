...
|huawei.com/scheduler.softShareDev.policy|标记当前Pod执行的软切分任务的策略。| <ul><li>fixed-share：固定配额模式，各vNPU按配比严格执行时间片。<br> <ul><li> 若有暂未分配的vNPU资源，则在最后一个vNPU时间片消耗完，睡眠一段时间。此时NPU将闲置空转。</li> <li> 某vNPU上无任务运行，仍会给定比例消耗完该vNPU的时间片。</li></ul></li><li>elastic：弹性模式，由于固定配额模式可能出现NPU资源浪费，在此模式下，在各vNPU按配比执行时间片的基础上，为提高卡资源利用率，优化了调度逻辑。<ul><li>最后一个vNPU时间片消耗完，马上将NPU使用权释放给下一个vNPU，跳过睡眠逻辑。</li> <li>在某vNPU上无任务运行，则直接跳过未消耗完的时间片，切换至另一个vNPU。</li> <li>时间片借用机制：正在执行的vNPU在识别到卡资源空闲的情况下，允许其他vNPU借用该时间片内浪费的资源，使卡上资源不被浪费，进而提升整卡利用率。</li></ul></li><li>best-effort：争抢模式，vNPU之间自行争抢NPU资源，该模式的资源利用率最高，但无法保证vNPU的QoS。</li></ul> |Volcano、Ascend Device Plugin|
