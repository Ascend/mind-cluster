...
- To uninstall Container Manager, see [Uninstalling Container Manager](#section1461059103619).
- To uninstall NPU Exporter, Ascend Device Plugin, K8s RDMA Shared Dev Plugin, Volcano, ClusterD, Ascend Operator, Infer Operator, NodeD, and Resilience Controller, see [Uninstalling Other Components](#section6361146202520).
...
>- Uninstalling Ascend Docker Runtime will restore the default runtime of Docker or Containerd to runc.

1. (Optional) Disable pingmesh UnifiedBus network detection.
...
2. <a name="li345320287225"></a>You can choose one of the following methods to uninstall Ascend Docker Runtime.
    - Method 1: (Recommended) Use the software package to uninstall
        ...
        2. Run the following uninstall command to uninstall Ascend Docker Runtime in the default installation path.
            ...
            The command output is as follows, indicating a successful uninstallation.
    ...
    - Method 2: Use the script to uninstall
        ...
        2. Run the uninstallation script to uninstall.
            ...
            - Docker scenario (or K8s integration with Docker)
...
4. Restart the service.
    ...
    >[!NOTE]
    > After the restart, some Pods on the node may report errors. An example of the error message is as follows:
    >
    > ```ColdFusion
    ...
    >  ```
    >
    > Solution: Delete the faulty Pod using the command `kubectl delete pod --force -n {pod_namespace} {pod_name}` and wait for the Pod to be recreated.
...
2. Go to the directory where the Container Manager package is extracted, and run the `deploy.sh` script to uninstall the Container Manager service.

    ```shell
    bash deploy.sh uninstall
    ```

    The uninstall script performs the following operations in sequence: stops and disables the systemd service and timer, deletes the systemd unit files, and deletes the binary files. The log directory is retained by default. Manually delete it if cleanup is required.

    The following command output indicates a successful uninstallation:
...
3. (Optional) Delete the log files. Confirm the actual path before deletion.
...
2. Uninstall components. Choose the corresponding uninstallation method based on how they were installed.
    - Uninstall via image method. The uninstallation method is similar for each component: navigate to the directory containing the component's YAML configuration file and perform a delete operation. This operation must be performed on the K8s management node. The following uses uninstalling Ascend Device Plugin as an example; you should complete the uninstallation of the remaining components on your own.
        ...
        3. In the Ascend Device Plugin installation environment, run the following command to uninstall Ascend Device Plugin.
    ...
    - Uninstall via the binary method. The following uses NPU Exporter as an example. You need to uninstall the remaining components yourself.
...
4. Delete log files. Refer to the [Creating Log Directories](./00_manual_installation/01_preparing_for_installation.md#optional-creating-log-directories) section and delete the log directories of the cluster scheduling components on the corresponding nodes. Take ClusterD as an example. Confirm before deleting.
