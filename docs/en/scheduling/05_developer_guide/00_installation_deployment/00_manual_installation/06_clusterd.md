...
- To detect slow node and slow network faults, you need to install ClusterD. For details, see [Slow Node and Slow Network Faults](../../../04_usage/04_resumable_training/01_solutions_principles.md#slow-nodes--slow-network-faults).
...
4. (Optional) In `clusterd-v{version}.yaml`, configure the detection switch for manually isolated chips, fault frequency, de-isolation time, etc.
    ...
    **Table 1** Parameter description of manually_separate_policy.conf
    ...
    |release|fault_free_hours|int|Time to unisolate, indicating the time elapsed since the last isolation triggered by reaching the frequency threshold. After this time, isolation will be removed. The value range is [1, 240] or -1, the default value is 48, and the unit is h (hour).<ul><li>The time when the frequency threshold was last reached is the LastSeparateTime in clusterd-manual-info-cm. For details about clusterd-manual-info-cm, see [clusterd-manual-info-cm](../../../06_api/04_clusterd/00_cluster_resources.md#clusterd-manual-info-cm).</li><li>Configuring -1 disables the unisolation function.</li><li>When automatic unisolation is performed upon reaching the unisolation time, the isolation will be removed regardless of whether the fault has recovered.</li></ul>|
...
<td class="cellrowborder" valign="top" width="40%" headers="mcps1.2.5.1.4 "><p id="phz002d-clusterd"><a name="phz002d-clusterd"></a><a name="phz002d-clusterd"></a>The port number on which the health check service listens, with a value range of 1025 to 65535. Configured as 11253 via the component YAML during K8s deployment. If the specified port is occupied, the component fails to start. </p>
</td>
</tr>
