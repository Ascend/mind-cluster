...
**Table 3**  Default mount directories and files (Atlas 200I A2 acceleration module and Atlas 200I DK A2 developer kit)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Configuration file of driver dynamic library.</td>
    </tr>
    <tr>
      <td>/etc/slog.conf</td>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.4 "><p id="p19200837239"><a name="p19200837239"></a><a name="p19200837239"></a>7 during installation, 6 during uninstallation</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.4 "><p id="p82017312312"><a name="p82017312312"></a><a name="p82017312312"></a>8 during installation, 7 during uninstallation</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.4 "><p id="p0201736235"><a name="p0201736235"></a><a name="p0201736235"></a>9 during installation, 8 during uninstallation</p>
</td>
</tr>
...
**Table 1** Fault code references

<a name="table87909405314"></a>

|Product|Reference Document|
|--|--|
|Inference Server (with Atlas 300I Inference Card)|<span>[Atlas 300I Inference Card Black Box Error Code Information List (Models 3000, 3010)](https://support.huawei.com/enterprise/en/doc/EDOC1100463874)</span>|
...
## Process‑Level Online Recovery Validation<a name="ZH-CN_TOPIC_0000002511426299"></a>

This section guides you through the adaptation steps for validating process‑level online recovery by stubbing the training code to simulate an on‑chip memory UCE.

>[!NOTE]
>
>- The modifications described in this section are for validation purposes only in a test environment. Do not deploy this stubbed version to a production environment.
>- Before proceeding with this section, ensure that training can be started normally and that process‑level online recovery has been configured.
>- To ensure proper operation of process‑level online recovery, keep the clocks of the K8s cluster master and worker nodes synchronized.
>- The code snippets provided below may differ from the actual version; always refer to the code of the actual release.
...
1. <a name="li977718409381"></a>Pull the MindCluster code.
    ...
    git checkout v26.0.0   # v26.0.0 is a repository version tag; switch to your target version accordingly
    ```
...
2. Modify the ClusterD code.
    ...
    2. Press `i` to enter insert mode and add the following bold code.
        ...
        …
              ...
              <strong>collector.ReportInfoCollector.ReportRetryInfo(podInfo.jobId, deviceInfo.RankID, constant.JobNotRecover, constant.UceFaultType)   // Set the service‑plane fault time to an invalid value to prevent duplicate triggers of process‑level online recovery for a single fault</strong>
            }
        …</pre>
...
3. <a name="li114977117517"></a>Compile ClusterD.
    ...
    sed -i 's|build_version="v[^"]\+"|build_version="xxx"|g' build.sh  # Replace xxx with the version number, e.g., v26.0.0
    sed -i 's|export CGO_ENABLED=0|export CGO_ENABLED=1|g' build.sh  # Enable CGO
    ./build.sh # Compiling ClusterD requiring Go 1.21 or later (recommended: 1.21)
    ```

    After a successful compilation, the relevant files will be generated in the `../output/` directory. You can verify this with:
...
4. <a name="li89701053589"></a>Navigate to the `output` directory and build the ClusterD image.

    ```shell
    cd ../output/
    docker build --no-cache -t clusterd:{tag} ./  # {tag} must match the value of build_version="xxx" used in step 3
    ```

5. (Optional) Save the image, and upload the saved image file and the `clusterd-{tag}.yaml` file to the master node. If [step l](#li977718409381) through [step 4](#li89701053589) were performed on the master node, this step can be skipped.

    ```shell
    docker save -o clusterd.tar clusterd:{tag}  # Save the image
    docker load -i clusterd.tar  # Load the image on the master node
    ```
...
6. Restart ClusterD on the master node.
    ...
    kubectl apply -f  clusterd-{tag}.yaml  # Start a new container
    ```
...
1. Set up the training environment and start training. For details, see [PyTorch Adaptation Example (Based on MindSpeed-LLM)](./usage/resumable_training/07_using_resumable_training_on_the_cli.md#adaptation-example).
2. Enable online process-level recovery. For details, see [Configuring Process-Level Online Recovery](./usage/resumable_training/04_configuring_fault_handling_policies.md#configuring-process-level-online-recovery).
3. Add the following bold code to `QWEN3_for_PyTorch_2.7_code/mindspeed_llm/training/training.py` to inject a fault via a stub. The new code retrieves the fault‑injection iteration position and fault rank information from the environment variable `RAISE_UCE_ERROR_STEP_AND_RANK`.
...
4. Modify the startup script `QWEN3_for_PyTorch_2.7_code/scripts/train_start.sh`.
    ...
    ```shell
    …
    export RAISE_UCE_ERROR_STEP_AND_RANK="{3:1,10:2}"  # Configure fault injection iteration and rank: inject UCE faults at iteration 3 on rank 1 and iteration 10 on rank 2
    sed -i 's/check_memory_result = torch_npu.npu.check_uce_in_memory(device)/check_memory_result = ha_constant.UCE_HIGH_LEVEL/g' /job/code/mindspeed_llm/core/high_availability/tft_stop_clean.py # Modify the PTA return value to identify the exception thrown by the training code as a UCE fault
    …
    ```

#### MindSpore Adaptation Example (Based on MindFormers)<a name="ZH-CN_TOPIC_0000002511346369"></a>

1. Set up the training environment and start training. For details, see [MindSpore Adaptation Example (Based on MindFormers)](./usage/resumable_training/07_using_resumable_training_on_the_cli.md#adaptation-example).
2. Enable online process-level recovery. For details, see [Configuring Process-Level Online Recovery](./usage/resumable_training/04_configuring_fault_handling_policies.md#configuring-process-level-online-recovery).
3. Add the following bold code to `QWEN3_for_MS_code/mindformers/core/callback/callback.py` to inject a fault via a stub.
...
<pre class="screen" id="screen1066354754917"><a name="screen1066354754917"></a><a name="screen1066354754917"></a>resources:
  requests:
    cpu: 35000m</pre>
<div class="note" id="note1956143192814"><a name="note1956143192814"></a><a name="note1956143192814"></a><span class="notetitle">[!NOTE]</span><div class="notebody"><p id="p55611931112817"><a name="p55611931112817"></a><a name="p55611931112817"></a>The overall CPU usage of the API Server is not limited by this parameter.</p>
</div></div>
</td>
...
<div class="note" id="note989123811420"><a name="note989123811420"></a><a name="note989123811420"></a><span class="notetitle">[!NOTE]</span><div class="notebody"><p id="p188918381649"><a name="p188918381649"></a><a name="p188918381649"></a>--auto-compaction-retention does not actually release space. You need to manually use etcdctl compact and etcdctl defrag to clean up space.</p>
</div></div>
</td>
</tr>
...
- Developing custom metrics using a file
    ...
    You create a custom metrics file that meets the requirements based on the [Custom Metrics File](./api/npu_exporter/03_custom_metrics_file.md). When starting NPU Exporter, configure the `-textMetricsFilePath` parameter to specify the path to this custom metrics file. For details, see [NPU Exporter Startup Parameters](./installation_guide/03_installation/manual_installation/03_npu_exporter.md). NPU Exporter reads the custom metrics file in each data collection cycle and reports the file content to Prometheus or Telegraf.
