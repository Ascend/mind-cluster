...
- The host machine must have the drivers and firmware installed. For details, see "Installing the NPU Driver and Firmware" in the [CANN Software Installation Guide (Commercial)](https://www.hiascend.com/document/detail/en/canncommercial/900/softwareinst/instg/instg_0005.html?Mode=PmIns&InstallType=local&OS=Debian) or "Installing the NPU Driver and Firmware" in the [CANN Software Installation Guide (Community)](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/900/softwareinst/instg/instg_0005.html?Mode=PmIns&InstallType=local&OS=openEuler).
...
|Software Name|Supported Version|Installation Location| Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|--|--|--|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Kubernetes | 1.17.x ~ 1.34.x (1.19.x and later recommended) <ul><li>It is recommended to choose the latest bugfix version.</li><li>If Volcano is required, install Kubernetes 1.19.x or later. For specific Kubernetes versions, refer to the [Volcano official website for corresponding Kubernetes versions](https://github.com/volcano-sh/volcano/blob/master/README.md#kubernetes-compatibility).</li></ul> | All nodes | For usage guidance, see the [Kubernetes documentation](https://kubernetes.io/docs/). |
| (Optional) Docker | 18.09.x ~ 28.5.1 | All nodes | Available from the [Docker community or official website](https://docs.docker.com/engine/install/). The Docker version must be compatible with Kubernetes; refer to the Kubernetes [release notes](https://github.com/kubernetes/kubernetes/tree/master/CHANGELOG) or the Kubernetes community for compatibility information. It is recommended to choose the latest bugfix version. <div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody"><p>Lower versions of Docker restrict the `clone` system call for security reasons, but the interception policy is overly strict and may incorrectly block legitimate thread creation. Volcano's internal CGO components rely on native threading capabilities; if thread creation is blocked, it will directly cause process crashes. Therefore, when using openEuler as the base image to build Volcano component images, Docker version must be no lower than <b>20.10.10</b>.</p></div></div> |
| (Optional) Containerd | 1.4.x ~ 2.1.4 (1.6.x recommended) | All nodes | Available from the [Containerd official website](https://containerd.io/downloads/) or [community](https://github.com/containerd/containerd/blob/main/docs/getting-started.md#installing-containerd). It is recommended to choose the latest bugfix version. Pay attention to the [CRI version](https://kubernetes.io/docs/setup/production-environment/container-runtimes/#cri-versions) compatible with your Kubernetes version. |
| Ascend driver and firmware | Select the driver and firmware compatible with MindCluster based on the actual hardware device model. | Compute nodes | For guidance, refer to the [Driver and Firmware Installation and Upgrade Guide](https://support.huawei.com/enterprise/en/ascend-computing/ascend-hdk-pid-252764743) for each hardware product. <div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody"><p>To ensure that NPU Exporter can be installed by a non-root user (e.g., hwMindX) in binary deployment, use the `--install-for-all` parameter when installing the driver. Example:</p><pre class="screen">./Ascend-hdk-&lt;chip_type&gt;-npu-driver_&lt;version&gt;_linux-&lt;arch&gt;.run --full --install-for-all</pre></div></div> |
| (Optional) CANN | If only cluster scheduling components are installed, CANN is not required. Users can choose to install the required CANN package based on actual needs; refer to the version compatibility table for the corresponding software package. | Compute nodes or training/inference containers | For installing CANN package on the host, refer to the [CANN Software Installation Guide](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/910/softwareinst/instg/instg_0000.html?Mode=PmIns&InstallType=netconda&OS=openEuler). <div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody">If using soft partitioning virtualization, CANN must be installed. The compatible CANN version is 8.5.0. For details, see [vCANN-RT](https://gitcode.com/openeuler/ubs-virt/blob/master/ubs-virt-enpu/vcann-rt/README.md).</div></div> |
| Python | 3.8 ~ 3.12 | Inside training or inference containers | The Python version should align with the specific AI framework being used. |
...
Divide the network planes into:
...
- Parameter plane: Used for parameter exchange between training nodes during distributed training.
...
|Component |CPU (Unit: Core)|Memory (Unit: GB)|
|--|--|--|
|Ascend Device Plugin|0.5|0.5|
|NodeD|0.5|0.3|
|NPU Exporter|1|1|
|K8s RDMA Shared Dev Plugin|0.5|0.5|
