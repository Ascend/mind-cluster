...
1. Run the following command to check whether the configuration file has been modified successfully.
    ...
    - Docker (or K8s integration with Docker scenario).
        ...
        The following example output indicates a successful modification.
    ...
    >[!NOTE]
    >If the configuration file path for Docker or Containerd does not match the actual path on your system, modify it accordingly.
...
2. Run the following command to check whether the base image exists.
    ...
    - Containerd (or K8s integration with Containerd scenario).
        ...
        The following is an example of the command output, indicating that the base image `ubuntu:22.04` exists. If the base image does not exist, run the `ctr images pull docker.io/library/ubuntu:22.04` command to pull the base image.
...
3. Run the following command to mount the chip with physical chip ID 0 using Ascend Docker Runtime.
...
4. Run the following command to check whether the chip is mounted successfully.
...
1. Check the component service status. The component status must be `active (running)`.
    ...
    >[!NOTE]
    >If information similar to the following appears in the output, you can ignore it. It does not affect actual functionality and may be caused by the RoCE NIC IP address and subnet mask not being configured. If you do not want this information to be printed, see the "[Configuration Function > Configuring the RoCE NIC IP Address and Subnet Mask](https://support.huawei.com/enterprise/en/doc/EDOC1100568362/83923a94)" section in *Atlas A2 Center Inference and Training Hardware 26.0.RC1 HCCN Tool Interface Reference* for configuration.
    >
    >```ColdFusion
