...
- MindCluster MindIO Try To Persist (MindIO TTP) is designed to accelerate fault recovery during large model training. When a fault occurs during training, it verifies the integrity and consistency of intermediate state data, generates a dying gasp checkpoint, and uses this checkpoint to resume training, thereby reducing the loss of training iterations caused by the fault.
- MindCluster MindIO Uncorrectable Memory Error (MindIO UCE) is designed for UCE fault in on-chip memory or network fault during large model training and performs online repair to achieve process-level online recovery functionality.
...
Additionally, for LLM training, the time required to save and load checkpoint data for each iteration is similarly long, comparable to the periodic checkpoint save and load time. MindIO UCE provides online repair. When a UCE fault or network fault occurs on an NPU, it first performs fault cleanup, recovery, and data rollback to resume training and restore the state just before the failure, minimizing training stop/restart time. If repair fails, it falls back to the TTP process as a safeguard.
...
- MindIO UCE
    - Once a UCE or network fault is detected, online repair begins immediately.
    - In large model training, whether using periodic checkpoint saving or MindIO TFT's dying gasp checkpoint saving, the cost of retraining is substantial. This feature provides process-level online recovery capability within the training framework, eliminating the need to restart processes while ensuring minimal iteration loss during training resumption. If it fails, the process falls back to the TTP flow.
