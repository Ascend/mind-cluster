...
- To use static virtualization, run the `npu-smi` command in this section to create vNPUs and mount them to containers and then refer to [Mounting vNPUs](./06_mounting_vnpu.md) section to mount them to the container.
- To use dynamic virtualization, skip this section, as there is no the need to create vNPUs in advance. Set parameters as follows during container startup.
    - Ascend Docker Runtime: Refer to [Method 1: Mounting vNPUs Using Ascend Docker Runtime](./06_mounting_vnpu.md#method-1-mounting-vnpus-using-ascend-docker-runtime) to virtualize multiple vNPUs from the physical chip and mount them to the container through the `ASCEND_VISIBLE_DEVICES` and `ASCEND_VNPU_SPECS` parameters.
    - Ascend Device Plugin and Volcano: Refer to [Dynamic Virtualization](./06_mounting_vnpu.md#dynamic-virtualization). vNPUs are automatically created by APIs based on configuration requirements during job running.
