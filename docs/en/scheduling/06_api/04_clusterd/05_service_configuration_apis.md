...
ClusterD generates `global-ranktable` in the RankTable field as the return message. Some fields in `global-ranktable` come from the `hccl.json` file. For details about the `hccl.json` file, see [hccl.json File Description](../14_hccl.json_file_description.md).
...
- Example (Atlas A3 training products):
...
|super_device_id|Unique identifier of the NPU within Atlas A3 training products/SuperPoD|
