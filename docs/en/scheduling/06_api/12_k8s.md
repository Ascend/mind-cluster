...
|--|--|--|--|
|accelerator|Automatically labeled by Ascend Device Plugin; no manual configuration required by the user.|<ul><li>huawei-npu</li><li>huawei-Ascend910</li><li>huawei-Ascend310</li><li>huawei-Ascend310P</li></ul>|Ascend Device Plugin|
...
|node.kubernetes.io/npu.chip.name| Reports the specific type of the current chip.|<ul><li>310</li><li>310P1</li><li>310P2</li><li>310P3</li><li>310P4</li><li>{xxx}A</li><li>910PremiumA</li><li>910ProA</li><li>910ProB</li><li>{xxx}Bx(where x can be 1, 2, 3, or 4)</li><li>Ascend950PR</li><li>Ascend950DT</li></ul>|<p>Ascend Device Plugin</p><div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody">{*xxx*} takes "910" as the chip model number.</div></div>|
|workerselector|Identifies the compute node of MindCluster.|dls-worker-node|Ascend Device Plugin, NodeD, NPU Exporter|
|servertype|Device type|<ul><li>npu-{aicore_core_count}</li><li>soc</li><li>Ascend910-{aicore_core_count}</li><li>Ascend310P-{aicore_core_count}</li></ul>|Volcano, Ascend Device Plugin|
|huawei.com/driver.version|HDK driver version number|The corresponding HDK version number of the node.|Ascend Device Plugin|
|<p>huawei.com/Ascend910-Recover</p><p>huawei.com/npu-Recover</p>|Fault recovery identifier for <term>Atlas training products</term>|Faulty chip ID|Ascend Device Plugin|
|<p>huawei.com/Ascend910-NetworkRecover</p><p>huawei.com/npu-NetworkRecover</p>|Network fault recovery identifier for <term>Atlas training products</term>|Faulty chip ID|Ascend Device Plugin|
|infer-card-type| Written by Ascend Device Plugin, indicating the inference card type of the node.|card-300i-duo|Volcano|
|mind-cluster/npu-chip-memory|On-chip memory of the chip|64G|Volcano, Ascend Device Plugin|
|huawei.com/scheduler.chip1softsharedev.enable|Indicates whether the node supports the soft-partitioning virtualization feature.|<ul><li>true</li><li>false</li></ul>|Volcano, Ascend Device Plugin<div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody"><ul><li>huawei.com/scheduler.chip1softsharedev.enable=true indicates that the node supports the soft-partitioning virtualization feature.</li><li>huawei.com/scheduler.chip1softsharedev.enable=false indicates that the node does not support the soft-partitioning virtualization feature.</li></ul></div></div>|
|huawei.com/topotree.rackid|Identifies the rack ID of the node.|The rack ID to which the node belongs.|Volcano|
|huawei.com/topotree.serverid| Identifies the index ID of the node within the SuperPoD.|The index ID within the SuperPoD to which the node belongs|Volcano|
|huawei.com/topotree.superpodid|Identifies the SuperPoD ID of the node.|The SuperPoD ID to which the node belongs.|Volcano|
|huawei.com/topotree.groupid| Identifies the Pod group ID of the node.|The Pod group ID to which the node belongs.|Volcano|
|huawei.com/topotree|Identifies the network topology tree ID of the node.|The network topology tree ID to which the node belongs.|Volcano|
...
|huawei.com/schedule_policy|Specifies the scheduling policy.|Currently supports the configurations in [Table 3 huawei.com/schedule_policy Configuration Description](./01_volcano.md#podgroup).|Volcano|
|volcano.sh/schedule.plugin|When NPU resources are not configured in MinResources, this annotation is used to declare the NPU type that the job uses, so that Volcano still schedules the job according to NPU affinity scheduling rules.| The value is NPU type, for example, Ascend910, Ascend310P, and Ascend310.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |Volcano|
...
|product-serial-number|NodeD obtains the node SN through the IPMI and writes it into the annotation for use when ClusterD receives a public fault.|String|ClusterD|
