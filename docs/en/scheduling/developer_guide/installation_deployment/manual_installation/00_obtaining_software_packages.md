...
| Volcano | Ascend-mindxdl-volcano\_<i>{version}</i>\_linux-<i>{arch}</i>.zip | Volcano package.<div class="note"><span class="notetitle">Note:</span><div class="notebody"><p>Select an appropriate version for installation based on the compatibility between K8s and open-source Volcano. For specific versions, see [the official Volcano official website](https://github.com/volcano-sh/volcano/blob/master/README.md#kubernetes-compatibility).</p><ul><li>The K8s version range compatible with Volcano v1.7.0 is 1.19.x to 1.28.x.</li><li>The K8s version range compatible with Volcano v1.9.0 is 1.21.x to 1.29.x.</li><li>The K8s version range compatible with Volcano v1.12.0 is 1.21.x to 1.34.x.</li></ul></div></div> | [Download Link](https://gitcode.com/Ascend/mind-cluster/releases/v26.0.0) |
...
| K8s RDMA Shared Dev Plugin | Ascend-mindxdl-k8s-rdma-shared-dev-plugin\_<i>{version}</i>\_linux-<i>{arch}</i>.zip | K8s RDMA Shared Dev Plugin package| [Download Link](https://gitcode.com/Ascend/mind-cluster/releases/v26.0.0) |
...
The cluster scheduling system provides open-source components such as Ascend Docker Runtime, NPU Exporter, Ascend Device Plugin, K8s Rdma Shared Dev Plugin, Volcano, Ascend Operator, NodeD, and ClusterD. If you need to understand the source code or customize component development, obtain the corresponding component source code according to [Table 2](#table978944123012).
