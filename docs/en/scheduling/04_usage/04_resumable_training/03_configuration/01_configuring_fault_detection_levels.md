...
<tr id="row416145918513"><td class="cellrowborder" valign="top" headers="mcps1.2.5.1.1 "><p id="p116115913517"><a name="p116115913517"></a><a name="p116115913517"></a>Public fault</p>
</td>
...
<tr id="row416145918513"><td class="cellrowborder" valign="top" headers="mcps1.2.5.1.1 "><p id="p116115913528"><a name="p116115913528"></a><a name="p116115913528"></a>Job hang fault</p>
</td>
<td class="cellrowborder" colspan="3" valign="top" headers="mcps1.2.5.1.2 "><p id="p147536536728"><a name="p147536536728"></a><a name="p147536536728"></a>NotHandleFault, SeparateNPU, PreSeparateNPU</p>
</td>
</tr>
<tr id="row416145918513"><td class="cellrowborder" valign="top" headers="mcps1.2.5.1.1 "><p id="p116115913528"><a name="p116115913528"></a><a name="p116115913528"></a>Optical link member port faults</p>
</td>
<td class="cellrowborder" colspan="3" valign="top" headers="mcps1.2.5.1.2 "><p id="p147536536728"><a name="p147536536728"></a><a name="p147536536728"></a>SeparateNPU, PreSeparateNPU, SubHealthFault</p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" valign="top" width="35.74%" headers="mcps1.2.5.1.2 "><p id="zh-cn_topic_0000002395188553_zh-cn_topic_0000002171521445_p1354813311915"><a name="zh-cn_topic_0000002395188553_zh-cn_topic_0000002171521445_p1354813311915"></a><a name="zh-cn_topic_0000002395188553_zh-cn_topic_0000002171521445_p1354813311915"></a>Handled based on the value of the subHealthyStrategy parameter configured in the job YAML. For details, see <a href="../../../06_api/15_yaml_configuration.md">YAML Configuration Description</a>.</p>
</td>
<td class="cellrowborder" valign="top" width="23.39%" headers="mcps1.2.5.1.3 "><p id="zh-cn_topic_0000002395188553_zh-cn_topic_0000002171521445_p3352524125220"><a name="zh-cn_topic_0000002395188553_zh-cn_topic_0000002171521445_p3352524125220"></a><a name="zh-cn_topic_0000002395188553_zh-cn_topic_0000002171521445_p3352524125220"></a>When a sub-health fault occurs on the chip, it needs to be handled according to the <a href="./02_configuring_fault_handling_policies.md#ZH-CN_TOPIC_0000002511426471">Configuring YAML</a>.</p>
...
- ClusterD determines faults based on the job dimension.
    - Each time a fault occurs on a chip, the fault frequency count for that chip is incremented by one.
    - If multiple cards in the same job experience the same fault within 30 seconds, it is considered not to be caused by a hardware fault and will not be counted toward the fault frequency. This rule applies to most scenarios, but the count may be biased in cases such as residual processes remaining after a Pod is deleted.
    - Only newly occurring faults will trigger the frequency upper-limit check. If the configured threshold is adjusted to a value equal to or lower than the current count, the isolation determination will not be triggered until the next fault occurs.
    - After ClusterD restarts, frequency count information will be lost, and the fault frequency will be reset to zero and restart counting.
    - After isolation is removed, if job scheduling does not meet expectations, check whether the label `huawei.com/scheduler.chip1softsharedev.enable=false` exists on the node. If it does, it needs to be deleted.
...
>[!NOTE]
>
>- For the fault code corresponding to each fault, see the [Chip Fault Code References](../../../07_references/05_appendix.md#chip-fault-code-references) section.
...
After Ascend Device Plugin obtains the chip fault code from the driver, it classifies the fault into several levels based on the impact of the fault code on the device and service. For details, see [Table 1](../../../06_api/02_ascend_device_plugin.md#custom-chip-faults).
...
- For the default configuration (default values) of the `faultCustomization.json` file, see [Table 2](../../../06_api/02_ascend_device_plugin.md#custom-chip-faults).
...
7. Wait for the `mindx-dl-fault-config` file update to take effect (`PollInterval` defaulted to `300s` if not specified), then check whether the operation is successful.
    ...
    2. Use the queried log name to query the component log information of Ascend Device Plugin.
        ...
        If the log shows "load fault code from configmap success", it indicates that the manual fault code configuration was successful.
...
8. (Optional) Manually restore a forcibly isolated chip. When the fault handling policy is `ManuallySeparateNPU`, the chip remains isolated after fault recovery. If the release conditions are not met and you need to manually restore the forcibly isolated chip, do as follows.
    ...
    3. Delete the name of the recovered, healthy chip following `ManuallySeparateNPU` under `data`.
        ...
        >[!NOTE]
        >Delete all chip names after the `ManuallySeparateNPU` field and set the value to `""`.
...
When ClusterD is installed, the ConfigMap (`clusterd-config-cm`) is automatically created as the detection basis for manually isolated chips. For parameter descriptions of this ConfigMap, see [Table 1](../../../05_developer_guide/00_installation_deployment/00_manual_installation/06_clusterd.md).
...
3. <a name="li01010203"></a>Run the following command to edit `clusterd-config-cm`.
    ...
    Modify the fault frequency for manual chip isolation based on the actual situation. For parameter descriptions, see [Table 1](../../../05_developer_guide/00_installation_deployment/00_manual_installation/06_clusterd.md).
...
>[!NOTE]
>Only <term>Atlas A3 training products</term> have **bus devices**, and the fault codes for such devices can be viewed in the `SwitchFaultCode.json` file.
...
3. <a name="zh-cn_topic_0000002007978080_li14147485427"></a>Run the following command to create the `mindx-dl-fault-config` required for dynamic fault code configuration.
    ...
    **Table 2** Parameter description
    ...
    <td class="cellrowborder" valign="top" width="54.96549654965496%" headers="mcps1.2.4.1.3 "><p id="zh-cn_topic_0000002007978080_p161481689426"><a name="zh-cn_topic_0000002007978080_p161481689426"></a><a name="zh-cn_topic_0000002007978080_p161481689426"></a>The name of the ConfigMap file required for<span id="zh-cn_topic_0000002007978080_ph214819813425"><a name="zh-cn_topic_0000002007978080_ph214819813425"></a><a name="zh-cn_topic_0000002007978080_ph214819813425"></a> dynamically configuring fault codes.</span> This file name cannot be modified.</p>
    </td>
    </tr>
    ...
    <td class="cellrowborder" valign="top" width="54.96549654965496%" headers="mcps1.2.4.1.3 "><p id="zh-cn_topic_0000002007978080_p17148118174218"><a name="zh-cn_topic_0000002007978080_p17148118174218"></a><a name="zh-cn_topic_0000002007978080_p17148118174218"></a>Used to store fault codes. Must be kept consistent with the file name SwitchFaultCode.json.</p>
    </td>
    </tr>
    <tr id="zh-cn_topic_0000002007978080_row1714868114216"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="zh-cn_topic_0000002007978080_p182611591223"><a name="zh-cn_topic_0000002007978080_p182611591223"></a><a name="zh-cn_topic_0000002007978080_p182611591223"></a>faultCode.json</p>
    </td>
    <td class="cellrowborder" valign="top" width="11.701170117011701%" headers="mcps1.2.4.1.2 "><p id="zh-cn_topic_0000002007978080_p1314868184218"><a name="zh-cn_topic_0000002007978080_p1314868184218"></a><a name="zh-cn_topic_0000002007978080_p1314868184218"></a>Yes</p>
    </td>
    <td class="cellrowborder" valign="top" width="54.96549654965496%" headers="mcps1.2.4.1.3 "><p id="zh-cn_topic_0000002007978080_p17148118174219"><a name="zh-cn_topic_0000002007978080_p17148118174219"></a><a name="zh-cn_topic_0000002007978080_p17148118174219"></a>Used to store basic fault codes. Must be kept consistent with the file name faultCode.json.</p>
    </td>
    </tr>
    <tr id="zh-cn_topic_0000002007978080_row1714868114217"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="zh-cn_topic_0000002007978080_p182611591224"><a name="zh-cn_topic_0000002007978080_p182611591224"></a><a name="zh-cn_topic_0000002007978080_p182611591224"></a>PollInterval</p>
    </td>
    <td class="cellrowborder" valign="top" width="11.701170117011701%" headers="mcps1.2.4.1.2 "><p id="zh-cn_topic_0000002007978080_p1314868184219"><a name="zh-cn_topic_0000002007978080_p1314868184219"></a><a name="zh-cn_topic_0000002007978080_p1314868184219"></a>No</p>
    </td>
    <td class="cellrowborder" valign="top" width="54.96549654965496%" headers="mcps1.2.4.1.3 "><p id="zh-cn_topic_0000002007978080_p17148118174220"><a name="zh-cn_topic_0000002007978080_p17148118174220"></a><a name="zh-cn_topic_0000002007978080_p17148118174220"></a>Configures the fault code polling interval, in seconds. The value range is 30 to 3600. If not specified, the default value is 300s.</p>
    </td>
    </tr>
    </tbody>
    </table>
...
## Public Faults<a name="ZH-CN_TOPIC_0000002479386564"></a>

### Configuration File Description<a name="ZH-CN_TOPIC_0000002511346487"></a>

Resumable training performs hierarchical processing for different levels of public faults. ClusterD obtains the fault code of the current fault and processes the fault accordingly based on the fault level configured for the fault code in the `publicFaultConfiguration.json` file. In special cases, if ClusterD receives an unrecognized fault code (not saved in the configuration file), it will discard this fault.

[publicFaultConfiguration.json](#zh-cn_topic_0000002181110120_table8202741102717) is the system configuration file for public faults. Do not modify it arbitrarily unless you have special requirements. If you need to modify the level and sender of public faults, you can do so by writing a custom configuration file named `publicCustomization.json` to `/user1/mindx-dl/clusterd`. The path to this file is configurable. The configuration method is as follows:
...
<td class="cellrowborder" valign="top" width="71.07%" headers="mcps1.2.3.1.2 "><p id="zh-cn_topic_0000002181110120_p220284110271"><a name="zh-cn_topic_0000002181110120_p220284110271"></a><a name="zh-cn_topic_0000002181110120_p220284110271"></a>Configuration related to public fault codes.</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="71.07%" headers="mcps1.2.3.1.2 "><p id="zh-cn_topic_0000002181110120_p1606118102218"><a name="zh-cn_topic_0000002181110120_p1606118102218"></a><a name="zh-cn_topic_0000002181110120_p1606118102218"></a>Configuration of the public fault sender.</p>
</td>
</tr>
</tbody>
</table>

**Table 3** `publicFaultCode` description
...
The fault code for public faults is 9 digits, as described below.
...
**Known Public Faults<a name="zh-cn_topic_0000002181110120_section4960201383813"></a>**
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="zh-cn_topic_0000002181110120_p1171822134316"><a name="zh-cn_topic_0000002181110120_p1171822134316"></a><a name="zh-cn_topic_0000002181110120_p1171822134316"></a>NPU <span id="ph17233131243911"><a name="ph17233131243911"></a><a name="ph17233131243911"></a>HCCS </span>network fault</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p1628951244719"><a name="p1628951244719"></a><a name="p1628951244719"></a>Optical link loose connection (chip fault)</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p316816364194"><a name="p316816364194"></a><a name="p316816364194"></a>Optical link loose connection (network fault)</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p10112152210476"><a name="p10112152210476"></a><a name="p10112152210476"></a>Optical module device sub-health</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p13241019443"><a name="p13241019443"></a><a name="p13241019443"></a>In backup SuperPoD scenarios, scheduling uses non-existent backup rack resources.</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p1424190104412"><a name="p1424190104412"></a><a name="p1424190104412"></a>Backup rack resource port fault</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p124800442"><a name="p124800442"></a><a name="p124800442"></a>Backup rack job ID occupation conflict</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p10241109444"><a name="p10241109444"></a><a name="p10241109444"></a>Suspected partial failure of backup rack link ports</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p18631743123513"><a name="p18631743123513"></a><a name="p18631743123513"></a>Slow network not caused by card fault (slow network fault)</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p7277854132820"><a name="p7277854132820"></a><a name="p7277854132820"></a>Insufficient shared storage DPC memory</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p7277855532820"><a name="p7277855532820"></a><a name="p7277855532820"></a>Insufficient file system capacity</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p7277819132820"><a name="p7277819132820"></a><a name="p7277819132820"></a>On-chip memory multi-bit ECC error (restart)</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p7272364132820"><a name="p7272364132820"></a><a name="p7272364132820"></a>On-chip memory multi-bit ECC error (isolation)</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p><a name="p7277854132820"></a><a name="p7277854132820"></a>Shared storage DTFS process exception</p>
</td>
...
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p><a name="p7277854132820"></a><a name="p7277854132820"></a>Shared storage DTFS link exception</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p><a name="p182781654132815"></a><a name="p182781654132815"></a>PreSeparateNPU</p>
</td>
</tr>
<tr id="row_hangfault001"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="p_hangfault001_code"><a name="p_hangfault001_code"></a><a name="p_hangfault001_code"></a>200001002</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>Job hang fault</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p_hangfault001_level"><a name="p_hangfault001_level"></a><a name="p_hangfault001_level"></a>NotHandleFault</p>
</td>
</tr>
<tr id="row_hangfault001"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="p_hangfault001_code"><a name="p_hangfault001_code"></a><a name="p_hangfault001_code"></a>020000002</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>Hyper-plane optical link member port fault (route can converge)</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p_hangfault001_level"><a name="p_hangfault001_level"></a><a name="p_hangfault001_level"></a>SubHealthFaultCodes</p>
</td>
</tr>
<tr id="row_hangfault001"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="p_hangfault001_code"><a name="p_hangfault001_code"></a><a name="p_hangfault001_code"></a>020001002</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>Hyper-plane optical link member port fault (route cannot converge)</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p_hangfault001_level"><a name="p_hangfault001_level"></a><a name="p_hangfault001_level"></a>SeparateNPUCodes</p>
</td>
</tr>
<tr id="row_hangfault001"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="p_hangfault001_code"><a name="p_hangfault001_code"></a><a name="p_hangfault001_code"></a>110001024</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>Parameter-plane optical link member port fault (UBOE fault, route cannot converge)</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p_hangfault001_level"><a name="p_hangfault001_level"></a><a name="p_hangfault001_level"></a>PreSeparateNPUCodes</p>
</td>
</tr>
<tr id="row_hangfault001"><td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.1 "><p id="p_hangfault001_code"><a name="p_hangfault001_code"></a><a name="p_hangfault001_code"></a>110000002</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.2 "><p id="p_hangfault001_desc"><a name="p_hangfault001_desc"></a><a name="p_hangfault001_desc"></a>Parameter-plane optical link member port fault (UBOE fault, route can converge)</p>
</td>
<td class="cellrowborder" valign="top" width="33.33333333333333%" headers="mcps1.2.4.1.3 "><p id="p_hangfault001_level"><a name="p_hangfault001_level"></a><a name="p_hangfault001_level"></a>SubHealthFaultCodes</p>
</td>
</tr>
...
### (Optional) Configuring the Level and Sender of Public Faults<a name="ZH-CN_TOPIC_0000002479226494"></a>
...
**Configuring the Level of Public Fault Codes<a name="zh-cn_topic_0000002180950420_section1384121854711"></a>**

Configuring the level of public fault codes is divided into the following two scenarios.

- Adjusting the level of existing fault codes.
- Adding new fault codes and their fault levels.

    The following uses fault code `010001008` as an example to describe how to configure a public fault code level.
...
**Configuring the Sender of Public Faults<a name="zh-cn_topic_0000002180950420_section5532327614"></a>**

The following uses the new fault sender XXX as an example to describe the steps for configuring the sender of public fault codes.
...
## Configuring Job Hang Fault Detection<a name="ZH-CN_TOPIC_0000002479387566"></a>

### Configuration File Description<a name="ZH-CN_TOPIC_0000002479387566_section_specifications "></a>

Job hang faults are detected by the Ascend Device Plugin component. This feature involves the following two configuration files:

- `faultCode.json`: Configures the fault level for job hang faults.
- `hangDetectionConfig.json`: Configures the job hang detection feature switch and detection metric thresholds.

<a name="zh-cn_topic_0000002479387566_table_hangdetectionconfig"></a>

**Table 6** `hangDetectionConfig.json` description

|Parameter|Description|Default Value|
|--|--|--|
|Enabled|Whether to enable hang fault detection. Set to false to disable; set to true to enable.|true|
|AICoreUtilization|AICore utilization threshold, in %.|5|
|HbmMemoryDelta|On-chip memory usage increment threshold, in %.|0|
|TrafficDelta|Network communication traffic increment threshold, in packets/minute.|100|
|CPUTimeDelta|Process CPU time increment threshold, in seconds.|5|
|DetectDuration|Threshold for the number of consecutive hang state detections. The fault is reported after this number is reached. If this threshold is too small, false detections may occur; please modify with caution.|5|

>[!NOTE]
>
>- If the current round's AICore utilization, on-chip memory usage increment, network communication traffic increment, and process CPU time increment are all below the thresholds, the hang state count is incremented by 1.
>- If any threshold parameter in the `hangDetectionConfig.json` configuration file is less than 0, the default value will be used instead.

### (Optional) Configuring Job Hang Detection Parameters<a name="ZH-CN_TOPIC_0000002479387566_custom"></a>

When building the Ascend Device Plugin image, the job hang detection configuration file `hangDetectionConfig.json` is embedded in the image. When Ascend Device Plugin starts, it reads the configuration from this file as the basis for handling job hang faults.

If you want to customize job hang detection parameters, modify the corresponding `hangDetectionConfig.json` file when building the Ascend Device Plugin image.

**Procedure<a name="zh-cn_topic_0000002479387566_section_custom_hangDetectionConfig"></a>**

1. Log in to the environment and navigate to the extracted Ascend Device Plugin directory.
2. Run the `vi hangDetectionConfig.json` command to edit the configuration file.

    ```shell
    vi hangDetectionConfig.json
    ```

    Modify the corresponding configuration parameters. Refer to [Table 6](#zh-cn_topic_0000002479387566_table_hangdetectionconfig) for parameter descriptions. After modifications are complete, press `Esc`, type `:wq!` to save and exit.
...
**(Optional) Modifying the Job Hang Fault Handling Level<a name="zh-cn_topic_0000002479387566_section_custom_hangfaultlevel"></a>**

See [Table 5](#zh-cn_topic_0000002181110120_section4960201383813) for information about job hang fault code 200001002. If you need to isolate the NPU after a job hang or perform other operations, see [Using faultCode.json to Configure Fault Levels](#zh-cn_topic_0000001951258609_section112139052513) to modify the fault level of this fault code. An example of the modified `mindx-dl-fault-config` is as follows:
...
## Optical Link Member Port Faults<a name="ZH-CN_TOPIC_0000002479387566"></a>

### Configuration File Description<a name="ZH-CN_TOPIC_0000002479387566_section_specifications "></a>

Parameter-plane optical link member port faults are detected by the Ascend Device Plugin component. This feature involves the following configuration file:

`faultCode.json`: Configures the fault level for parameter-plane optical link member port faults.
...
If you need to allow the NPU to pass through after a parameter-plane optical link member port fault or perform other operations, refer to [Using faultCode.json to Configure Fault Levels](#zh-cn_topic_0000001951258609_section112139052513) to modify the fault level of this fault code. An example of the modified `mindx-dl-fault-config` is as follows.
When customizing, the product form factors must be distinguished. When configuring faults on the UBoE egress port for Atlas 850E SuperPoD and Atlas 650E server, configure `110001024` and `110000002`. For other product form factors, configure `020001002` and `020000002`.
