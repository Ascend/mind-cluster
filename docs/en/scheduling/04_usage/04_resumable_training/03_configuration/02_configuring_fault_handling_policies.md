...
metadata:
   labels:
     ...
     fault-scheduling: "grace"        # You can choose either force or grace based on the actual situation. When configured as force, the Pod cannot use the host network. When configured as force, there may be cases where rescheduling fails and triggers multiple rescheduling attempts until success.
```
...
metadata:
   labels:
     ...
     <strong>fault-scheduling: "force"   # You can choose either force or grace based on the actual situation. When configured as force, the Pod cannot use the host network. When configured as force, there may be cases where rescheduling fails and triggers multiple rescheduling attempts until success.</strong>
...
        spec:
...
           containers:
...
1. In the startup script (for example, `train_start.sh`), add the following bold fields as shown in the example below.
    ...
    <pre codetype="shell">
    ...
    <strong>export MS_ENABLE_TFT="{RSC:1}"    # Configure this field to enable Pod-level rescheduling in MindSpore scenarios (optional in Ascend Job scenario)</strong>
    ...
...
2. After the distributed environment initialization is complete and the global rank is obtained, modify the training script to start TaskD Manager in the training script.
    ...
    2. Add the following code to the training script (for example, `train_start.sh`) to start TaskD Manager. In the following code:
        ...
        - `TASKD_PROCESS_ENABLE` configuration instructions: If `recover-strategy` in the job YAML does not configure a recovery policy and does not enable hot switching, you need to configure `export TASKD_PROCESS_ENABLE="off"`; if `recover-strategy` is configured or hot switching is enabled, you do not need to configure `export TASKD_PROCESS_ENABLE="off"`.
...
In the job YAML, add the following fields. `recover-strategy` is the strategy used for training process recovery, where `recover` indicates enabling process-level recovery.

Currently, process-level rescheduling supports `recover` and `recover-in-place` modes. Choose one based on the actual usage scenario.

<pre codetype="yaml">
metadata:
  labels:
    ...
    <strong>fault-scheduling: "grace"                     # Choose either force or grace based on the actual situation. When configured as force, the Pod cannot use the host network. When configured as force, there may be cases where rescheduling fails and triggers multiple rescheduling attempts until success.</strong>
  annotations:
    ...
    <strong>recover-strategy: "recover,recover-in-place"   # Recovery strategies (retry: process-level online recovery; recover: process-level rescheduling; recover-in-place: process-level in-place recovery; elastic-training: elastic training; dump: save dying gasps; exit: exit training). Six strategies can be combined arbitrarily, separated by commas.</strong>
...
  ...
    ...
      ...
        spec:
          ...
          containers:
            ...
            - env:
              <strong>- name: POD_IP                        # Configures the communication address for MindIO.</strong>
                <strong>valueFrom:</strong>
                  <strong>fieldRef:</strong>
                    <strong>fieldPath: status.podIP         # Used for MindIO communication. If this parameter is not configured, it will affect the normal startup of training jobs.</strong>
              ports:
                <strong>- containerPort: 9601                # Adds port 9601 used for TaskD communication under all Pods.</strong>
                  <strong>name: taskd-port</strong>
...
</pre>
...
>[!NOTE]
>To test the process-level online recovery feature, configure it by referring to [Process-level Online Recovery Verification](../05_verifying_fault_handling_policies.md#verifying-process-level-online-recovery).
...
- Complete the installation of the following components on the corresponding nodes: Ascend Docker Runtime, Ascend Operator, ClusterD, Ascend Device Plugin, and Volcano (The MindCluster component versions must match the TaskD version). For details, see [Installation and Deployment](../../../03_installation_guide/02_installation/00_helm_installation.md).
- Install the following components in the container: TorchNPU (7.1.RC1 or later), CANN (8.2.RC1 or later), TaskD, and MindIO (7.1.RC1 or later). For details, see [Creating a MindSpeed-LLM Training Image (PyTorch)](../04_using_resumable_training_on_the_cli.md#building-a-mindspeed-llm-training-image-pytorch).
...
2. Modify the training framework code.
    1. Go to the "[mindcluster-deploy](https://gitcode.com/Ascend/mindxdl-deploy)" repository, switch to the corresponding version branch according to [mindcluster-deploy Open Source Repository Version Description](../../../07_references/05_appendix.md#mindcluster-deploy-open-source-repository-version-description), obtain the `train_start.sh` file from the `samples/train/resumable-training/fault-tolerance/without-ranktable/pytorch/Qwen3` directory, and construct the following directory structure on the management node.
...
3. Modify the job YAML.
    ...
    Add the following bold fields to the job YAML to enable process-level online recovery, and modify other necessary configurations.
...
- Complete the installation of the following components on the corresponding nodes: Ascend Docker Runtime, Ascend Operator, ClusterD, Ascend Device Plugin, and Volcano (The MindCluster component versions must match the TaskD version). For details, see [Installation and Deployment](../../../03_installation_guide/02_installation/00_helm_installation.md).
- Install the following components in the container: MindSpore (2.7.0 or later), CANN (8.2.RC1 or later), TaskD, and MindIO (7.1.RC1 or later). For details, see [Creating a MindFormers Training Image (MindSpore)](../04_using_resumable_training_on_the_cli.md#building-the-mindformers-training-image-mindspore).
...
2. Modify the training framework code to enable link failover and switchback.
...
3. Modify the job YAML.
    ...
    Add the following bold fields in the job YAML to enable process-level online recovery, and modify other necessary configurations.
...
...
      ...
      - image: ascend-k8sdeviceplugin:v{version}
        ...
        command: [ "/bin/bash", "-c", "--"]
        args: [ "device-plugin
                 -volcanoType=true                    # Volcano must be used for rescheduling
                 ...
                 -hotReset=1      # When using resumable training, the hot reset feature can be enabled on top of Job-level or Pod-level rescheduling to use graceful fault tolerance mode (this feature has been deprecated)
        ...
        env:
              ...
              - name: POD_IP                        # Configures the communication address for MindIO.
                valueFrom:
                  fieldRef:
                    fieldPath: status.podIP         # Used for MindIO communication. If this parameter is not configured, it will affect the normal startup of training jobs.
...
```
...
2. Modify the job YAML.
    ...
    Add the following bold fields in the job YAML to enable process-level rescheduling and modify other necessary configurations.
      ...
      <pre codetype="yaml">
        ...
        ...
           annotations:
             ...
             <strong>recover-strategy: "recover"    # Recovery strategy. The value recover indicates enabling process-level rescheduling. If stress testing has a fault occurred, use this field to enable process-level rescheduling.</strong>
         ...
        ...
        spec:
          replicaSpecs:
                      ...
                      <strong>- name: POD_IP                        # Configures the communication address for MindIO.</strong>
                        <strong>valueFrom:</strong>
                          <strong>fieldRef:</strong>
                            <strong>fieldPath: status.podIP         # Used for MindIO communication. If this parameter is not configured, it will affect the normal startup of training jobs.</strong>
                    <strong>ports:</strong>
                      <strong>- containerPort: 9601                 # Adds port 9601 used for TaskD communication under all Pods.1</strong>
                        <strong>name: taskd-port</strong>
        ...</pre>
...
3. Modify the job YAML.
    ...
    Add the following bold fields to the job YAML to enable process-level rescheduling, and modify other necessary configurations.
      ...
      <pre codetype="yaml">
        ...
        ...
           annotations:
             ...
             <strong>recover-strategy: "recover"    # Recovery policy. The value is recover, indicating that process-level rescheduling is enabled. If the stress testing has a fault occurred, set this field to enable process-level rescheduling.</strong>
         ...
        ...
        spec:
          replicaSpecs:
...
In the job YAML, add the following fields to enable hot switching and modify other necessary parameters.
...
2. Modify the job YAML.
    ...
    Add the following bold fields to the job YAML to enable elastic training, and modify other necessary configurations.
            ...
            Master:
              template:
                spec:
                  ...
                  - name: ascend # do not modify
                    env:
                      <strong>- name: MINDIO_WAIT_MINDX_TIME         # When process-level rescheduling is not enabled, it is recommended to configure a value of 60 or higher in elastic training scenarios.</strong>
...
3. Modify the training framework code.
    ...
    Go to the "[mindcluster-deploy](https://gitcode.com/Ascend/mindxdl-deploy)" repository, switch to the corresponding version branch according to the [mindcluster-deploy Open Source Repository Version Description](../../../07_references/05_appendix.md#mindcluster-deploy-open-source-repository-version-description), obtain the `train_start.sh` file from the `samples/train/resumable-training/fault-tolerance/without-ranktable/pytorch/Qwen3` directory, and construct the following directory structure on the management node.
...
<tr id="row448045664010"><td class="cellrowborder" valign="top" width="15.24%"><p id="p1848011565403"><a name="p1848011565403"></a><a name="p1848011565403"></a>--enable-high-availability (MindSpeed-LLM parameter)</p>
</td>
...
<tr id="row112463954119"><td class="cellrowborder" valign="top" width="15.24%"><p id="p76389163416"><a name="p76389163416"></a><a name="p76389163416"></a>--enable-hbmfault-repair (MindSpeed-LLM parameter)</p>
</td>
...
<tr id="row3150821154117"><td class="cellrowborder" valign="top" width="15.24%"><p id="p7151102124114"><a name="p7151102124114"></a><a name="p7151102124114"></a>--enable-worker-reboot (MindSpeed-LLM parameter)</p>
</td>
