...
Currently, more than 200 faults can be detected. For details about the fault types, see [Table 1](#zh-cn_topic_0000002039699773_table9980135316395).For detailed fault descriptions, see [Typical Faults](../../../resource/Typical_Faults.xlsx).
...
|--|--|
|Node faults|<p>Include node health status, node hardware faults, and DPC shared storage faults.</p><p>For fault code descriptions, see [Node Fault Code References](../../07_references/05_appendix.md#node-fault-code-references).</p><p>If a node hardware fault causes the node to crash or restart, NodeD cannot detect the specific fault type and report it.</p>|
|Chip faults|<p>Chip faults are reported via the DCMI and chip network faults are detected by the device network probing tool `hccn_tool`.</p><p>For fault code descriptions, see the [Chip Fault Code References](../../07_references/05_appendix.md#chip-fault-code-references).</p>|
|Parameter plane network faults|Include chip network-related faults and UnifiedBus device faults.<ul><li>Chip network-related faults: Faults occur on the dedicated network used for parameter exchange between chips, such as NPU network port faults.</li><li>UnifiedBus device faults: Faults occur on the UnifiedBus device of <term>Atlas A3 training products</term>.</li></ul>|
|Service plane faults|<p>The training job exits abnormally, causing the Pod status to change to `Failed`.</p><p>You can run the <strong>kubectl describe pod <em>{pod name} </em>-n <em>\{NAMESPACE\}</em> \|grep Status:</strong> command to check whether the current Pod status is `Failed`. A command output is as follows:<pre class="screen"><strong>Status: Failed</strong></pre></p>|
...
|Job hang faults |A job hang fault refers to an abnormal state in which a process is running on an NPU but fails to make substantial progress over an extended period due to reasons such as compute hang, memory hang, or communication hang. |
|Optical link member port faults|An optical link member port fault on the parameter plane refers to a fault on the parameter-plane optical link member port of an NPU. It applies to union and 5808 faults, as well as UBOE egress port faults, etc. Since Ascend Device Plugin cannot detect remote-end faults and can only make a comprehensive judgment based on fault code devices and port status, this type includes multiple levels of fault codes.|

**ConfigMap Description<a name="zh-cn_topic_0000002039699773_section49901206282"></a>**

- Ascend Device Plugin on each computing node creates a ConfigMap file that records the NPU and UnifiedBus device information of the node. This ConfigMap file is named `mindx-dl-deviceinfo-<nodename>` (hereinafter referred to as `device-info-cm`), and fault information is reported through this ConfigMap. For descriptions of the fields in this ConfigMap file, see the [DeviceInfoCfg](../../06_api/02_ascend_device_plugin.md#chip-resources) table.
- When a node fault exists on a node, NodeD on each computing node creates a ConfigMap file that records the device information of the node. This ConfigMap file is named `mindx-dl-nodeinfo-<nodename>` (hereinafter referred to as `node-info-cm`), and node fault information is reported through this ConfigMap. For descriptions of the fields in this ConfigMap file, see the [mindx-dl-nodeinfo-<nodename\>](../../06_api/02_ascend_device_plugin.md) table.
- ClusterD creates a ConfigMap file that records the cluster device information. The ConfigMap file is named `cluster-info-<device/switch>-<[0-5]>` or `cluster-info-node-cm` (hereinafter referred to as `cluster-info-cm`). Node and chip fault information is reported via [cluster-info-cm](../../06_api/04_clusterd/00_cluster_resources.md).
- When creating each job, you need to configure a ConfigMap file in the YAML. The ConfigMap file is named `reset-config-<job-name>` (hereinafter referred to as `reset-info-cm`). This ConfigMap is mounted to the container's `/user/restore/reset/config` path. Ascend Device Plugin automatically mounts the ConfigMap to the `/user/restore/reset/<job-namespace>.<job-name>` path on the local node.
    ...
    You can also replace the ConfigMap with `/user/restore/reset/<job-namespace>.<job-name>` on the node and mount it to the container's `/user/restore/reset/config` path. For field descriptions of this ConfigMap file, see the [reset-config-&lt;job-name&gt;](../../06_api/02_ascend_device_plugin.md#job-information) table.
...
- When a node fault occurs, NodeD updates the `node-info-cm` content of the current node within a minimum of 5 seconds (default). For field descriptions, see the [mindx-dl-nodeinfo-&lt;nodename&gt;](../../06_api/03_noded.md#node-resources) table.
...
To ensure the proper functioning of the node fault detection feature, the following components must be installed: Volcano, Ascend Operator, NodeD, and ClusterD.
...
**Figure 2** Chip fault reporting<a name="fig3951191610267"></a>
...
**Figure 3**  Reporting faults to K8s<a name="fig10951101692610"></a>

![](../../../figures/scheduling/reporting-faults-to-K8s.png)

For different fault handling modes, the reporting paths differ.

- Rescheduling mode: After obtaining a chip fault, Ascend Device Plugin writes the chip fault information into `device-info-cm` of the node to which it belongs. For field descriptions, see the [DeviceInfoCfg](../../06_api/02_ascend_device_plugin.md#chip-resources) table. ClusterD reads `device-info-cm` of each node to detect chip faults and reports them to the scheduler.
  - Graceful fault tolerance mode (sunset): After obtaining a recoverable chip fault, Ascend Device Plugin writes the chip fault information into `reset-info-cm` of the job to which it belongs. The service container detects the chip fault by mounting `reset-info-cm` as a file and reading the file.
...
Resumable training provides the default fault frequency, duration, fault level, and fault handling policy for processor faults. If you want to modify the fault handling policy, see [Chip Faults](./03_configuration/01_configuring_fault_detection_levels.md#ZH-CN_TOPIC_0000002511346521_0101). Do not modify these settings unless you have specific requirements.

**Supported Fault Handling Types<a name="section099935818571"></a>**

Job-level rescheduling, Pod-level rescheduling, process-level rescheduling, process-level online recovery, and graceful fault tolerance (sunset).
...
Parameter plane network fault detection is handled by Ascend Device Plugin. [Figure 4](#fig68743107307) shows the detailed workflow.

**Figure 4**  Fault detection<a name="fig68743107307"></a>
...
**Reporting Mechanism<a name="section1874141093019"></a>**
...
After obtaining the parameter plane network fault, Ascend Device Plugin writes the fault information into `device-info-cm` and reports it to K8s in the form of a ConfigMap. For descriptions of each field in `device-info-cm`, see the [DeviceInfoCfg](../../06_api/02_ascend_device_plugin.md#chip-resources) table.

The fault reporting mechanism of Ascend Device Plugin is shown in [Figure 5](#fig1587571063011).

**Figure 5**  Fault reporting<a name="fig1587571063011"></a>
...
Resumable training provides the default fault level and fault handling policy for parameter plane faults. If you need to modify the fault handling policy, see [Parameter Plane Network Faults](./03_configuration/01_configuring_fault_detection_levels.md#ZH-CN_TOPIC_0000002479226486). Do not modify it unless you have special requirements.
...
[Figure 6](#fig1761563615337) shows the detection principle of service plane faults.

**Figure 6** Detection principle<a name="fig1761563615337"></a>
...
>[!NOTE]
>
>- To use the unconditional retry function, you need to configure the following three parameters in the job YAML: `fault-retry-times`, `restartPolicy`, and `policies`. For detailed parameter descriptions, see [YAML Configuration](../../06_api/15_yaml_configuration.md) (`policies` is a native `vcjob` field).
...
Abnormal task execution on an NPU (service plane fault) may prevent normal NPUs in the task from communicating with the faulty NPU, causing the collective communication of normal NPUs to enter a timeout waiting state. The task exits only after a collective communication wait timeout exception occurs (defaulted to 30 minutes). Enabling the watchdog function (which requires simultaneously enabling the unconditional retry capability for service plane faults) can isolate the faulty NPU after such an exception occurs and reschedule the task to a healthy NPU, thereby enabling the task to exit quickly within 6 minutes.

>[!NOTE]
>Abnormal task execution on an NPU only supports the watchdog function for the PyTorch framework on <term>Atlas A2 training products</term>.
...
Job-level rescheduling, Pod-level rescheduling, process-level rescheduling, and graceful fault tolerance (sunset)
...
- ConfigMap: The fault discoverer writes fault information into a ConfigMap, and ClusterD obtains the fault information. You can call the ConfigMap interface to inject public faults by referring to [ConfigMap](../../06_api/04_clusterd/03_public_fault_apis.md#configmap).
- gRPC: The fault discoverer sends fault information to ClusterD through gRPC, and ClusterD obtains the fault information. You can call the gRPC interface to inject public faults by referring to [gRPC Interface](../../06_api/04_clusterd/03_public_fault_apis.md#grpc-interface).

**Figure 7** Public fault reporting<a name="fig72618571585"></a>
...
Resumable training provides the default fault level and supported fault sender for public faults. If you want to modify the fault level and fault sender of public faults, see [Public Faults](./03_configuration/01_configuring_fault_detection_levels.md#ZH-CN_TOPIC_0000002479386564). Do not modify them unless you have special requirements.

### Task Hang Faults<a name="ZH-CN_TOPIC_0000002216292814"></a>

A job hang fault refers to an abnormal state in which a process is running on an NPU but fails to make substantial progress over an extended period. In actual scenarios, there are multiple types of hang conditions:

- Compute hang: The process stalls at a specific operator, with no output but low NPU utilization.
- Memory hang: Abnormal memory allocation causes the process to hang.
- Communication hang: HCCL collective communication deadlock.
- Other hang conditions

The job hang detection feature can promptly detect and report job hang faults, preventing tasks from running without progress and improving efficiency and resource utilization.

**Detection Principle<a name="zh-cn_topic_0000002216292814_section_hangdetectprinciple"></a>**

Hang fault detection is handled by Ascend Device Plugin, which performs one round of detection on each NPU every 60 seconds by default. The detection process is as follows:

1. Ascend Device Plugin retrieves the process information running on the NPU. If no process is currently running, the detection round is skipped.
2. Collects key NPU runtime metrics for the current round, including:
   - AICore utilization: Obtained through the driver interface for NPU AICore usage.
   - On-chip memory usage: Obtained through the driver interface for NPU on-chip memory usage.
   - Network communication traffic:
     - For <term>Ascend 950 products</term>,`tx_busi_flit_num` and `rx_busi_flit_num` of UB are collected using hccn_tool.
     - For other scenarios, `roce_tx_all_pkt_num` and `roce_rx_all_pkt_num` of RoCE are collected using hccn_tool.
   - Process CPU time: The `utime` and `stime` of the process are obtained by reading the `/proc/{pid}/stat` file, and the CPU time of all NPU-associated processes is calculated.
3. Compares the metrics of the current round with the metrics saved from the previous round, and calculates the incremental value of each metric.
4. The current round is determined as a hang state when **all of the following conditions are met**:
   - There is a process running on the NPU.
   - Current AICore utilization < AICore utilization threshold (default 5%).
   - Current on-chip memory usage increment ≤ On-chip memory threshold (default 0%).
   - Current network communication traffic increment < Traffic threshold (default 100 packets/minute).
   - Current process CPU time increment < CPU time threshold (default 5 seconds).

>[!NOTE]
>
>- You can customize detection thresholds according to actual scenarios. For detailed instructions, see[(Optional) Configuring Detection Parameters for Job Hang Faults](./03_configuration/01_configuring_fault_detection_levels.md).
>- Hang fault detection is only performed on NPUs with running processes; NPUs without processes are not subject to detection.

**Reporting Mechanism<a name="zh-cn_topic_0000002216292814_section_hangreportmechanism"></a>**

Job hang faults employ a continuous detection confirmation mechanism to avoid false alarms caused by occasional metric fluctuations:

- After each detection round determines a hang state, the hang counter is incremented by 1. When the number of consecutive hang detections reaches the detection persistence threshold (default 5 times, i.e., approximately 5 minutes cumulative), Ascend Device Plugin reports the hang fault to the `device-info-cm` ConfigMap. The job hang fault code is 200001002, and the fault level is defined in the `faultCode.json` configuration file.
- When the hang conditions are no longer met, the hang counter is cleared. If a hang fault was previously reported, that fault is cleared.

**Required Components<a name="zh-cn_topic_0000002216292814_section_hangcomponents"></a>**

To ensure the normal operation of the NPU hang fault detection feature, Ascend Device Plugin must be installed.

**Supported Fault Handling Types<a name="zh-cn_topic_0000002216292814_section_hanghandler"></a>**
...
- (Required) A [namespace has been created](../../05_developer_guide/00_installation_deployment/00_manual_installation/01_preparing_for_installation.md#creating-a-namespace).
- The following components have been installed on the corresponding nodes: [NodeD](../../05_developer_guide/00_installation_deployment/00_manual_installation/09_noded.md) (required), [Ascend Device Plugin](../../05_developer_guide/00_installation_deployment/00_manual_installation/04_ascend_device_plugin.md) (optional), [ClusterD](../../05_developer_guide/00_installation_deployment/00_manual_installation/06_clusterd.md) (optional).
- (Required) The [NodeD startup parameter resultMaxAge has been configured](../../05_developer_guide/00_installation_deployment/00_manual_installation/09_noded.md#parameter-description).
...
2. Enable or disable UnifiedBus network detection.
    - (Recommended) Ascend Device Plugin and ClusterD installed
        ...
        2. Run the following command to create a ConfigMap file named `pingmesh-config`.
            ...
            The following is a command output:
    ...
    - Ascend Device Plugin and ClusterD not installed.
        ...
        Create a ConfigMap named `super-pod-<superPodID>` with the label `app=pingmesh` in the namespace `cluster-system`. The fields in this ConfigMap must be filled in according to the [super-pod-<super-pod-id\>](../../06_api/04_clusterd/00_cluster_resources.md) table. An example is shown below.
...
| `PublicFault` | Key for public fault information | For details, see the [fault field description](../../06_api/04_clusterd/03_public_fault_apis.md#configmap) table. |
...
|<p>Step</p><p>(step latency)</p>|PyTorch, MindSpore|<ul><li>PyTorch<ul><li>Native optimizer scenario: If TorchNPU version is 7.1.RC1, mstx_torch_plugin is required; if TorchNPU version is version later than 7.1.RC1, mstx_torch_plugin is not required, because TorchNPU itself provides step dotting.</li><li>Custom optimizer scenario: Manually add data dotting configurations.</li></ul></li><li>MindSpore<ul><li>MindFormers: Step data dotting are provided by MindFormers.</li><li>MindSpeed: Step data dotting is not provided.</li></ul></li></ul>|
|<p>Communication</p><p>(communication operators)</p>|PyTorch, MindSpore|<ul><li>PyTorch: TorchNPU</li><li>MindSpore</li></ul>|
|<p>SaveCheckpoint</p><p>(time consumed by SaveCheckpoint)</p>|PyTorch, MindSpore|<ul><li>PyTorch: TorchNPU</li><li>MindSpore</li></ul>|
|<p>DataLoader</p><p>(time consumed by DataLoader)</p>|PyTorch, MindSpore|<ul><li>PyTorch: TorchNPU</li><li>MindSpore</li></ul>|
...
- (Optional) [ClusterD](../../05_developer_guide/00_installation_deployment/00_manual_installation/06_clusterd.md), [Ascend Device Plugin](../../05_developer_guide/00_installation_deployment/00_manual_installation/04_ascend_device_plugin.md), and [Volcano](../../05_developer_guide/00_installation_deployment/00_manual_installation/05_volcano.md) have been installed (the versions of the above MindCluster components must be compatible with TaskD).
- Install [TorchNPU](./04_using_resumable_training_on_the_cli.md#building-a-mindspeed-llm-training-image-pytorch) (**optional**; required for PyTorch scenarios; version ≥ 7.1.RC1), MindSpore (**optional**; required for MindSpore scenarios; version ≥ 2.7.0), [CANN](./04_using_resumable_training_on_the_cli.md#building-a-mindspeed-llm-training-image-pytorch) (**mandatory**; version ≥ 8.2.RC1), and [TaskD](./04_using_resumable_training_on_the_cli.md#building-the-mindformers-training-image-mindspore) (**mandatory**) in the container.
...
- **PyTorch**
  ...
  1. Choose one of the following two methods as required.
      - Install mstx_torch_plugin in the container.
          ...
          3. Import the whl package in the AI task execution script.
              ...
              Ensure that it is imported after torch and TorchNPU are imported.
  ...
  4. After the distributed environment is initialized and the global rank can be obtained, modify the training script to start TaskD Manager in the training script, start TaskD Proxy in the management process, and start TaskD Worker inside the training process.
      1. <a name="li399811541"></a>(Optional) Start TaskD Manager and TaskD Proxy. If lightweight profiling is enabled through the gRPC interface to obtain data written to disk, perform the following steps. If lightweight profiling is enabled through ConfigMap to obtain data written to disk, skip this step.
          1. Create the `manager.py` file and place it in the current directory when invoking the training script. The content of the `manager.py` file is as follows.
              ...
              >[!NOTE]
              >For detailed parameter descriptions in the manager.py file, see [def init_taskd_manager(config:dict\) -\> bool:](../../06_api/07_taskd/04_taskd_manager_apis.md#def-init_taskd_managerconfigdict---bool).
  ...
  6. <a name="li52986profiling"></a>Enable lightweight profiling to obtain data written to disk. The following two methods are supported:
      - Modify the gRPC interface provided by ClusterD: If [4.a](#li399811541) is configured, you need to use this method to enable it. For detailed interface information, see [ModifyTrainingDataTraceSwitch](../../06_api/04_clusterd/04_performance_degradation_apis.md#modifytrainingdatatraceswitch).

          >[!NOTE]
          >When you enable or modify lightweight profiling to obtain data written to disk through the gRPC interface provided by ClusterD, the lifecycle of the created data-trace-<\job_name\> ConfigMap is deleted along with the job. If the job does not exist, the interface fails to be called.
...
- For Pods with the environment variable [MINDX_TASK_ID](../../06_api/13_environment_variable_description.md#ascend-operator-environment-variables), the path of rank 0 is `/user/cluster-info/profiling/$MINDX_TASK_ID/0`.
...
|<p>Step</p><p>(step latency)</p>|PyTorch, MindSpore|<ul><li>PyTorch<ul><li>Native optimizer scenario: If TorchNPU version is 7.1.RC1 or earlier, mstx_torch_plugin is required; if TorchNPU version is later than 7.1.RC1, mstx_torch_plugin is not required, as TorchNPU includes built-in the step data dotting function.</li><li>Custom optimizer scenario: Manually add data dotting configurations.</li></ul></li><li>MindSpore<ul><li>MindFormers: Step data dotting is provided by MindFormers.</li><li>MindSpeed: Step data dotting is not provided.</li></ul></li></ul>|
|<p>Communication</p><p>(communication operators)</p>|PyTorch, MindSpore|<ul><li>PyTorch: TorchNPU</li><li>MindSpore</li></ul>|
|<p>SaveCheckpoint</p><p>(time consumed by SaveCheckpoint)</p>|PyTorch, MindSpore|<ul><li>PyTorch: TorchNPU</li><li>MindSpore</li></ul>|
|<p>DataLoader</p><p>(time consumed by DataLoader)</p>|PyTorch, MindSpore|<ul><li>PyTorch: TorchNPU</li><li>MindSpore</li></ul>|
...
- (Optional) [ClusterD](../../05_developer_guide/00_installation_deployment/00_manual_installation/06_clusterd.md), [Ascend Device Plugin](../../05_developer_guide/00_installation_deployment/00_manual_installation/04_ascend_device_plugin.md), and [Volcano](../../05_developer_guide/00_installation_deployment/00_manual_installation/05_volcano.md) have been installed (the versions of the above MindCluster components must be compatible with TaskD).
- Install [TorchNPU](./04_using_resumable_training_on_the_cli.md#building-a-mindspeed-llm-training-image-pytorch) (**optional**; required for PyTorch scenarios; version ≥ 7.0.0), MindSpore (**optional**; required for MindSpore scenarios; version ≥ 2.6.RC1), [CANN](./04_using_resumable_training_on_the_cli.md#building-a-mindspeed-llm-training-image-pytorch) (**mandatory**; version ≥ 8.1.RC1), and [TaskD](./04_using_resumable_training_on_the_cli.md#building-a-mindspeed-llm-training-image-pytorch) (**mandatory**, version ≥ 7.0.RC1) in the container.
...
- **PyTorc**
  ...
  5. <a name="li230238965"></a>After the distributed environment is initialized and the global rank is obtained, modify the training script to start TaskD Worker inside the training process.
      ...
      The following example is for the PyTorch-MindSpeed scenario. You need to modify the `QWEN3_for_PyTorch_2.7_code/mindspeed_llm/training/training.py` file and add the following bold fields in the code.
        ...
        >[!NOTE]
        >The input parameter 5000 in the above code `init_taskd_worker(rank,5000)` is the upper limit size of `/user/cluster-info/profiling`. For detailed description, see the "`upper_limit_of_disk_in_mb`" parameter in [def init\_taskd\_worker\(rank\_id: int, upper\_limit\_of\_disk\_in\_mb: int = 5000, framework: str = "pt"\) -\> bool](../../06_api/07_taskd/01_taskd_worker_apis.md#def-init_taskd_workerrank_id-int-upper_limit_of_disk_in_mb-int--5000-framework-str--pt---bool).
  ...
  7. <a name="li52986890profiling"></a>Enable lightweight profiling to obtain data written to disk. Modify the data-trace ConfigMap corresponding to the task or the gRPC interface provided by ClusterD (see [ModifyTrainingDataTraceSwitch](../../06_api/04_clusterd/04_performance_degradation_apis.md#modifytrainingdatatraceswitch) for interface details) to dynamically enable or disable the lightweight profiling capability.
...
1. In the container, log in to the environment as the running user of the CANN package and run the `source ${install_path}/set_env.sh` command to set environment variables. `${install_path}` is the installation directory of the CANN software. For example:
...
3. <a name="li23023896501"></a>After the distributed environment initialization is complete and the global rank can be obtained, modify the training script to start TaskD Worker inside the training process.
    ...
    >[!NOTE]
    >In the code above, the input parameter `5000` in `init_taskd_worker(rank,5000)` is the upper limit size of `/user/cluster-info/profiling`. For details, see the "`upper_limit_of_disk_in_mb` parameter in [def init\_taskd\_worker\(rank\_id: int, upper\_limit\_of\_disk\_in\_mb: int = 5000, framework: str = "pt"\) -\> bool](../../06_api/07_taskd/01_taskd_worker_apis.md#def-init_taskd_workerrank_id-int-upper_limit_of_disk_in_mb-int--5000-framework-str--pt---bool).
...
- For Pods with the environment variable [MINDX\_TASK\_ID](../../06_api/13_environment_variable_description.md#ascend-operator-environment-variables), the rank 0 data path inside the container is `/user/cluster-info/profiling/$MINDX_TASK_ID/0`.
...
Currently, only online deployment integrated with ClusterD and NodeD is supported. See the [Installation and Deployment](../../03_installation_guide/02_installation/00_helm_installation.md) chapter to complete the deployment of ClusterD and NodeD.
...
- The owner group of `/user/slownode-cluster` has been modified.
...
Currently, it only supports online deployment integrated with ClusterD and NodeD. See the [Installation and Deployment](../../03_installation_guide/02_installation/00_helm_installation.md) section to complete the deployment of ClusterD and NodeD.
...
2. Enable the fault detection switch.
    1. Log in to the environment and go to the NodeD decompression directory.
    2. Run the following command to create a ConfigMap file named `pingmesh-config.yaml` is the pingmesh configuration file, which can be obtained from the NodeD installation package.
...
### Optical Link Member Port Faults<a name="ZH-CN_TOPIC_0000002511426421"></a>

Include port faults from NPUs to unions and 5808 in the Atlas 950 SuperPoD, as well as 1825 and 5808 port faults of NPUs in the Atlas 850E SuperPoD and Atlas 650E server, among others.

**Detection Principle<a name="zh-cn_topic_0000002216292814_section_hangdetectprinciple"></a>**

Optical link member port fault detection on the parameter plane is handled by the Ascend Device Plugin component, which performs one round of detection on each NPU at fixed intervals by default. The detection process is as follows:

1. Ascend Device Plugin retrieves all fault code information for NPUs.
2. Based on the different fault codes, it determines whether the NPU has a parameter-plane optical link member port fault and whether the fault scope compromises route convergence.
3. Write faults at different levels to the `device-info-cm` ConfigMap.

**Required Components<a name="zh-cn_topic_0000002216292814_section_hangcomponents"></a>**

To ensure the normal operation of this detection feature, Ascend Device Plugin must be installed.

**Fault Types to Be Handled<a name="zh-cn_topic_0000002216292814_section_hanghandler"></a>**

UB PORT link state change (UP -> DOWN); UBOE PORT link state change (UP -> DOWN).

**Constraints<a name="zh-cn_topic_0000002193288232_section156679598384"></a>**

This feature is applicable to the Atlas 950 SuperPoD, Atlas 850E SuperPoD, and Atlas 650E server.

|Fault Code|Fault Description|Fault Level|
|--|--|--|
|`020001002`|Hyper-plane route cannot converge.       |`SeparateNPUCodes`|
|`020000002`|Hyper-plane route can converge but device is sub-healthy.|`SubHealthFaultCodes`|
|`110001024`|Parameter-plane route cannot converge.       |`PreSeparateNPUCodes`|
|`110000002`|Parameter-plane route can converge but device is sub-healthy.|`SubHealthFaultCode`|
...
1. Rescheduling mode: Schedules jobs to healthy chips and isolates faulty chips.
    ...
    **Table 1**  Differences between various rescheduling levels
    ...
    |Process-level rescheduling (process-level recovery)|Process-level rescheduling can reduce the training initialization time, shorten the overall recovery time, and is independent of or weakly correlated with the job scale.|<p>Compared with Pod-level rescheduling, process-level rescheduling requires you to additionally integrate high-availability training capabilities into the training framework. Users of MindCluster need to modify the training script and enable the corresponding configuration switch to use it.</p><p>For key configuration procedures, see [Configuring Process-Level Rescheduling](./03_configuration/02_configuring_fault_handling_policies.md#configuring-process-level-rescheduling).</p>|To address the issue of short MTBF in large-scale scenarios and further reduce the overall recovery time, you can choose to enable process-level online recovery on top of process-level rescheduling.|
    |Process-level online recovery|Process-level online recovery has a lower recovery time compared with process-level rescheduling.|<p>Compared with process-level rescheduling, process-level online recovery requires users to configure the corresponding configuration switch before use.</p><p>For key configuration procedures, see [Configuring Process-Level Online Recovery](./03_configuration/02_configuring_fault_handling_policies.md#configuring-process-level-online-recovery).</p>|Currently, process-level online recovery supports on-chip memory faults and network faults. Other fault scenarios will fall back to other handling methods.|
    |Operator-level online recovery|-|For key configuration procedures, see [Configuring Operator-Level Online Recovery](././03_configuration/02_configuring_fault_handling_policies.md#configuring-operator-level-online-recovery).|-|
...
For key configuration procedures of Job-level rescheduling, see [Configuring Job-Level Rescheduling](./03_configuration/02_configuring_fault_handling_policies.md#configuring-job-level-rescheduling).
...
|--|--|--|
|<term>Atlas training products</term>|<ul><li>Atlas 800 training server (model 9000)</li><li>Atlas 800 training server (model 9010)</li></ul><div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody">If the chip work mode of the Atlas 800 training server is SMP and the number of requested NPUs by each Pod is 1 or 2, rescheduling is not supported. For details about how to query and set the NPU chip work mode, see [Querying and Setting NPU Chip Work Mode (npuworkmode)](https://support.huawei.com/enterprise/en/doc/EDOC1100136583/b6e6ed5a) in *Atlas 800 Training Server iBMC User Guide (Model 9000)*.</div></div>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A2 training products</term>|<ul><li>Atlas 800T A2 training server</li><li>Atlas 200T A2 Box16 heterogeneous subrack</li><li>Atlas 900 A2 PoD cluster basic unit</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 900 A3 SuperPoD</li><li>Atlas 800T A3 SuperPoD server</li><li>Atlas 9000 A3 SuperPoD cluster computing system</li><li>A200T A3 Box8 SuperPoD server</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Ascend 950 products</term>|<ul><li>Atlas 950 SuperPoD</li><li>Atlas 850E SuperPoD</li><li>Atlas 650E server</li></ul>|PyTorch|
...
For key configuration procedures of Pod-level rescheduling, see [Configuring Pod-Level Rescheduling](./03_configuration/02_configuring_fault_handling_policies.md#configuring-pod-level-rescheduling).
...
For the key configuration procedure of process-level rescheduling, see [Configuring Process-Level Rescheduling](./03_configuration/02_configuring_fault_handling_policies.md#configuring-process-level-rescheduling).
...
- For <term>Atlas A3 training products</term>, faults such as NPU removal or OS disconnection may cause process-level rescheduling to fail.
- When a fault occurs during the HCCL link setup phase, process-level rescheduling will fail. If, in addition to the HCCL link setup during training initialization, there are other HCCL link Setup phases during training, you can refer to the [Configuring Proactive HCCL Link Setup](./03_configuration/03_configuring_training_recovery.md#configuring-proactive-hccl-link-setup) section to establish links in advance to prevent faults from occurring during the HCCL link setup phase.
- This feature depends on MindIO. Please first read [Constraints](../../07_references/00_fault_recovery_acceleration/02_installation_and_deployment.md#constraints) of MindIO.
...
|--|--|--|
|<term>Atlas A2 training products</term>|<ul><li>Atlas 800T A2 training server</li><li>Atlas 200T A2 Box16 heterogeneous subrack</li><li>Atlas 900 A2 PoD cluster basic unit</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 900 A3 SuperPoD</li><li>Atlas 800T A3 SuperPoD server</li><li>Atlas 9000 A3 SuperPoD cluster computing system</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Ascend 950 products</term>|Atlas 950 SuperPoD|PyTorch|
...
<td class="cellrowborder" rowspan="9" valign="top" width="21.862186218621858%" headers="mcps1.2.5.1.4 "><p id="p252632095917"><a name="p252632095917"></a><a name="p252632095917"></a><a href="../../07_references/00_fault_recovery_acceleration/03_usage_guidance.md#integrating-with-non-mindspeed-llm-frameworks">Integrating Non-MindSpeed-LLM Frameworks</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.5.1.3 "><p id="p11417425315"><a name="p11417425315"></a><a name="p11417425315"></a><a href="https://gitcode.com/Ascend/mind-cluster/tree/branch_v26.1.0/component/ascend-for-volcano/internal/rescheduling" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
</tbody>
</table>
...
This fault handling mode is disabled by default. To enable it, see [(Optional) Configuring Components](./04_using_resumable_training_on_the_cli.md#optional-configuring-components).

For key configuration procedures of process-level online recovery, see [Configuring Process-Level Online Recovery](./03_configuration/02_configuring_fault_handling_policies.md#configuring-process-level-online-recovery).
...
|Product Type|Hardware Form Factor|Training Framework|
|--|--|--|
|<term>Atlas A2 training products</term>|<ul><li>Atlas 800T A2 training server</li><li>Atlas 900 A2 PoD cluster basic unit</li><li>Atlas 900 A2 PoDc cluster basic unit</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 900 A3 SuperPoD</li><li>Atlas 800T A3 SuperPoD server</li><li>Atlas 9000 A3 SuperPoD cluster computing system</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Ascend 950 products</term>|Atlas 950 SuperPoD|PyTorch|
...
<td class="cellrowborder" rowspan="5" valign="top" width="26.68266826682668%" headers="mcps1.2.5.1.4 "><p id="p1878873515913"><a name="p1878873515913"></a><a name="p1878873515913"></a><a href="../../07_references/00_fault_recovery_acceleration/03_usage_guidance.md#integrating-with-non-mindspeed-llm-frameworks">Integrating Non-MindSpeed-LLM Frameworks</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="26.68266826682668%" headers="mcps1.2.5.1.4 "><p id="p19472244965"><a name="p19472244965"></a><a name="p19472244965"></a><a href="https://gitcode.com/Ascend/mind-cluster/tree/branch_v26.1.0/component/clusterd/pkg/application/recover" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.5.1.3 "><p id="p12472134412615"><a name="p12472134412615"></a><a name="p12472134412615"></a><a href="https://gitcode.com/Ascend/mind-cluster/tree/branch_v26.1.0/component/ascend-for-volcano/internal/rescheduling" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
</tbody>
</table>
...
<td class="cellrowborder" rowspan="9" valign="top" width="26.68%" headers="mcps1.2.5.1.4 "><p id="p7146223174212"><a name="p7146223174212"></a><a name="p7146223174212"></a><a href="../../07_references/00_fault_recovery_acceleration/03_usage_guidance.md#integrating-with-non-mindspeed-llm-frameworks">Integrating with Non-MindSpeed-LLM Frameworks</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="26.68%" headers="mcps1.2.5.1.4 "><p id="p14571125414116"><a name="p14571125414116"></a><a name="p14571125414116"></a><a href="https://gitcode.com/Ascend/mind-cluster/tree/branch_v26.1.0/component/clusterd/pkg/application/recover" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.5.1.3 "><p id="p957195451114"><a name="p957195451114"></a><a name="p957195451114"></a><a href="https://gitcode.com/Ascend/mind-cluster/tree/branch_v26.1.0/component/ascend-for-volcano/internal/rescheduling" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
</tbody>
</table>
...
<term>Atlas A3 training products</term> support HCCL performing communication operator retransmission when a parameter plane network fault occurs. With the faulty process not exiting, operator-level online recovery can tolerate longer network anomalies without interrupting the training job.
...
For key configuration procedures of operator-level online recovery, see [Configuring Operator-Level Online Recovery](./03_configuration/02_configuring_fault_handling_policies.md#configuring-operator-level-online-recovery).
...
|--|--|--|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 900 A3 SuperPoD</li><li>Atlas 9000 A3 SuperPoD cluster computing system</li></ul>|-|
...
For <term>Atlas A3 training products</term>, MindCluster cluster scheduling components provide suspension and switchback functions for link failover communication of training jobs, allowing you to freely switch RoCE network ports used by NPUs during training via active link failover and switchback interfaces.

For details about how to configure suspension and switchback of link failover communication, see [Configuring Suspension and Switchback of Link Failover Communication](./03_configuration/02_configuring_fault_handling_policies.md#configuring-suspension-and-switchback-for-link-failover-communication).

- Before calling the [link failover and switchback APIs](../../06_api/04_clusterd/08_link_failover_and_switchback_apis.md) to perform link failover and switchback, understand NPU networking first and ensure that the network link of the target NPU is normal. If the target NPU is in the linkdown state, the operation fails.
- The following uses the interface interconnection in the networking guide as an example to describe the `dev-op` mapping when the `SwitchNicTrack` API is called.
    ...
    **Figure 1**  Port interconnection relationship<a name="fig111354543222"></a>
...
|Product Type|Hardware Form|Training Framework|
|--|--|--|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 900 A3 SuperPoD</li><li>Atlas 800T A3 SuperPoD server</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
...
To understand the key configuration procedure for graceful fault tolerance, see [Configuring Graceful Fault Tolerance](./03_configuration/02_configuring_fault_handling_policies.md#configuring-graceful-fault-tolerance).
...
|Product Type|Hardware Form|Training Framework|
|--|--|--|
|<term>Atlas training products</term>|<ul><li>Atlas 800 training server (model 9000)</li><li>Atlas 800 training server(model 9010)</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A2 training products</term>|<ul><li>Atlas 800T A2 training server</li><li>Atlas 900 A2 PoD cluster basic unit</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 900 A3 SuperPoD</li><li>Atlas 800T A3 SuperPoD server</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
...
<a name="zh-cn_topic_0000002039194017_section140112935318"></a>**

**Table 1** Products and frameworks that support online stress testing

|Product Type|Hardware Form|Training Framework|
|--|--|--|
|<term>Atlas A2 training products</term>|Atlas 800T A2 training server|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A3 training products</term>|Atlas 900 A3 SuperPoD|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
...
<td class="cellrowborder" rowspan="3" valign="top" width="23.75%" headers="mcps1.2.5.1.4 "><p id="p10701822403"><a name="p10701822403"></a><a name="p10701822403"></a><a href="../../07_references/00_fault_recovery_acceleration/03_usage_guidance.md#integrating-with-non-mindspeed-llm-frameworks">Integrating with Non-MindSpeed-LLM Frameworks</a></p>
</td>
</tr>
...
|--|--|--|
|<term>Atlas A2 training products</term>|Atlas 800T A2 training server|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
|<term>Atlas A3 training products</term>|<ul><li>Atlas 800T A3 SuperPoD server</li><li>Atlas 9000 A3 SuperPoD cluster computing system</li></ul>|<ul><li>MindSpore</li><li>PyTorch</li></ul>|
...
<td class="cellrowborder" valign="top" headers="mcps1.2.5.1.3 "><p id="p071621117393"><a name="p071621117393"></a><a name="p071621117393"></a><a href="https://gitcode.com/Ascend/mind-cluster/blob/branch_v26.1.0/component/ascend-operator/pkg/controllers/v1/ascendjob_controller.go" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
</tbody>
</table>
...
|Product Type|Hardware Form|Training Framework|
|--|--|--|
|<term>Atlas A2 training products</term>|Atlas 800T A2 training server|PyTorch|
|<term>Atlas A3 training products</term>|Atlas 900 A3 SuperPoD|PyTorch|
...
<td class="cellrowborder" rowspan="6" valign="top" width="21.090000000000003%" headers="mcps1.2.6.1.5 "><p id="p7146223174212"><a name="p7146223174212"></a><a name="p7146223174212"></a><a href="../../07_references/00_fault_recovery_acceleration/03_usage_guidance.md#integrating-with-non-mindspeed-llm-frameworks">Table 2</a></p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.4 "><p id="p10446112592310"><a name="p10446112592310"></a><a name="p10446112592310"></a><a href="https://gitcode.com/Ascend/mind-cluster/tree/branch_v26.1.0/component/ascend-for-volcano/internal/rescheduling" target="_blank" rel="noopener noreferrer">Link</a></p>
</td>
</tr>
</tbody>
</table>
...
For details about how to save dying gasp checkpoints, see [Fault Recovery Acceleration](../../07_references/00_fault_recovery_acceleration/01_product_description.md).

For details about how to configure dying gasp checkpoint saving, see [Dying Gasp Checkpoint Saving](./03_configuration/03_configuring_training_recovery.md#dying-gasp-checkpoint-saving).
...
To learn about the checkpoint configuration procedure on the parameter plane, see [Restoring Parameter Passing on the Parameter Plane](./03_configuration/03_configuring_training_recovery.md#restoring-parameter-passing-on-the-parameter-plane).
