...
4. ClusterD sends the `global-ranktable` to MindIE Controller. For details about the `global-ranktable`, see [SubscribeRankTable](../../06_api/04_clusterd/05_service_configuration_apis.md#subscriberanktable).
...
> [!NOTE]
> In the following examples, comments starting with `#` are only used to explain the meaning of the fields. Before applying the ConfigMap, please delete all comment lines starting with `#`; otherwise, the application will fail to parse `elastic_scaling.json` strings due to JSON parsing errors.
...
## Viewing Elastic Scaling Results<a name="zh-cn_topic_0000002356673977_zh-cn_topic_0000002098814658_section463203519255"></a>

Assume the scaling rules used by the service are as follows:
...
data:
  elastic_scaling.json: |          # Fixed field, do not modify
    {
      "version": "1.0",            # Fixed field, do not modify
      "elastic_scaling_list": [    # The following is a template; users set it according to their own requirements
        {
          "group_list": [                 # A normal running job ratio state
            {
              "group_name": "group0",      # Set by user
              "group_num": "2",            # Set by user; must not increase from top to bottom
              "server_num_per_group": "2"  # Set by user; must remain the same for the same group_name
            },
            {
              "group_name": "group1",
              "group_num": "1",
        ...
        {
          "group_list": [                 # Another normal running job ratio state
            {
              "group_name": "group0",
              "group_num": "1",
              "server_num_per_group": "2"
...
kind: ConfigMap
metadata:
  name: scaling-rule              # Set by user
  namespace: mindie-service       #  Set by user; must be consistent with the inference job
```

Run the following command to check Pod scheduling status.
...
...
mindie-motor     p0-master-0                         1/1     Running   0          20M
mindie-motor     p0-worker-0                         1/1     Running   0          20M
mindie-motor     p1-master-0                         1/1     Running   0          20M
mindie-motor     p1-worker-0                         1/1     Running   0          20M
mindie-motor     d0-master-0                         1/1     Running   0          20M
mindie-motor     d0-worker-0                         1/1     Running   0          20M
...
```

When a hardware or software fault occurs on the node where the d0 instance is running during inference task execution, and the current resources are insufficient to bring up all instances, the number of running instances is reduced.
...
mindie-motor     p0-master-0                         1/1     Running   0          20M
mindie-motor     p0-worker-0                         1/1     Running   0          20M
mindie-motor     p1-master-0                         1/1     Pending   0          1S
mindie-motor     p1-worker-0                         1/1     Pending   0          1S
mindie-motor     d0-master-0                         1/1     Running   0          1S
mindie-motor     d0-worker-0                         1/1     Running   0          1S
...
```

At this point, based on the configured scaling rules, the job runs according to another normal running job ratio state. The d0 Pod will exit and be re-created, while p0 or p1 will enter a `Pending` state waiting for resources.
