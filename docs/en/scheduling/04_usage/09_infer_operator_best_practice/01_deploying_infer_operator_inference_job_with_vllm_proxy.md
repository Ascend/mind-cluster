...
Currently, the vLLM Proxy-based Infer Operator inference job is deployed using the `InferServiceSet`-customized CRD. For Infer Operator deployment, see [Installation and Deployment](../../03_installation_guide/02_installation/00_helm_installation.md).
...
spec:
  ...
  template:
    ...
    - name: prefill # Prefill definition
      ...
      workload:     # CRD type information for instances in prefill
        apiVersion: apps/v1
        kind: StatefulSet # Workload type; currently supports StatefulSet/Deployment
      metadata:
        labels:
          infer.huawei.com/gang-schedule: 'false' # Disable gang scheduling; when enabled, a PodGroup will be created for each workload instance
      spec:
        replicas: 1 # Number of pod replicas for the workload in prefill
        podManagementPolicy: Parallel # This configuration is optional. When workload is StatefulSet and infer.huawei.com/gang-schedule is true, it must be set to Parallel
        selector:
          matchLabels:
        ...
        template:
          metadata:
            ...
            annotations:
              huawei.com/schedule_policy: chip8-node8 # Set according to hardware form factor
          spec:
            ...
            - name: prefill
              ...
              resources:
                ...
                limits:
                  huawei.com/Ascend910: 8
              ... # Add necessary container mount items and runtime commands
    - name: decode  # decode definition
      replicas: 1   # Number of decode replicas
      workload:     # CRD type information for instances in decode
        apiVersion: apps/v1
        kind: StatefulSet # Workload type; currently supports StatefulSet/Deployment
      metadata:
        labels:
    ...
    - name: router  # router definition
      ...
      services:     # router services definition; the service defined here creates only one within a role scope
      - name: vllm-router-service
        spec:
          ...
          type: ClusterIP
      workload:     # CRD type information for instances in router
        apiVersion: apps/v1
        kind: Deployment # Workload type; currently supports StatefulSet/Deployment
      spec:
        replicas: 1 # Number of pod replicas for the workload in router
        selector:
          matchLabels:
            app: test-router # User-defined; must be consistent with the app configuration in the labels below
        template:
...
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 "><p >Set to<span class="parmvalue" >"volcano"</span>.</p>
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >Configures the scheduler <span >as Volcano</span>.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" width="27.16%" headers="mcps1.2.4.1.1 "><p >pod-rescheduling</p>
</td>
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 "><ul ><li>on:<span > Enable</span> Pod-level rescheduling.</li><li>Other values or not using this field: Disable <span >Pod</span>-level rescheduling.</li></ul>
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p ><span >Pod</span>-level rescheduling means that when a job fails, not all job Pods in the PodGroup are deleted.<span > Instead,</span> only the faulty <span >Pod</span> is deleted, and the controller recreates a new <span >Pod</span> for rescheduling.</p>
</td>
</tr>
...
<ul ><li><span >Parallel</span>: Concurrent scheduling, the default scheduling method, creates all instances concurrently.</li><li><span >Priority</span>: Priority scheduling, creates instances of different roles sequentially in order of priority.</li></ul>
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >When adaptation to other serving platforms is required to scale down prefill instances and reserve decode instances, priority scheduling must be enabled, and the decode instance priority must be configured higher than the prefill instance priority to ensure that higher-priority instances are scheduled before lower-priority ones.</p>
<p>See the description of the priority field for priority configuration.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" width="27.16%" headers="mcps1.2.4.1.1 "><p >priority</p>
</td>
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 ">
Numeric value, ranging from 1 to 32.
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >Only takes effect when priority scheduling is enabled. A smaller value indicates a higher priority. In PD-disaggregated scenarios, it is recommended to configure: priority value for prefill role > priority value for decode role.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" width="27.16%" headers="mcps1.2.4.1.1 "><p >infer.huawei.com/gang-schedule</p>
</td>
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 "><ul ><li>true: Enable gang scheduling.</li><li>Other values or not using this field: Disable gang scheduling. Disabled by default.</li></ul>
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >When gang scheduling is enabled, Infer Operator creates a corresponding PodGroup for each instance (workload), ensuring that all Pods within the same PodGroup can start simultaneously.</p>
<p>In StatefulSet workload scenarios, when gang scheduling is enabled, podManagementPolicy must also be configured as Parallel; otherwise, the StatefulSet cannot be scheduled properly.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" width="27.16%" headers="mcps1.2.4.1.1 "><p >huawei.com/schedule_policy</p>
</td>
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 ">Refer to <a href="../../06_api/01_volcano.md#podgroup">Table 3</a> in Parameter Description for details on this field.
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >Select different values based on the node type where the job needs to run.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" width="27.16%" headers="mcps1.2.4.1.1 "><p >huawei.com/Ascend910</p>
</td>
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 "><ul ><li><span >Atlas 800I A2 inference server</span>:1-8</li><li><span >Atlas 800I A3 SuperPoD server</span>:1-16</li><li><span >Atlas 350 accelerator card (8 NPUs within a node)</span>: 1-8</li><li><span >Atlas 350 accelerator card (16 NPUs within a node)</span>: 1-16</li><li><span >Atlas 850E SuperPoD, Atlas 650E server</span>:1-8</li><li><span >Atlas 950 SuperPoD</span>:1-8</li></ul>
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >The number of NPUs requested. Currently, only full-server scheduling is supported. Please modify according to the actual number of hardware cards and the required number of cards.</p>
<div class="note"><span class="notetitle">[!NOTE]</span><div class="notebody">For Atlas 350 accelerator card, Atlas 850E SuperPoD, Atlas 650E server, and Atlas 950 SuperPoD, change the parameter name to huawei.com/npu.</div></div>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >When graceful deletion is configured, the original Pod is gracefully deleted <span >first.</span> If deletion is not successful after 15 minutes, the original Pod is forcibly<span > deleted</span></p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" headers="mcps1.2.4.1.1 "><p >force</p>
</td>
<td class="cellrowborder" valign="top" headers="mcps1.2.4.1.2 "><p >If forced deletion is configured, the original Pod is forcibly deleted<span > directly</span>.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" headers="mcps1.2.4.1.1 "><p >external-grace</p>
</td>
<td class="cellrowborder" valign="top" headers="mcps1.2.4.1.2 "><p >Enable instance-level rescheduling. Infer Operator first deletes the workload, and K8s deletes the Pod in a cascading manner (following the Pod's own <code>terminationGracePeriodSeconds</code>). Infer Operator starts a timer based on the Pod's <code>terminationGracePeriodSeconds</code>, and forcibly deletes remaining Pods after timeout (GracePeriodSeconds=0). If the Pod does not have <code>terminationGracePeriodSeconds</code> configured, the default wait time is 30 seconds.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" headers="mcps1.2.4.1.1 "><p >external-force</p>
</td>
<td class="cellrowborder" valign="top" headers="mcps1.2.4.1.2 "><p >Enable instance-level rescheduling. Infer Operator immediately forcibly deletes the workload and Pod (GracePeriodSeconds=0). <code>terminationGracePeriodSeconds</code> does not take effect.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" headers="mcps1.2.4.1.1 "><p >off</p>
</td>
<td class="cellrowborder" rowspan="3" valign="top" headers="mcps1.2.4.1.2 "><p >The inference job does not use the fault rescheduling feature.</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >To handle service-layer faults, the number of unconditional retries for service-layer faults must be configured.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" headers="mcps1.2.4.1.1 "><p >None (no fault-retry-times) or 0</p>
</td>
<td class="cellrowborder" valign="top" headers="mcps1.2.4.1.2 "><p >The job does not use the unconditional retry feature. After a service-layer fault occurs, Volcano will not actively delete the faulty Pod.</p>
</td>
</tr>
<tr ><td class="cellrowborder" valign="top" width="27.16%" headers="mcps1.2.4.1.1 "><p >scalingPolicy</p>
</td>
<td class="cellrowborder" valign="top" width="36.28%" headers="mcps1.2.4.1.2 "><p >See<a href="05_configuring_elastic_scaling.md"> Configuring Elastic Scaling Policies</a>.</p>
</td>
<td class="cellrowborder" valign="top" width="36.559999999999995%" headers="mcps1.2.4.1.3 "><p >Configures a load-based elastic scaling policy for the corresponding instance.</p>
</td>
</tr>
</tbody></table>
