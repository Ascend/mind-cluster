...
- Training jobs: [Full-NPU Scheduling](../../01_introduction/02_feature_description.md#full-npu-scheduling), [Multi-Level Scheduling](../../01_introduction/02_feature_description.md#multi-level-scheduling), and [Resumable Training](../../04_usage/04_resumable_training/00_feature_description.md).
- Inference jobs: [Full-NPU Scheduling](../../01_introduction/02_feature_description.md#full-npu-scheduling), [Recovery of Inference Card Faults](../../01_introduction/02_feature_description.md#recovery-of-inference-card-faults), and [Rescheduling upon Inference Card Faults](../../01_introduction/02_feature_description.md#rescheduling-upon-inference-card-faults).

    Different features depend on different components. For details, see the [Basic Scheduling](../../01_introduction/02_feature_description.md#basic-scheduling) chapter.
...
- Configure resource information through files: Training job collective communication configuration file (RankTable File, also known as [hccl.json](../../06_api/14_hccl.json_file_description.md)). Users who use this method can create the following three types of objects: Volcano Job (hereinafter referred to as vcjob), Ascend Job (hereinafter referred to as acjob), and Deployment (hereinafter referred to as deploy).
    - (Recommended) Ascend Job: It is a custom job type defined by MindCluster. Currently, it supports launching training or inference jobs through two methods: configuring resource information via environment variables and configuring resource information via files.
        ...
        Each acjob YAML contains some fixed fields, such as `apiVersion`, `kind`, etc. For detailed descriptions of these fields, please refer to [acjob Yaml Description](../../06_api/15_yaml_configuration.md).
...
- Single-job scheduling time
    ...
    **Table 1** Single-job multi-pod scheduling description
    ...
    >- To achieve the optimized scheduling reference time for 4,000 or 5,000 nodes, you need to follow the scheduling time performance tuning steps in [Installing Volcano](../../05_developer_guide/00_installation_deployment/00_manual_installation/05_volcano.md#installing-volcano) to make the corresponding modifications.
