...
>[!NOTE]
>
>The multi-level scheduling feature is configured based on full-NPU scheduling.

### Creating an Image <a name="ZH-CN_TOPIC_0000002479227164duoji"></a>

For details, see [Creating an Image](./03_full_npu_scheduling.md#image-creation).

### Script Adaptation <a name="ZH-CN_TOPIC_0000002511347097duoji"></a>

For details, see [Script Adaptation](./03_full_npu_scheduling.md#script-adaptation).
...
Multi-level scheduling tasks are configured additionally on top of full-NPU scheduling. The following YAML examples illustrate this.
...
This section only provides the YAML parameters required for multi-level scheduling. For detailed YAML parameter descriptions, see [acjob YAML Description](../../06_api/15_yaml_configuration.md).

**Table 2** YAML parameter description
...
metadata:
  ...
  labels:
    ...
    podgroup-sched-enable: "true"  # Only configured when the cluster uses the openFuyao customized Kubernetes and volcano-ext components. When set to the string "true", batch scheduling is enabled; when set to any other string, batch scheduling does not take effect and normal scheduling is used; if this parameter is not configured, batch scheduling does not take effect and normal scheduling is used.
  annotations:
    huawei.com/schedule_policy: multilevel # Configures the scheduling policy as multilevel scheduling policy
    huawei.com/affinity-config: level1=2,level2=4 # Configures the network group size for different levels according to the actual needs of the job
spec:
  schedulerName: volcano  # Takes effect when the Ascend Operator component's startup parameter enableGangScheduling is set to true
  runPolicy:
    schedulingPolicy:    # Takes effect when the Ascend Operator component's startup parameter enableGangScheduling is set to true
  ...
  replicaSpecs:
    Master:
      replicas: 1   # Number of job Master replicas, can only be 1
      restartPolicy: Never
      template:
        metadata:
          labels:
...
...
            ports:                     # Distributed training collective communication ports
              - containerPort: 2222         # Determined by user
                name: ascendjob-port        # Do not modify
            resources:
              limits:
                huawei.com/Ascend910: 8   # Number of chips requested by the job; for multilevel scheduling jobs, this must match the maximum number of NPUs per node
              requests:
                huawei.com/Ascend910: 8   # Must match the limits value
...
    Worker:
      replicas: 7   # Number of job worker replicas; configure according to actual requirements
      restartPolicy: Never
      template:
        metadata:
          labels:
...
If you need to configure CPU and memory resources, or modify the mount paths of training scripts and code, see the corresponding steps in [Configuring YAML](./03_full_npu_scheduling.md#configuring-yaml).

### Delivering a Job

For details, see [Delivering a Job](./03_full_npu_scheduling.md).

### Viewing Job Progress

For details, see [Viewing Job Progress](./03_full_npu_scheduling.md#viewing-job-progress).

### Viewing Job Scheduling Results

For details, see [Viewing Job Scheduling Results](./03_full_npu_scheduling.md#viewing-full-npu-scheduling-results).

### Deleting a Job

For details, see [Deleting a Job](./03_full_npu_scheduling.md#deleting-a-job).
