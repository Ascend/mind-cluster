...
>[!NOTE]
>This section describes elastic training based on the Resilience Controller component, which has reached the end of life. Related documents will be deleted on the 30th of September, 2026. For the latest elastic training capabilities, see [Elastic Training](../04_usage/04_resumable_training/01_solutions_principles.md#elastic-training).
...
- Ensure that the environment has a corresponding storage solution configured. For example, to use the network file system (NFS), perform operations as described in [Installing NFS](./02_common_operations.md#installing-nfs).
...
- If not installed, refer to the [Installation and Deployment](../05_developer_guide/00_installation_deployment/00_manual_installation/00_obtaining_software_packages.md) section for operations.
...
1. Modify the startup YAML of Ascend Device Plugin. The modifications are shown in bold below.
    ...
    ...
          ...
          - image: ascend-k8sdeviceplugin:v{version}
            ...
            command: [ "/bin/bash", "-c", "--"]
            args: [ "device-plugin
                     <strong>-volcanoType=true                    # Volcano must be used for rescheduling.</strong>
                     <strong>-autoStowing=true                    # This field is deprecated. It controls whether automatic management is enabled. The default value is true. Setting it to false disables automatic management. When automatic management is disabled, a chip will not be automatically added back to the schedulable resource pool after its health status changes from `unhealthy` to `healthy`. Similarly, when automatic management is disabled, a chip will not be automatically added back to the schedulable resource pool after a parameter-plane network fault is recovered. This feature only applies to <term>Atlas training products</term></strong>
...
2. Run the following command on the K8s management node to start Ascend Device Plugin.
    ...
    For example, to start the component on <term>Atlas training products</term>, run the following command:
...
Elastic training requires a training base image. You need to create it by referring to the [Creating an Image](./02_common_operations.md#creating-an-image) section based on the training framework you are using.
...
Obtain the software packages for the corresponding operating system, as well as the Dockerfile and script files required for image creation, as shown in [Table 1](#zh-cn_topic_0272789326_table13971125465512). In the name of the resumable training software package, `{version}` represents the version number.
...
5. Go to the [mindcluster-deploy](https://gitcode.com/Ascend/mindxdl-deploy) repository, switch to the corresponding branch according to the [mindcluster-deploy open-source repository version description](./05_appendix.md#mindcluster-deploy-open-source-repository-version-description), obtain the `train_start.sh`, `utils.sh`, and `rank_table.sh` files from the `samples/train/resumable-training/fault-rescheduling/withRanktable/pytorch/resnet50` directory, create a `scripts` directory in the training code, and construct the following directory structure on the management node.
...
3. Go to the [mindcluster-deploy](https://gitcode.com/Ascend/mindxdl-deploy) repository, enter the corresponding branch according to the [mindcluster-deploy open-source repository version description](./05_appendix.md#mindcluster-deploy-open-source-repository-version-description), obtain the `train_start.sh` and `main.sh` files from the `samples/train/resumable-training/fault-rescheduling/withRanktable/mindspore/resnet50` directory, and combine them with the `resnet/scripts` directory in the training code to construct the following directory structure on the management node.
...
5. Modify the configuration file `resnet50_imagenet2012_config.yaml` in the `/data/atlas_dls/public/code/resnet/config/` directory. Configure model and graph build saving and loading functions.
    ...
    has_trained_epoch: 0 # Epoch of model pre-training; defaulted to 0
    has_trained_step: 0 # Step of model pre-training; defaulted to 0
    ---
...
8. <a name="li12175938673"></a>Add the code for loading checkpoints in `train.py`. The following is a checkpoint loading example. Some checkpoint loading code already exists, and you need to add the checkpoint loading code related to the elastic training feature. The parameters used can be defined and set in the configuration file `src/utils.py` by referring to [Step 9](#li13178638874).
    ...
    # Add the following code before mindspore.set_auto_parallel_context. Refer to https://www.mindspore.cn/tutorials/experts/en/r2.0/index.html for usage instructions of the set_auto_parallel_context parameter
...
<table><thead align="left"><tr id="row15488613165116"><th class="cellrowborder" valign="top" width="12.98259651930386%" id="mcps1.2.6.1.1"><p id="p1694212055114"><a name="p1694212055114"></a><a name="p1694212055114"></a>Job Type</p>
</th>
<th class="cellrowborder" valign="top" width="21.754350870174033%" id="mcps1.2.6.1.2"><p id="p894252065116"><a name="p894252065116"></a><a name="p894252065116"></a>Model</p>
</th>
<th class="cellrowborder" valign="top" width="21.754350870174033%" id="mcps1.2.6.1.3"><p id="p1694216209514"><a name="p1694216209514"></a><a name="p1694216209514"></a>YAML File</p>
</th>
<th class="cellrowborder" valign="top" width="21.754350870174033%" id="mcps1.2.6.1.4"><p id="p8942122025113"><a name="p8942122025113"></a><a name="p8942122025113"></a>Link</p>
</th>
<th class="cellrowborder" valign="top" width="21.754350870174033%" id="mcps1.2.6.1.5"><p id="p7942172025115"><a name="p7942172025115"></a><a name="p7942172025115"></a>Description</p>
</th>
</tr>
</thead>
...
<td class="cellrowborder" valign="top" width="21.754350870174033%" headers="mcps1.2.6.1.5 "><p id="p1694222015119"><a name="p1694222015119"></a><a name="p1694222015119"></a>The default example is a single-node 8-processor job.</p>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.3 "><p id="p1561116674613"><a name="p1561116674613"></a><a name="p1561116674613"></a>The default example is a single-node single-processor job.</p>
</td>
</tr>
...
<td class="cellrowborder" valign="top" headers="mcps1.2.6.1.4 "><p id="p156115614615"><a name="p156115614615"></a><a name="p156115614615"></a>The default example is a 2-node 16-processor job.</p>
</td>
</tr>
</tbody>
</table>
...
Before performing specific operations, refer to [YAML Configuration](../06_api/15_yaml_configuration.md#yaml_configuration) to understand the parameter descriptions of the relevant YAML examples, and then proceed with the operations.
...
2. Run the following command on the management node to submit the training job using the YAML file.
    ...
    >[!NOTE]
    >If the job YAML is modified after the job is successfully submitted, you need to run the `kubectl delete -f XXX.yaml` command to delete the original job first, and then resubmit the job.
