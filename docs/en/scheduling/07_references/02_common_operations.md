...
Volcano supports native K8s scheduling and can use `nodeAffinity` for scheduling. The following example uses mandatory node affinity for scheduling. For more information about the `nodeAffinity` field, see the [official Kubernetes document](https://kubernetes.io/docs/tasks/configure-pod-container/assign-pods-nodes-using-node-affinity/).
...
- In the Volcano Job YAML, add the following fields in bold.
    ...
    ...
              nodeSelector:
                example-key: example-value    # Example value.Configure nodeSelector based on scheduling intent.
    ...</pre>
                      ...
                      <strong>- matchExpressions:</strong>
                        <strong>- key: aaa            # Match nodes with label key "aaa" and value "yyy"</strong>
                          ...
                          <strong>values:</strong>
                            <strong>- yyy</strong>
              nodeSelector:
                example-key: example-value    # Optional value. Configure nodeSelector based on actual requirements.
    ...</pre>
...
The query command is `kubectl describe cm -n volcano-system vcjob-fault-npu-cm`. The command output is as follows. For key parameterdescriptions, see [Table 2 vcjob-fault-npu-cm field description](../06_api/01_volcano.md#job-information).
...
Taking <term>Atlas A3 training products</term> as an example, the command output is as follows. The response parameters may vary for different devices, and the actual output shall prevail. For key parameter descriptions, see [Table 1 DeviceInfoCfg](../06_api/02_ascend_device_plugin.md#chip-resources).
...
Fault events collected by Ascend Device Plugin can be reported through K8s event events. The query command is `kubectl get events -n kube-system`. Taking <term>Atlas training products</term> as an example, the command output is as follows. For parameter descriptions, see [Table 1](#table66076214393).
...
Taking the <term>Atlas A3 training products</term> as an example, the command output is as follows. The response parameters may vary for different devices, and the actual output shall prevail. For key parameter descriptions, see [Table 1 cluster-info-node-cm](../06_api/04_clusterd/00_cluster_resources.md#configmap-description).
...
Taking the <term>Atlas A3 training products</term> as an example, the command output is as follows. The response parameters may vary for different devices, and the actual output shall prevail. For key parameter descriptions, see [Table 2 cluster-info-device-$\{m\}](../06_api/04_clusterd/00_cluster_resources.md#configmap-description).
...
Taking <term>Atlas A3 training products</term> as an example, the command output is as follows. The response parameters may vary for different devices, and the actual output shall prevail. For key parameter descriptions, see [Table 1](#table9246232250).

```ColdFusion
{"FaultCode":[000001c1],"FaultLevel":"NotHandle","UpdateTime":1722845555,"NodeStatus":"Healthy"}
```

**Table 1** UnifiedBus device fault parameter description
...
The query command is `kubectl describe cm mindx-dl-nodeinfo-<nodename> -n mindx-dl`. A command output is shown below. For key parameter descriptions, see [Table 1 mindx-dl-nodeinfo-_<nodename\>_](../06_api/03_noded.md#node-resources).
...
|<p>torch_npu-<em>v{version}</em><em>.</em>post<em>{version}</em>-cp3x-cp3x-manylinux_2_17_<em>{arch}</em>.manylinux2014_<em>{arch}</em>.whl</p>|<p><span>Ascend Extension for PyTorch</span> plugin.</p><p>cp3x in the package name indicates the Python version. For example, x being 10 indicates Python 3.10.</p>|<p>[Download Link](https://www.hiascend.com/document/detail/en/Pytorch/600/configandinstg/instg/insg_0001.html)</p><ul><li>Select a TorchNPU version that is compatible with <span>PyTorch</span>.</li><li>If using <span>PyTorch</span> models from the MindSpeed-LLM repository, <span>Ascend Extension for PyTorch</span> 2.1.0 or later is required.</li></ul>|
...
>[!NOTE]
>The MindSpore package and software used in <term>Atlas training products</term> must meet the corresponding compatibility requirements. See [MindSpore Installation Guide](https://www.mindspore.cn/install/en) for details.
...
9. (Optional) Verify that the base image is available.
    ...
    2. Check whether the MindSpore software in the base image is installed successfully.
        ...
        The command output is as follows, indicating that MindSpore is installed successfully.
...
## Integrating UMDK into the Container Base Image<a name="ZH-CN_TOPIC_0000002516255288"></a>

1. From the archive directory of the [Huawei Cloud Mirror Repository](https://mirrors.huaweicloud.com/ascend/), download the UMDK package in the corresponding format as needed, taking `umdk-urma-*.aarch64.rpm.tar.gz` as an example.
2. Log in to the server as the `root` user.
3. Create the `/home/umdk` directory and upload the UMDK package to this directory.

   ```shell
   mkdir -p /home/umdk
   ```

4. Go to the `/home/umdk` directory and perform the following steps to prepare a Dockerfile.
    1. Run the following command to create a Dockerfile (example filename: "Dockerfile").

        ```shell
        vi Dockerfile
        ```

    2. Add the relevant instructions for UMDK installation in the file.
       - openEuler container base image, using `openEuler 24.03-lts` as an example.
       ...
       - Ubuntu container base image, using `Ubuntu 24.04` as an example.
    ...
    3. After writing the new content to the Dockerfile, execute the `:wq` command to save the content.

5. Go to the directory where the Dockerfile is located and execute the image build command to build the image. Note that the dot (".") at the end of the command should not be omitted.

    ```shell
    docker build [OPTIONS] -t image_name_system_architecture:image_tag --build-arg UMDK_PKG=UMDK package name .
    ```

   The command is explained in the following table.
   ...
   **Table 1** Command parameter description
   ...
   | Name | Description |
   |--|---------------------------|
   | <em>OPTIONS</em> | "--no-cache" option: rebuild the image without using cache. |
   | -t | Image name.|
   | image_name_system_architecture:image_tag | Image name and tag. Fill in according to the actual situation. |
   | --build-arg UMDK_PKG | UMDK package name |
   ...
   When "Successfully built xxx" appears, it indicates that the image has been built successfully.
6. After the build is complete, run the following command to view the image information.
