...
- For newly installed devices, create a separate partition to mount the `"/var/lib/docker"` directory. Refer to [Operating System Drive Partitioning](../03_installation_guide/01_environment_dependencies.md).
...
3. <a name="li463812451313"></a>Create a new folder named `conf` under path A, create a file named `nginx.conf` in that folder, and write the following content into the file:
    ...
    ```conf
       ...
       http {
        ...
        server {
         listen <ClusterD pod IP>:9500 ssl;  # The Pod IP address of ClusterD. The port should be consistent with the port in the ClusterD configuration file. If it is an IPv6 address, configure it as [<ClusterD pod IP>]:9500 ssl;
         http2 on;
         ...
         location / {
          grpc_pass grpc://<ClusterD pod IP>:8899;                    # The Pod IP address of ClusterD. If the `useProxy` startup parameter of ClusterD is enabled, the IP address here is 127.0.0.1. If it is an IPv6 address, configure it as `grpc://[<ClusterD pod IP>]:8899;`
           }
        }
       }
    ```
...
4. Modify or add the following bolded fields in the ClusterD startup YAML file.
    ...
    # Add to the volumes section in Deployment
               ...
               <strong>- name: nginx-conf</strong>
                 <strong>hostPath:</strong>
                   <strong>path: /{PathA}/config       # nginx startup configuration file. Replace Path A with the file path from Step 2</strong>
                   <br>
        ...
        ```conf
           ...
           http {
            ...
            server {
             ...
             location / {
              grpc_pass grpcs://<ClusterD service IP>:9500;                    # Service IP address of ClusterD, which can be queried using the following command: kubectl get svc -A | grep clusterd. If it is an IPv6 address, configure it as grpcs://[<ClusterD service IP>]:9500;
              grpc_ssl_verify on;
    ...
    http {
     ...
     server {
      listen 127.0.0.1:8899;
                    http2 on;
      location / {
       grpc_pass grpcs://{ClusterD Pod IP}:9500;      # If it is an IPv6 address, configure it as grpcs://[{Pod IP of ClusterD}]:9500;
       grpc_ssl_verify on;
     ...
     server {
      listen {master Pod IP}:9601 ssl;                # If it is an IPv6 address, configure it as [{IP of the master Pod}]:9601 ssl;
      proxy_ssl_session_reuse off;
      http2 on;
...
5. Inject the environment variable for using the local proxy in the job YAML.
...
## Health Probe Security Hardening<a name="ZH-CN_TOPIC_0000002511346467"></a>

Ascend Device Plugin, Ascend Operator, ClusterD, Infer Operator, NodeD, and NPU Exporter have a built-in HTTP health probe service, which is used by the K8s livenessProbe mechanism to detect the component's liveness status. The HTTP protocol is used by default.

>[!NOTE]
>Kubelet **does not validate the server certificate** when executing HTTPS probes. Therefore, enabling HTTPS probes alone provides no additional security gain for K8s probe scenarios. It is only necessary to configure HTTPS for the probe when external systems need to call the health probe interface and require server identity verification (for example, third-party monitoring systems or custom health check scripts).

**Applicable Scenarios**

- External monitoring systems call the component health interface via HTTPS and need to verify the validity of the server certificate.
- Security compliance requires that all HTTP services must use encrypted communication.

If the above scenarios do not apply, the default HTTP configuration is sufficient and the following operations are not required.

**Configuration Steps**

1. Prepare the certificate and private key.

   Users must generate or obtain the TLS certificate and private key files themselves. The certificate SAN must contain the domain name or IP address of the target node. The certificate and private key are managed and periodically renewed by the user; MindCluster does not provide certificate generation or automatic rotation capabilities.
   >[!NOTICE]
   >
   >- The private key is stored in **plaintext** in the K8s Secret, posing a risk of leakage. It is recommended to strictly restrict access to the Secret using K8s RBAC.
   >- After the certificate expires, the probe service will become unavailable. The Secret must be manually updated and the Pod restarted. It is recommended to reserve sufficient time for replacement before the expiration date.
   >- Do not write the certificate and private key directly into the component YAML file or ConfigMap, to avoid leakage through version management tools.
   >- Kubelet's HTTPS probe does not validate the certificate. The liveness detection of the probe itself is not affected by certificate expiration or certificate mismatch, but external callers will be unable to verify the server identity due to certificate issues.

2. Create a K8s Secret.

   Store the certificate and private key in a Secret:
...
3. Modify the component deployment YAML.

    Make the following modifications in the component deployment YAML:

    - Mount the Secret into the container (`volumeMounts` + `volumes`).
    - Append `--tls-cert-file` and `--tls-private-key-file` to the container startup arguments.
    - Change `livenessProbe.httpGet.scheme` to `HTTPS`.

    A complete example is as follows (using ClusterD as an example, appending the TLS parameters at the end of the original startup arguments):
    ...
    The default ports for each component are as follows:
...
- Use `sed` to modify directly:
    ...
    **sed -i 's/^HISTSIZE=**_number_**/HISTSIZE=**_newNumber_**/' /etc/profile**, where *number* represents the number of commands before modification, and *newNumber* represents the number of commands after modification. For example, changing the number of saved commands from 1,000 to 200:
