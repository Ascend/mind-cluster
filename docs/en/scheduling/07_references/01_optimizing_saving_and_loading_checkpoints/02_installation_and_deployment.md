...
- When the [MindIO TFT](../00_fault_recovery_acceleration/01_product_description.md) framework is saving a checkpoint to MindIO ACP, if the checkpoint save fails, the checkpoint currently being saved cannot be used as a training recovery point. The training framework needs to recover from the last complete checkpoint.
...
To prevent software packages from being maliciously tampered with during transmission or storage, integrity verification must be performed when downloading software packages. For the verification process, refer to [Software Package SUM Value Verification](../../05_developer_guide/00_installation_deployment/00_manual_installation/00_obtaining_software_packages.md#section51703441649).
...
5. Change the permissions of executable files and code scripts in the software installation directory to `550` to prevent unauthorized tampering.
    ...
    ```bash
    chmod -R 550 {MindIO_ACP_SDK_installation_directory}
    ```
...
1. Change the permissions of executable files and code scripts in the software installation directory to 750.
    ...
    ```bash
    chmod -R 750 {MindIO_ACP_SDK_installation_directory}
    ```
