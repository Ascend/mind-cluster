...
1. 拉取mind-cluster整体源码，例如放在/home目录下：
        ...
        git clone -b branch_v26.1.0 https://gitcode.com/Ascend/mind-cluster.git

2. 修改组件版本配置文件 `build/service_config.ini`（相对源码根目录）中mind-cluster-version字段值为所需编译版本，默认值如下：
...
1. 参考以下命令，分别在对应目录下手动拉取Volcano各版本分支的官方开源代码。
    ...
    > 注：以上拉取的均为 release 分支（如 `release-1.12`、`release-1.9`、`release-1.7`），而非 tag。

    > 注：国内网络从 github 克隆 volcano 限速严重。可改用 gitee 镜像加速：
    >
    > `git clone -b release-1.9 https://gitee.com/mirrors/volcano.git`（1.7 对应 `-b release-1.7`，1.12 对应 `-b release-1.12`）。
